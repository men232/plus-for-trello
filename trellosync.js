    ...
    function okTransaction() {
        insertPendingSERows(function (responseInsertSE) {
            ...
            if (bMadeChanges) {
                ...
                localStorage.setItem("plus_first_trello_sync_completed", "true"); //if user later resets sync , we purposely dont restore this so a first sync happens again 
            }
...
function processTrelloActions(tokenTrello, alldata, actions, boards, hasBoardAccessDirect, sendResponseParam) {
    ...
    var bFirstSync = ((localStorage.getItem("plus_first_trello_sync_completed") || "") != "true");
    var iAction = -1;
...
function doSearchTrelloChanges(bUserInitiated, tokenTrello, idBoardsSearch, cDaysDelta, cCardsLimit, callback, waitRetry) {
    ...
    var dscTrello=localStorage.getItem('plus_trello_auth_dsc');
