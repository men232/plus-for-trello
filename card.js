...
function getUserLast(bUseLast) {
  ...
  if (!g_bUseLastSEBarUser || !bUseLast) return Promise.resolve(val);
  return new Promise(function (resolve, reject) {
    chrome.storage.sync.get([SYNCPROP_USERSEBAR_LAST], function (obj) {
      if (chrome.runtime.lastError) console.log(chrome.runtime.lastError.message); //eat it and default to "" (me)
      else val = obj[SYNCPROP_USERSEBAR_LAST] || "";
      resolve(val);
    });
  });
}

function getSeCurForUser(user, keyword) {
  //note returns null when not loaded yet
  assert(user);
  if (g_seCardCur === null) return null;
...
function updateEOnSChange(cRetry) {
  ...
  setTimeout(function () {
    ...
    if (bHilite) hiliteOnce(spinE, 500);
    updateNoteR();
  }, 1);
}
...
function updateNoteR() {
  ...
  if (statusPre.length == 0 || spinS.length == 0 || spinE.length == 0 || userElem.length == 0) return;

  var userCur = getUserFromCombo(userElem);
  if (!userCur) return; //user not loaded yet
  var keyword = comboKeywords.val() || "";
  var mapSe = getSeCurForUser(userCur, keyword);
  if (mapSe == null) return; // table not loaded yet. this will be called when table loads

  var strLinkHelp =
    "&nbsp;&nbsp;<a class='agile_linkSoftColor no-print agile_unselectable' href='' target='_blank'>Help</a>";
  var sRaw = spinS.val();
  var eRaw = spinE.val();
  ...
  else statusPre.closest("tr").addClass("agile_SER_normal").removeClass("agile_SER_negative");
  ...
  statusR.html(
    g_bNoEst
      ...
      : "&nbsp;&nbsp;R=" +
          rDiffFormatted +
          (g_bAllowNegativeRemaining || rDiff != 0 ? "" : ". Increase E if not done.") +
          strLinkHelp
  );
  done();
}
...
function updateCurrentSEData(bForceNow) {
  ...
  function worker() {
    ...
    if (valUser == g_strUserOtherOption || valDays == g_strDateOtherOption) return;
...
function getUserFromCombo(combo) {
  ...
  if (userCur == g_strUserMeOption) userCur = getCurrentTrelloUser();
  return userCur || ""; //prevent null
}

function isRecurringCard() {
  var elemTitle = $(".card-detail-title-assist");
  if (elemTitle.length == 0) return false; //no longer in card window. just pretend not recurring
...
function fillComboKeywords(comboKeywords, rg, kwSelected, classItem, strPrependNonDisabled, bNoPrependKWHeader) {
  function add(elem, kwSelected) {
    ...
    if (!disabled && strPrependNonDisabled) str = strPrependNonDisabled + str;
    var elemOption;
    if (disabled) elemOption = $('<optgroup label="' + str + '">');
    else {
      elemOption = $(new Option(str, val));
      if (val == kwSelected) elemOption[0].selected = true;
    }

    if (classItem) elemOption.addClass(classItem);
    if (title) elemOption.attr("title", title);
    comboKeywords.append(elemOption);
  }

  comboKeywords.empty();
  if (!bNoPrependKWHeader) comboKeywords.append($("<optgroup label='keyword:'></optgroup>"));
...
function fillComboUsers(bUseLast, comboUsers, userSelected, idCard, nameBoard, bDontEmpty, callbackParam) {
  getUserLast(bUseLast).then((userLast) =>
    fillComboUsersWorker(comboUsers, userSelected || userLast, idCard, nameBoard, bDontEmpty, callbackParam)
  );
}

function fillComboUsersWorker(comboUsers, userSelected, idCard, nameBoard, bDontEmpty, callbackParam) {
  function callback(status) {
    if (status != STATUS_OK) sendDesktopNotification(status);
  ...
  getSQLReport(sql, [], function (response) {
    var map = {};
    function add(user) {
      if (g_rgExcludedUsers.indexOf(user) >= 0) return;
      var opt = new Option(user, user);
      if (user == userSelected) opt.selected = true;
    ...
    if (response.status == STATUS_OK) {
      ...
      for (var i = 0; i < response.rows.length; i++) {
        ...
        mapUsers[user] = true;
        if (user == userGlobal) userGlobal = "";
        if (user == g_valUserExtra) g_valUserExtra = null;
        if (user == userMe) continue;
        add(user);
      }
      ...
      FindIdBoardFromBoardName(nameBoard, idCard, function (idBoardFound) {
        ...
        getTrelloBoardMembers(idBoardFound, 1000 * 60 * 2, function (members) {
          for (var i = 0; i < members.length; i++) {
            ...
            if (!member || !member.username || mapUsers[member.username] || member.username == userMe) continue;
            add(member.username);
          }
          if (userGlobal) add(userGlobal);
          if (g_valUserExtra) add(g_valUserExtra);
...
function createSEButton(parent) {
  if (parent.length == 1) {
    ...
    var icon = $("<img style='margin-top:2px;'>").attr("src", chrome.runtime.getURL("images/iconaddse.png"));
...
function showSEBarContainer(bDontRemember, bFocusS, bFocusE, bDontHilite) {
  ...
  if (!bDontRemember) g_bShowSEBar = true;
  if (bFocusS || bFocusE) {
    setTimeout(function () {
...
function fillDaysList(comboDays, cDaySelected) {
  ...
  function addItem(iDays, iDaysSelected) {
    ...
    var str = null;
    var title = "";
    if (iDays == g_strDateOtherOption) str = iDays;
    else if (iDays == 0) {
      str = g_strNowOption;
      title = "today";
    } else {
    ...
    if (bStrSelected) bSelected = str === iDaysSelected;
    if (bSelected) optAdd.selected = true;
  ...
  for (iDays = 1; iDays <= g_iLastComboDays; iDays++) addItem(iDays, cDaySelected);

  if (g_valDayExtra) addItem(g_valDayExtra, cDaySelected);
...
function promptNewUser(combo, idCardCur, callbackParam) {
  ...
  var userNew = prompt(
    "Enter the Trello username.\nThat member will see s/e only if is a board member.\n\nTo hide users from the s/e bar, see Plus Preferences.",
    userNew
  );
  if (userNew) userNew = userNew.trim().toLowerCase();
  if (userNew && userNew.indexOf("@") == 0) userNew = userNew.substring(1);
  if (userNew == g_strUserOtherOption) userNew = "";

  if (userNew) g_valUserExtra = userNew;
  ...
  fillComboUsers(false, combo, userNew, idCardCur, board, false, function (status) {
    ...
    if (
      userNew &&
      userNew.toLowerCase().indexOf(DEFAULTGLOBAL_USER.toLowerCase()) != 0 &&
      userNew.toLowerCase().indexOf(g_globalUser.toLowerCase()) != 0
    ) {
      if (!idCardCur) return; //shouldnt happen and no biggie if does
      board = getCurrentBoard();
      if (!board) return; //shouldnt happen and no biggie if does
...
function createCardSEInput(parentSEInput, idCardCur, board) {
  ...
  if (!g_bShowSEBar && !g_bAlwaysShowSEBar) containerBar.hide();
  ...
  var rowPad = $("<tr>")
    .addClass("agile-card-background")
    .append($("<td style='font-size:50%;'>&nbsp;</td>").addClass("agile_tablecellItem"));

  containerBar.append($('<tbody class="agile-card-background">')).append(row).append(rowStatus).append(rowPad);

  var comboUsers = setSmallFont(
    $('<select id="plusCardCommentUsers"></select>').addClass("agile_general_box_input agile_combo_input")
  );
  ...
  comboUsers.change(function () {
    ...
    if (val == g_strUserOtherOption) promptNewUser(combo, idCardCur);
    else rememberSEUser(val);
  });

  var comboDays = setSmallFont(
    $('<select id="plusCardCommentDays"></select>').addClass("agile_days_box_input agile_combo_input")
  );
  ...
  comboDays.change(function () {
    ...
    if (val == g_strDateOtherOption) {
      function process(dayNew) {
        if (dayNew) {
          if (dayNew > g_iLastComboDays) g_valDayExtra = dayNew;
        }
      ...
      getSEDate(function (dateIn) {
        if (!getIdCardFromUrl(document.URL)) return; //rare. user managed to close the card but not the date dialog.
        var date = 0;
        if (dateIn) date = getDeltaDates(dateNow, dateIn);
        process(date);
      });
    }
  });
  var spinS = setNormalFont(
    $('<input id="plusCardCommentSpent" placeholder="S" maxlength="10"></input>').addClass(
      "agile_spent_box_input agile_placeholder_small agile_text_input"
    )
  );
  spinS.attr("title", "Click to type Spent.\nIf needed, Plus will increase E (right) when your total S goes over E.");
  spinS[0].onkeypress = function (e) {
    validateSEKey(e);
    checkEnterKey(e);
  };
  //thanks for "input" http://stackoverflow.com/a/14029861/2213940
  spinS.bind("input", function (e) {
    updateEOnSChange();
  });
  var spinE = setNormalFont(
    $('<input id="plusCardCommentEstimate" placeholder="E" maxlength="10"></input>').addClass(
      "agile_estimation_box_input agile_placeholder_small agile_text_input"
    )
  );
  var spanSpinE = $("<span>");
  spanSpinE.append(spinE);
  spinE.attr("title", "Click to type Estimate.");
  spinE[0].onkeypress = function (e) {
    validateSEKey(e);
    checkEnterKey(e);
  };
  spinE.bind("input", function (e) {
    updateNoteR();
  });
  var slashSeparator = setSmallFont($("<span>").text("/"));
  var comment = setNormalFont(
    $('<input maxlength="250" name="Comment" placeholder="Comment"/>')
      .attr("id", "plusCardCommentComment")
      .addClass("agile_comment_box_input agile_placeholder_small agile_text_input")
  );
  ...
  spinS.focus(function () {
    $(this).select();
  });
  spinE.focus(function () {
    $(this).select();
  ...
  var spanIcon = $("<span />");
  var icon = $("<img>").attr("src", chrome.runtime.getURL("images/iconspent.png"));
  icon.attr("title", "Add S/E to this card. Use negative numbers to reduce.");
  icon.addClass("agile-spent-icon-cardcommentSE");
  spanIcon.append(icon);

  var buttonEnter = setSmallFont(
    $('<button id="plusCardCommentEnterButton"/>')
      .addClass("agile_enter_box_input")
      .addClass("agile_buton")
      .text("Enter")
  );
  ...
  if (g_optEnterSEByComment.IsEnabled()) {
    ...
    if (rgkeywords.length > 1) {
      bAppendKW = true;
      comboKeyword = setSmallFont(
        $('<select id="plusCardCommentKeyword"></select>').addClass("agile_general_box_input")
      );
      ...
      row.append(
        $("<td />")
          .addClass("agile_tablecellItem")
          .append($("<div>").addClass("agile_keywordsComboContainer").append(comboKeyword))
      );
  ...
  var tdStatusPre = $("<td  style='text-align: right;' colspan='" + (bAppendKW ? "3" : "2") + "'>").addClass(
    "agile_tablecellItem agile_tablecellItemStatus"
  );
  ...
  row.append(
    $("<td />")
      .addClass("agile_tablecellItem")
      .append($("<div>").addClass("agile_usersComboContainer").append(comboUsers))
  );
  ...
  function doEnter() {
    testExtension(function () {
      ...
      var keyword = null;
      if (comboKeyword) keyword = comboKeyword.val() || "";
      ...
      if (!userCur) return; //shouldnt happen but for safety
      ...
      if (!verifyValidInput(sTotal, eTotal)) return;
      ...
      function onFinished(bOK) {
        ...
        if (bOK) {
          if (idCardCur == getIdCardFromUrl(document.URL)) {
            if (!g_bUseLastSEBarUser) comboUsers.val(g_strUserMeOption); //if we dont reset it, a future timer could end up in the wrong user
  ...
  comment.bind("input", function (e) {
    updateCurrentSEData();
  });
  parentSEInput.before(container);
  fillCardSEStats(tableStats, function () {
    container.show();
    if (!g_bNoSE) createSEButton(parentSEInput);
    insertCardTimer();
    g_currentCardSEData.loadFromStorage(idCardCur, function () {
      if (g_currentCardSEData.idCard != idCardCur) return; //timing. should never happen but just in case

      if (!g_currentCardSEData.s && !g_currentCardSEData.e && !g_currentCardSEData.note) return;
      var bFocus = false;
      ...
      if (comboKeyword) set(comboKeyword, g_currentCardSEData.keyword, true);
      updateNoteR();
      if (bFocus) {
        if (g_currentCardSEData.s != 0 || g_currentCardSEData.e != 0) {
          if (!g_timerStatus || !g_timerStatus.bRunning || g_timerStatus.idCard != idCardCur) {
            ...
            sendDesktopNotification(
              "Card has a draft s/e row (" +
                strWhen +
                ")\n• Click 'Enter', or\n• Click the timer to unpause it, or\n• Clear the bar to forget it.",
              15000
            );
          }
        }
...
function getTrelloBoardMembers(idShortBoard, msCacheMax, callback) {
  ...
  sendExtensionMessage(
    {
      method: "getTrelloBoardData",
      tokenTrello: null,
      idBoard: idShortBoard,
      fields: "memberships&memberships_member=true&memberships_member_fields=username",
    },
    function (response) {
...
function verifyBoardMember(userLowercase, idShortBoard, callbackNotFound, callbackFound) {
  ...
  getTrelloBoardMembers(idShortBoard, 0, function (members) {
    for (var i = 0; i < members.length; i++) {
      var member = members[i].member;
      if (member && member.username && member.username.toLowerCase() == userLowercase) break; //found
    }
    if (i == members.length) callbackNotFound();
    else if (callbackFound) callbackFound();
  });
}
...
<button id="agile_dialog_SEDate_cancel">Cancel</button> \
    ...
    elemDate.change(function () {
      ...
      if (dateCur) {
        ...
        if (delta > 0) strMsg = "" + delta + (delta == 1 ? " day" : " days") + " ago.";
      }
  ...
  divDialog
    .find("#agile_dialog_SEDate_cancel")
  ...
  divDialog
    ...
    .on("click.plusForTrello", function (e) {
      ...
      if (dateCur) {
        var delta = getDeltaDates(new Date(), dateCur);
        if (delta >= 0 && delta <= -1 * g_dDaysMinimum) bOK = true;
      }
...
function verifyValidInput(sTotal, eTotal) {
  ...
  var err = null;
  if (sTotal < 0) err = "Spent total will go negative.";
  else if (!g_bNoEst) {
    if (eTotal < 0) err = "Estimate total will go negative.";
    else if (rTotal < 0 && !g_bAllowNegativeRemaining)
      err =
        "Spent total will be larger than estimate total.\nTo avoid this see Plus Preferences 'Allow negative Remaining'";
  }
  if (err != null) {
    err = err + "\n\nAre you sure you want to enter this S/E row?";
    if (!confirm(err)) return false;
  }
  return true;
}
...
function fillCardSEStats(tableStats, callback) {
  ...
  if (!idCard || userCur == null) return; //ignore

  getInitialCardBalances(idCard, function (mapEstOrig) {
    var sql =
      "select '' as keyword, CB.idCard, CB.user, CB.spent, CB.est, CB.date \
    ...
    if (g_optEnterSEByComment.IsEnabled() && g_optEnterSEByComment.rgKeywords.length > 0) {
      sql =
        "select H.keyword, H.idCard, H.user, SUM(H.spent) as spent, SUM(H.est) as est, MAX(H.date) as date FROM HISTORY AS H WHERE H.idCard=? \
    ...
    getSQLReport(sql, values, function (response) {
      ...
      if (response.status == STATUS_OK && (response.rows.length > 0 || (isTourRunning() && !g_bNoSE))) {
        if (!g_bNoSE) containerStats.show();
        if (elemRptLink.length == 0) {
          estimateBadge = BadgeFactory.makeEstimateBadge()
            .addClass("agile_badge_cardfront")
            .attr("title", "E sum\nall users");
          spentBadge = BadgeFactory.makeSpentBadge()
            .addClass("agile_badge_cardfront agile_badge_cardfrontFirst")
            .attr("title", "S sum\nall users");
          remainBadge = BadgeFactory.makeRemainingBadge()
            .addClass("agile_badge_cardfront")
            .attr("title", "R sum\nall users");

          var elemTransferE = $(
            '<a class="agile_linkSoftColor no-print agile_unselectable" href="" target="_blank" title="Transfer estimates between users">transfer E</a>'
          );
          if (g_bNoEst) {
            elemTransferE.hide();
            estimateBadge.hide();
          ...
          containerStats.prepend(
            $(
              '<a class="agile_card_report_link agile_linkSoftColor no-print" href="' +
                chrome.runtime.getURL("report.html?chartView=s&idCard=") +
                encodeURIComponent(idCard) +
                '" target="_blank" title="Open a detailed S/E rows report">Report</a>'
            )
          );
        ...
        if (isTourRunning() && !g_bNoSE)
          addDataRow({
            //ensure at least one row exists and it has est != estOrig
            user: "sample user",
            spent: 6,
            estOrig: 11,
            est: 15,
...
<span class="agile_lightMessage">Use "Modify" or use the "S/E bar" ?<br>Modify a 1ˢᵗ estimate ? See </span> <button class="agile_modify_help agile_buton" style="display:inline-block;"  href="">help</button> \
    ...
    divDialog.find(".agile_mtse_s")[0].onkeypress = function (e) {
      validateSEKey(e);
    };
    divDialog.find(".agile_mtse_e")[0].onkeypress = function (e) {
      validateSEKey(e);
    };
    divDialog.find(".agile_mtse_r")[0].onkeypress = function (e) {
      validateSEKey(e);
    };
  ...
  if (bHideComboKeyword) {
    ...
    elemKwLink.attr(
      ...
      chrome.runtime.getURL(
        "report.html?chartView=s&idCard=" +
          encodeURIComponent(idCard) +
          "&orderBy=keyword&user=" +
          user +
          "&sortList=%5B%5B%22Keyword%22%2C0%5D%2C%5B%22Date%22%2C1%5D%5D"
      )
    );
  }
  ...
  var strMessageInitial =
    "Plus will enter a new S/E row on the given date, with the needed values to make your new totals.";
  ...
  function updateEfromS() {
    ...
    if (sNewNum <= sOrigNum) {
      ...
      if (isRecurringCard()) valESet = sNew;
      else {
        if (sNewNum > eOrigNum) {
          if (g_bPreventIncreasedE) bUpdateRInstead = true;
          else valESet = sNew;
        } else {
          bUpdateRInstead = true;
        }
    ...
    if (bUpdateRInstead) updateMessage(true);
    else {
      if (valESet !== null) {
        elemE.val(valESet);
        hiliteOnce(elemE);
  ...
  function updateMessage(bUpdateR) {
    ...
    if (data.s == 0 && data.e == 0) strMessageNew = strMessageInitial;
    else strMessageNew = "" + data.s + "/" + data.e + " will be added to " + user + " in a new S/E row.";
  ...
  function getSEData(bSilent) {
    ...
    if (!bSilent && !verifyValidInput(sNewNum, eNewNum)) return null;
    var kw = null;
    if (!bHideComboKeyword) kw = comboKeyword.val() || "";
  ...
  function doEnter() {
    ...
    setNewCommentInCard(
      ...
      data.e,
      note,
      data.prefix, //empty prefix means time:now
      user,
      null,
      onBeforeStartCommit,
      onFinished
  ...
  divDialog
    .find("#agile_modify_SETotal")
    .off("click.plusForTrello")
    .on("click.plusForTrello", function (e) {
      doEnter();
    });

  elemS.unbind("input").bind("input", function (e) {
    updateEfromS();
  });
  elemE.unbind("input").bind("input", function (e) {
    updateMessage(true);
  });
  elemR.unbind("input").bind("input", function (e) {
    updateEFromR();
    updateMessage(false);
  });
...
function addCardSERowData(tableStats, rowData, bHeader) {
  ...
  if (bHeader) row.addClass("agile-card-background-header");
  else row.addClass("agile-card-statrow-data");
  ...
  if (bHeader) {
    ...
    var urlReport =
      '<a class="agile_link_noUnderline" href="' +
      chrome.runtime.getURL("report.html?chartView=s&idCard=") +
      encodeURIComponent(idCardCur) +
      "&user=" +
      rowData.user +
      '" target="_blank">' +
      rowData.user +
  ...
  if (bHeader) {
    ...
    if (eVal != eOrigVal) eValDisplay = eVal + " (" + eOrigVal + ")";
    s.text(sVal);
    e.text(eValDisplay);
    r.text(rVal);
    if (true) {
  ...
  if (!bHeader) {
    if (bSample) {
      ...
      u.on("mousemove", function (e) {
        ...
        if (msdateNow - msdateCalc < 3000) return true; //not handled

        msdateCalc = msdateNow;
        var maxRows = 10;
        var maxNote = 50;
        var sql =
          "select H.keyword, H.spent, H.est, H.date, H.week, h.comment FROM HISTORY AS H WHERE H.idCard=? and user=? order by date DESC, rowid DESC LIMIT " +
          (maxRows + 1);
        var values = [idCardCur, rowData.user];

        getSQLReport(sql, values, function (response) {
          var strTitle = "";
          var bMultipleKeywords =
...
function parseSEInput(ctl, bHiliteError, bExact, bDontZeroNan) {
  if (bHiliteError === undefined) bHiliteError = true;
  if (bHiliteError) ctl.removeClass("agile_box_input_hilite");
  var val = (ctl[0].value || "").trim();
  var retVal;
  if (val.indexOf(":") < 0) {
    if (val.length == 0) retVal = 0;
    else retVal = parseFixedFloat(val, bDontZeroNan);
    if (bDontZeroNan && isNaN(retVal)) retVal = null;
  } else retVal = parseColonFormatSE(val, bExact);
  if (retVal === null) {
    if (bHiliteError) ctl.addClass("agile_box_input_hilite");
    return null; //invalid
  }
  return retVal;
}
...
function recalcChecklistTotals() {
  ...
  if (elemAfter.length == 0) return;
  elemAfter = elemAfter.parent();
  if (seChecks.length == 0) {
    seChecks = $(
      "<div style='cursor:default;margin-left:1em;' class='agile_se_checks' title='This S/E is not included in card totals. It is shown only as an aid.'>"
    );
  ...
  if (estimateBadge.length == 0) {
    ...
    if (g_bNoEst) estimateBadge.hide();
...
function addCardCommentHelp() {
  if (!g_bReadGlobalConfig) return; //wait til later
  ...
  if (!isSEInputInitialized) {
    ...
    if (board == null) return; //wait til later

    var idCardCur = getIdCardFromUrl(document.URL);
    if (!idCardCur) return; //timing issue, really shouldnt happen as we found card elements above
...
function createRecurringCheck() {
  var check = $(
    '<input class="agile_linkSoftColor" id="agile_checkRecurringCard" style="vertical-align:middle;margin-bottom:0px;display:inline;" type="checkbox"  />'
  );
  var icon = $("<img>")
    .attr("src", chrome.runtime.getURL("images/recurring.png"))
    .addClass("agile-card-icon-recurring");
  ...
  span.append(
    $(
      '<label style="display:inline;margin-right:1.5em;font-weight:normal" for="agile_checkRecurringCard" class="agile_unselectable agile_linkSoftColor">Recurring</label>'
    )
  );
  ...
  check.click(function () {
    ...
    var bIsRecurring = titleCur.indexOf(TAG_RECURRING_CARD) >= 0;
    if ((bIsRecurring && bChecked) || (!bIsRecurring && !bChecked)) return;
    if (bChecked) titleCur = titleCur + " " + TAG_RECURRING_CARD;
    else {
  ...
  if (g_bNoEst) span.hide();
  return span;
}

function updateRecurringCardImage(bRecurring, icon) {
  if (!icon) icon = $(".agile-card-icon-recurring");
  if (bRecurring) icon.show();
  else icon.hide();
}
...
function createSEMenu(divParent) {
  ...
  if (!g_bNoSE) {
    ...
    if (!g_bNoEst) {
      comboSE.append(
        $(new Option("add Estimate", "E")).prop(
          "title",
          "Create or add Estimate for a user.\nuser's Estimate = sum of all 'E' entries."
        )
      );
      comboSE.append(
        $(new Option("transfer Estimate", "TE")).prop(
          "title",
          'transfer "E 1ˢᵗ" between users.\n\
Useful to transfer from a global estimate to a specific user.'
  ...
  function comboUpdateView() {
    var scombo = comboSE.select2({
      minimumResultsForSearch: Infinity,
      placeholder: (g_bNoSE ? "" : "S/E & ") + "More",
    ...
    chrome.storage.sync.get([SYNCPROP_bShowedFeatureSEButton], function (obj) {
      if (!obj[SYNCPROP_bShowedFeatureSEButton]) {
        ...
        chrome.storage.sync.set(obj, function () {
          if (chrome.runtime.lastError !== undefined) return; //just to reference it so chrome debugger doesnt complain
        });
      }
    });
  }
...
function createHashtagsList(divParent) {
  ...
  comboK.on("change", function () {
    ...
    if (val == txtOther) {
      ...
      if (iHash == 0) newHash = newHash.substring(1);
      else if (iHash > 0) {
      ...
      if (
        !listCached.every(function (k) {
          if (k.toLowerCase() == newHash.toLowerCase()) return false;
          return true;
        })
      ) {
      ...
      if (
        listCached.every(function (k) {
          if (k.toLowerCase() == val.toLowerCase()) return false;
          return true;
        })
      ) {
        listCached.push(val);
...
function checkNeedHiliteTimer() {
  chrome.storage.local.get(LOCALPROP_bHiliteTimerOnOpenCard, function (obj) {
    ...
    if (value) g_cHilitesTimer = 2;
    else g_cHilitesTimer = 0;
    var pair = {};
...
function updateTimerTooltip(timerElem, bRunning, bRemoveSmallTimerHelp, bUpdateCards) {
  ...
  if (bRunning) {
    if (!bReplaceClass) timerElem.addClass(strClassRunning);
    else timerElem.removeClass(strClassRunning);
  ...
  if (bReplaceClass) timerElem.addClass("agile_timer_hilite");
  else timerElem.removeClass("agile_timer_hilite");
...
function clearTimerInterval(timerStatus) {
  if (timerStatus && (timerStatus.bRunning || timerStatus.idInterval)) {
    ...
    if (timerStatus.idInterval) clearInterval(timerStatus.idInterval);
...
function handleCardTimerClick(msDateClick, hash, timerElem, timerStatus, idCard, ptClick) {
  chrome.storage.sync.get([SYNCPROP_ACTIVETIMER], function (objActiveTimer) {
    ...
    getCardTimerData(hash, function (obj) {
      //get it again in case it changed from another device
      ...
      if (
        ...
        if (elemSpent.length == 1) {
          ...
          if (sCur != null) {
            if (parseFixedFloat(g_sTimerLastAdd) == sCur) sCur = g_sTimerLastAdd;
        ...
        clearTimerInterval(timerStatus);
        chrome.storage.sync.set(objNew, function () {
          if (chrome.runtime.lastError !== undefined) return;
          timerStatus.bRunning = true;
          updateTimerTooltip(timerElem, timerStatus.bRunning, false, true);
        ...
        if (bRemoveActive) rgRemove.push(SYNCPROP_ACTIVETIMER);
        ...
        chrome.storage.sync.remove(rgRemove, function () {
          ...
          if (sUse != 0) {
            ...
            sendDesktopNotification(
              "Ellapsed time too small (under 0.01 " + UNITS.getLongFormat() + "). Timer ignored\n.",
              10000
            );
          }
          if (bRemoveActive) findNextActiveTimer();
        });
      }
    });
  });
...
function clearBlinkButtonInterval() {
  if (g_intervalBlinkButton != null) clearInterval(g_intervalBlinkButton);
  g_cBlinkButton = 0;
}
...
function addSEFieldValues(s) {
  ...
  g_intervalBlinkButton = setInterval(function () {
    ...
    if (elemEnter.hasClass(classBlink)) elemEnter.removeClass(classBlink);
    else {
      elemEnter.addClass(classBlink);
      if (g_cBlinkButton > 1)
        //do it here so it remains yellow
        clearBlinkButtonInterval();
    }
  }, 500);
}
...
function setNewCommentInCard(
  idCardCur,
  keywordUse, //blank uses default (first) keyword
  s,
  e,
  commentBox,
  ...
  //called after commit finished or failed. onFinished(bOK)
  if (prefix == g_strNowOption || prefix == null) prefix = "";
  var comment = "";
  var prefixComment = ""; //for transfer command
  if (!keywordUse) keywordUse = g_optEnterSEByComment.getDefaultKeyword();
  ...
  if (memberTransferTo) {
    ...
    if (member == g_strUserMeOption) member = null; //defaults later to user
    if (member && member != getCurrentTrelloUser()) comment = comment + "@" + member + " ";
    if (prefix.length > 0) comment = comment + " " + prefix + " ";
  }
  ...
  if (!idCardCur) return; //should never happen
...
function HandleNoBackendDbEntry(
  s,
  e,
  commentBox,
  idBoard,
  ...
  var rgUsers = [userCur];
  if (memberTransferTo != null) rgUsers.push(memberTransferTo); //must be there before the first call to appendCommentBracketInfo
...
function helperInsertHistoryRow(
  dateNow,
  idCard,
  idBoard,
  strBoard,
  ...
  if (rgUsers.length == 1) {
    ...
    if (idHistoryRowUse) idHistoryUse2 = idHistoryRowUse + SEP_IDHISTORY_MULTI + "1";
...
function doEnterSEIntoCard(
  ...
  if (elem.length == 0) return; //trello html broke.
  ...
  if (!IsStealthMode() && !memberTransferTo) {
    if (!g_optEnterSEByComment.IsEnabled() && g_configData && g_strServiceUrl && g_strServiceUrl != "") {
      ...
      if (se.bSFTFormat) titleCardNew = "(" + estimation + ") " + cleanTitle + " [" + spent + "]";
      else titleCardNew = "(" + spent + "/" + estimation + ") " + cleanTitle;
    } else {
      commentEnter = comment;
    }
  }

  handleEnterCardComment(
    titleCardNew,
    commentEnter,
    strBoard,
    idBoard,
...
function handleEnterCardComment(
  titleCard,
  comment,
  strBoardParam,
  idBoardParam,
  ...
  assert(onFinished);
  if (onBeforeStartCommit) onBeforeStartCommit();
  ...
  sendExtensionMessage({ method: "beginPauseSync" }, function (response) {
    if (IsStealthMode()) {
      ...
      addCardCommentByApi(idCard, comment, function (response) {
        ...
        if (!member) member = response.commentObj.memberCreator.username;
...
function addCardCommentByApi(idCard, comment, callback, waitRetry) {
  ...
  var url = "https://trello.com/1/cards/" + idCard + "/actions/comments";
  var payload = "text=" + encodeURIComponent(comment) + "&dsc=" + g_dscTrello;
...
function FindIdBoardFromBoardName(boardNameNew, idCard, callback) {
  ...
  function getFromTrelloApi(callbackFind) {
    ...
    sendExtensionMessage(
      { method: "getTrelloCardData", tokenTrello: null, idCard: idCard, fields: "id", bBoardShortLink: true },
      function (response) {
        if (response.status != STATUS_OK || !response.card || !response.card.board || !response.card.board.shortLink)
          callbackFind(null);
        else callbackFind(response.card.board.shortLink);
      }
    );
  }
      ...
      getFromTrelloApi(function (idFound) {
        if (idFound != null) {
          ...
          getFromCardSql(function (idFound) {
            if (idFound != null) callback(idFound);
            else getFromBoardNameSql(callback);
          });
        }
      });
    } else {
  ...
  getSQLReport(sql, values, function (response) {
    if (response.rows && response.rows.length == 1 && response.rows[0].name) {
      ...
      helperInsertHistoryRow(
        ...
        0,
        0,
        ["Plus: card moved from '" + boardCur + "'"],
        undefined,
        undefined,
        function (status) {
          if (status == STATUS_OK) sendDesktopNotification("Plus has moved the card's data to the new board.", 8000);
        }
      );
    }
  });
...
<img style="float:right;cursor:pointer;" id="agile_dialog_SEHelp_OK" src="' +
...
<div class="agile_sehelpsection" id="agile_sehelp_secommentexamples">' +
        "<p><b>" +
        kw +
        " 0/4</b>  :  add 4 to your estimate.</p>\
<p><b>" +
        kw +
        ' -2d 5/3 fix doc</b> : add 2days ago 5/3 with note "fix doc".</p>\
<p><b>' +
        kw +
        " @john 0/6</b> : add to john 6 estimate.</p>\
<p><b>" +
        kw +
        ' @john @paul -2d 3/3 code review</b> : add to john and paul -2days ago 3/3 with note "code review".</p>\
<p><i>use @me as shortcut to add yourself. @me is always the comment owner.</i></p>\
<p><b>' +
        kw +
        ' @global @me /7 ^etransfer</b> : transfer 7 "E 1ˢᵗ" from global to me.</p>\
<p><b>' +
        kw +
        ' 7</b> : (without "/") spends 7/0 or 7/7 for Plus recurring "[R]" cards.</p>\
<br>\
...
&nbsp;<br>\
<img style="margin-left:5em;" src="' +
        chrome.runtime.getURL("images/cardplusreport.png") +
        '"/>\
&nbsp;<br>\
&nbsp;<br>\
...
<img src="' +
        chrome.runtime.getURL("images/plusbarlabeled.png") +
        '" />\
<br><br>\
...
<img src="' +
        chrome.runtime.getURL("images/cardsemodify.png") +
        '" />\
<br><br>\
...
<p><b>' +
        g_optEnterSEByComment.getDefaultKeyword() +
        ' ^resetsync</b> as a card comment. Plus will re-read all S/E comments just for that card.</p>\
