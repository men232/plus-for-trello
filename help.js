...
function putKeywordsStringInUi(rg, inputKeywords) {
  ...
  rg.forEach(function (keyword) {
    if (strKeywords.length == 0) strKeywords = keyword;
    else strKeywords = strKeywords + ", " + keyword;
  });
...
function convertKWListToArray(inputKeywords) {
  ...
  rg.forEach(function (keyword) {
    ...
    if (k) rgNew.push(k); //skip blanks etc
  });
  return rgNew;
}
...
var Help = {
  ...
  raw: function (h, container) {
    if (!container) container = this.m_container;
    var elem = $(h);
  ...
  display: function () {
    ...
    testExtension(function () {
      //show help only if connected, plus this also commits pending log messages
      chrome.storage.sync.getBytesInUse(null, function (bytesInUse) {
        thisObj.storageTotalSync = bytesInUse;
        chrome.storage.local.getBytesInUse(null, function (bytesInUse2) {
          thisObj.storageTotalLocal = bytesInUse2;
          sendExtensionMessage({ method: "getlocalStorageSize" }, function (response) {
            ...
            sendExtensionMessage({ method: "getTotalDBRows" }, function (response) {
              ...
              sendExtensionMessage({ method: "getTotalDBRowsNotSync" }, function (response) {
                if (response.status != STATUS_OK) thisObj.totalDbRowsHistoryNotSync = response.status;
                else thisObj.totalDbRowsHistoryNotSync = response.cRowsTotal;

                chrome.storage.local.get([LOCALPROP_DONTSHOWSYNCWARN], function (obj) {
                  var value = obj[LOCALPROP_DONTSHOWSYNCWARN];
                  if (value !== undefined) thisObj.bDontShowAgainSyncWarn = value;

                  sendExtensionMessage({ method: "getTotalDBMessages" }, function (response) {
                    if (response.status != STATUS_OK) thisObj.totalDbMessages = response.status;
                    else thisObj.totalDbMessages = response.cRowsTotal;

                    sendExtensionMessage({ method: "detectLegacyHistoryRows" }, function (response) {
  ...
  enableIntervalScroll: function (bEnable) {
    if (bEnable) {
      if (this.intervalCorrectScroll) return;
  ...
  displayWorker: function () {
    ...
    if (bNotSetUp && bSEByComments) bNotSetUp = false;

    if (g_bDisableSync) bNotSetUp = true;
    ...
    function onClosePane() {
      if (!helpWin.isSyncEnabled() && !helpWin.bDontShowAgainSyncWarn) {
        var msgAlert =
          "You have not enabled sync! You will not see full reports, Chrome Boards & Cards menu and team Spent & Estimates.\n\nClick Cancel to configure sync, or click OK to use without sync.";
        var bHiliteGSButton = false;
        if (
          g_strServiceUrl == "" &&
          (comboSync.val() == SYNCMETHOD.googleSheetLegacy || comboSync.val() == SYNCMETHOD.googleSheetStealth)
        ) {
          msgAlert =
            "You have not set a google spreadsheet sync url.\nClick Cancel to configure it, or click OK to use without sync.";
    ...
    setTimeout(function () {
      ...
      var elemClose = helpWin.raw(
        '<img id="agile_help_close" class="agile_close_button agile_almostTopmost1" src="' +
      ...
      var elemTop = helpWin.raw(
        '<img class="agile_help_top agile_almostTopmost1" src="' +
          chrome.runtime.getURL("images/helptop.png") +
          '"></img>',
        containerFixed
      );
    ...
    if (!g_bFirstTimeUse) {
      helpWin.raw(
        '<span style="float:right;padding-right:6em;">\
...
<img src="https://www.linkedin.com/favicon.ico" title="Connect at LinkedIn" style="margin-bottom:-3px;margin-right:1px;border:0;width:16px;height:16px;"/></A></span>'
    ...
    helpWin
      .para(
        "version " + g_manifestVersion + "&nbsp;&nbsp<button style='float:right' class='agile_buton'>Close</button>"
      )
      .children("button")
      .click(onClosePane);
    ...
    helpWin.para("&nbsp;");
    if (g_bFirstTimeUse) {
      var elemFirstTime = helpWin.raw(
        "<div class='agile-help-firstTime'><b>To show this help again click <img src='" +
          chrome.runtime.getURL("images/iconspenthelp.png") +
          "' style='width:22px;height:22px;' /> next to the tour <img style='padding-left:4px;padding-bottom:5px' src='" +
          chrome.runtime.getURL("images/helparrow.png") +
          "' /></b></div>"
      );
    ...
    var paraLangOtherDetails = helpWin.raw(
      '<p>Currently only the Plus Tour is translated.<br>\
...
<A href="http://www.plusfortrello.com/p/help-us-translate-plus-to-your-language.html" target="_blank">Help translate or improve the tour</A> for your language!'
    ...
    function onComboLangChange() {
      ...
      } else paraLangOtherDetails.hide();

      if (valNew == "") return; //dont save the fake "other" selection. just there so user sees the help text below when lang is not english
    ...
    if (bNeedUpgrade) {
      ...
      var paraUpgrade = helpWin.para(
        "Installing now will close any open Plus chart/reports and refresh all Trello pages.<br><input type='button' value='Install now' />"
      );
    ...
    if (getIdBoardFromUrl(document.URL) != "0jHOl1As") {
      helpWin
        .para(
          "<div style='display:inline-block;border: 1px solid;border-radius:3px;border-color:var(--ds-border);padding:1em;background-color: var(--ds-background-inverse-subtle);'>Visit the <span style='font-weight:bold;font-size:110%;'><A target='_blank' href=''>Plus Help board</A></span> for the best place to learn about Plus.</div>"
        )
        .find("A")
        .click(function (e) {
    ...
    if (helpWin.totalDbMessages > 0) {
      helpWin
        .para(
          'Alert: Error log has entries. <A target="_blank" href="' +
            chrome.runtime.getURL("plusmessages.html") +
            '">View</A>.'
        )
    ...
    if (!g_bFirstTimeUse) {
      ...
      var paraCheckDisable = helpWin.para(
        '<input style="vertical-align:middle;" type="checkbox" class="agile_checkHelp" value="checkedDisablePlus">Disable changing trello.com pages. </input>\
<a href="">Tell me more</a>'
      ...
      paraCheckDisable.children("a").click(function (ev) {
        helpTooltip(
          ev,
          "If checked, Plus will still sync and reports will continue working.<br>This is an emergency option so you can keep using Trello in the unlikely case of a conflict."
        );
      });
      var bAddedRefresh = false;
      if (isPlusDisplayDisabled()) checkDisablePlus[0].checked = true;
      //.css("color", COLOR_ERROR);
      function setEDColor(check) {
        if (check.is(":checked")) paraCheckDisable.addClass("agile_color_warning");
        else paraCheckDisable.removeClass("agile_color_warning");
      }
    ...
    if (true) {
      ...
      var checkNoSE = helpWin
        .para(
          '<input style="vertical-align:bottom;" type="checkbox" class="agile_checkHelp">\
Do not use Timers, Spent, Estimates or Points.</input>'
        )
        .children("input:checkbox:first");
      var checkNoEst = helpWin
        .para(
          '<input style="vertical-align:bottom;" type="checkbox" class="agile_checkHelp">\
Do not use Estimates (just Spent/Points).</input>'
        )
        .children("input:checkbox:first");
      ...
      checkNoSE.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError !== undefined) {
            ...
            if (divAnalogy && bValue) divAnalogy.hide();
    ...
    if (bNotSetUp) {
      if (!g_bFirstTimeUse) {
        helpWin.para(
          '<div class="agile_box_input_hilite_red" style="display:inline-block;border: 1px solid;border-radius:3px;border-color:RGB(77,77,77);padding:1em;">\
Enable "➤ sync" below to see Reports, full Chrome Plus menu, team S/E and use from mobile\
      ...
      if (!bSEByComments && helpWin.totalDbRowsHistoryNotSync > 0) {
        ...
        if (helpWin.totalDbRowsHistoryNotSync > 9) {
          //simple sync test. could happen also if user entered a lot of S/E rows within 5 minutes.
          helpWin
            .para(
              "If still not finished in 10 minutes, make sure spreadsheet sharing is setup correctly with Write access to you."
            )
            .css("color", COLOR_ERROR);
        } else {
    ...
    if (g_msStartPlusUsage !== null) {
      ...
      if (cDaysUsingPlus > 2) strUsingPlusDays = "" + cDaysUsingPlus + " days with Plus. ";
    }
    ...
    function addProSection() {
      ...
      var paraPro = helpWin.para(
        '<input style="vertical-align:middle;margin-bottom:0px;" type="checkbox" class="agile_checkHelp" value="checkedProVersion" id="agile_plus_checkPro" /><label style="display:inline-block;color:var(--ds-text-brand) !important;" for="agile_plus_checkPro">Enable "Pro" features</label>'
      );
      ...
      if (bNotSetUp) textEnablePro += ' <a href="" id="agile_pro_more">Tell me more</a>';

      textEnablePro +=
        '<br><div id="agile_pro_more_content" style="display:none;">\
...
<A href="http://www.plusfortrello.com/p/plus-for-trello-pro-version.html" target="_blank">More</A>';
      textEnablePro +=
        '<br /></div></div><div id="sectionPayProNow" style="display:none;">➤ <A id="linkPayProNow" href="">Activate your "Pro" license now</A></div>\
...
<div id="sectionLiDetailsStripe" style="display:none;">\
    ...
    <input readonly class="agile_stripe_liUrl" size="80" title="copy and email this URL to the team" spellcheck="false" />\
      ...
      if (!bNotSetUp) paraProEnable.find("#agile_pro_more_content").show();
      ...
      if (helpWin.hasLiStripe) btnMorePmtOptions.hide();
      ...
      btnEditLicense.click(function (ev) {
        if (!helpWin.liDataStripe) return;
      ...
      var showProSections = function (bProEnabled, bHilitePay) {
        ...
        if (bShowLicDetailsCS)
          sectionLiDetailsCS
            .find(".agile_cs_licData")
            .html(
              "<b>Chrome store License</b> start date: " +
                makeDateCustomString(new Date(helpWin.liDataCS.msCreated)) +
                " <A target='_blank' href='https://payments.google.com/#subscriptionsAndServices'>View</A>"
            );

        if (bShowLicDetailsStripe) {
          sectionLiDetailsStripe
            .find(".agile_stripe_licData")
            .html(
              "<b>stripe.com License</b> start date: " +
                makeDateCustomString(new Date(helpWin.liDataStripe.msCreated)) +
                " for " +
                (helpWin.liDataStripe.quantity || "0") +
                " Trello users."
            );
          sectionLiDetailsStripe
            .find(".agile_stripe_liUrl")
            .val(
              "https://trello.com/" +
                URLPART_PLUSLICENSE +
                "/" +
                helpWin.liDataStripe.userTrello +
                "/" +
                helpWin.liDataStripe.li
            );
        ...
        elemShowHide(
          btnshowLiDetails,
          (bShowLicDetailsCS || bShowLicDetailsStripe) && !$("#agile_showLiDetails_contents").is(":visible")
        );
        ...
        if (bShowPay && bHilitePay) hiliteOnce(elemLinkPay, 5000);
      };
      ...
      checkEnablePro.click(function () {
        ...
        if (!bValue) {
          ...
          handleProAproval(function (status) {
            if (status != STATUS_OK) {
              bValue = false;
              if (status != STATUS_CANCEL) sendDesktopNotification(status, 10000);
            }
            saveCheck();
            if (bValue) {
              ...
              setTimeout(function () {
                return;
                if (!helpWin.hasLi()) $("#linkPayProNow").click();
              }, 200);
            } else {
    ...
    helpWin.para(
      "<h2 style='display:inline-block;'>Contents</h2><span style='color: #8c8c8c;margin-left:1em;'>click a section</span><ul id='tocAgileHelp'></ul>"
    );
    ...
    helpWin
      .para('<A target="_blank" href="">Our Plus help board</A> is the best place to learn about Plus.')
      .children("A")
      .click(function (e) {
        window.open("https://trello.com/b/0jHOl1As/plus-for-trello-help", "_blank");
        e.preventDefault();
    ...
    if (bAddFirstSyncNote)
      helpWin.para(
        'Now you only need to decide if Plus stores data inside or outside Trello, called the "sync" mode (later below).'
      );
    helpWin.para("<br>");
    helpWin.para("<b>Plus header</b>");
    helpWin.para('<img src="' + chrome.runtime.getURL("images/s3.png") + '"/>');

    helpWin.para(
      "The <A target='_blank' href='http://en.wikipedia.org/wiki/ISO_week_date'>ISO week</A> as in 2014-W49 is 2014's week 49. Weeks start on Sunday unless you change it in <b>Preferences</b>."
    );
    helpWin.paraSE(
      'Click the week to change the view on trello.com charts and reports. <A href="https://plus.google.com/photos/+PlusfortrelloNews/albums/6004371895359551937/6004371896981799010"  target="_blank"><br>Click chart titles</A> in trello.com to zoom charts to full window.'
    );
    helpWin.para("&nbsp");

    helpWin.para("<b>Plus Board toolbar</b>");
    helpWin.para('<img src="' + chrome.runtime.getURL("images/s2.png") + '"/>');
    helpWin.para("The full toolbar shows when the board has Spent & Estimates. Otherwise shows only the Report icon.");
    helpWin.para("Use the Report icon to make reports and charts for the board.").addClass(CLASS_onlyNonPlusSE);
    helpWin.paraEst(
      "Boxes display <b>S</b>pent / <b>E</b>stimate / <b>R</b>emaining totals of all visible cards. Mouse-over them to see <b>% complete</b>."
    );
    helpWin.para("&nbsp");

    helpWin.paraSE('<b>Plus card "S/E bar"</b>');
    helpWin.paraSE('<img src="' + chrome.runtime.getURL("images/cardplusbar.png") + '"/>');
    helpWin.paraSE('Open any card and click "Add S/E" or the little Plus icon inside the card comment.');
    helpWin.paraSE('<img src="' + chrome.runtime.getURL("images/showsebar.png") + '"/>');
    helpWin.paraSE("&nbsp");

    helpWin.paraEst("<b>E</b>stimate the units needed (per card or per user).");
    helpWin.paraEst("<b>S</b>pend units from the estimate.");
    helpWin.paraEst(
      "<b>R</b>emain units: How many more units until all Estimate is Spent (Remain = Estimate minus Spent)"
    );

    helpWin.paraSE(
      "Units (days, hours or minutes) can be configured in Preferences below. Do so before entering any S/E."
    );
    helpWin.paraSE("&nbsp");
    helpWin.paraSE('<hr class="agile_hr_help"><br>');
    helpWin.paraSE('<b><h2 id="agile_help_sesystem">The Plus Spent / Estimate system</h2></b>');
    helpWin.paraSE(
      'Plus tracks estimate, spent and changes by storing "S/E rows" per card and user, then summing the rows.'
    );
    ...
    helpWin.paraSE(
      '★ The best way to learn the system is to <b><A href="http://www.plusfortrello.com/p/how-plus-tracks-spent-and-estimate-in.html" target="_blank">read a typical case of using Plus (web page)</A>.'
    );
    helpWin.paraSE("It is similar to summing rows on a spreadsheet, with columns for Estimate and Spent.<br><br>");
    var linkShowAnalogy = helpWin
      .paraSE('★ <A href="">Click here</A> for an analogy with time tracking on a spreadsheet.<br><br>')
      .find("A")
      .eq(0);
    ...
    helpWin.para(
      "Imagine you are entering Spent time as rows in a spreadsheet, adding rows from top to bottom:",
      divAnalogy
    );
    helpWin.para(
      'Using the Plus "Card S/E bar" is inserting new rows to the table below above the running total:',
      divAnalogy
    );
    helpWin.para('<img src="' + chrome.runtime.getURL("images/help-spent-table.png") + '"/>', divAnalogy);
    helpWin.para("&nbsp", divAnalogy);
    helpWin.para(
      'Plus does the same, except the "rows" are entered inside Trello cards as special card comments (or as Google spreadsheet rows, when using stealth sync mode).',
      divAnalogy
    );
    helpWin.para("In the sample above, the current Spent is 13, the sum of all spend history.", divAnalogy);
    helpWin.paraEst(
      'Plus uses the same concept for estimates: Enter a "first estimate" on the first row, then increase or decrease it in later rows if needed:',
      divAnalogy
    );
    helpWin.paraEst('<img src="' + chrome.runtime.getURL("images/help-spent-est-table.png") + '"/>', divAnalogy);
    helpWin.paraEst(
      'This gives more information that just the previous "spent history" table because it shows a first estimate of 11, later increased by 2 hours.',
      divAnalogy
    );
    helpWin.paraEst(
      "Plus reports and charts show these estimate changes per user, board, label, hashtag and much more. Knowing the actual estimate gives you burn-downs and projected end dates as Plus knows how much work Remains and how it changed over time.",
      divAnalogy
    );
    helpWin.paraEst(
      'Plus automatically fills the "Estimate" column as you type "Spent" or stop timers, calculating any Estimate increases needed (which you can overwrite.)',
      divAnalogy
    );
    helpWin.paraEst(
      'Plus also has an extra "User" column for each S/E row as Plus keeps Spent/Estimates per user.',
      divAnalogy
    );
    helpWin.para(
      'Use the Plus "Card S/E bar" to add more rows, or directly modify the running totals using "Modify"  (which adds a row for you with the needed differences, positive or negative) or transfer Estimates.',
      divAnalogy
    );
    helpWin.para("&nbsp", divAnalogy);
    helpWin.paraEst('Plus can also assign a "global" card estimate and transfer estimates.');
    helpWin.paraSE(
      'This is the "Plus S/E bar" inside a card front, along with a card report and commands like "modify" above it.'
    );
    helpWin.paraSE(
      '<img id="seHelpAfterSpreadsheet" src="' + chrome.runtime.getURL("images/cardplusreport.png") + '"/>'
    );

    helpWin.paraSE(
      'Open a card to enter new <b>S</b>pent<span class="onlyPlusEst"> or <b>E</b>stimate</span> history rows.'
    );
    helpWin.paraSE('The table above the "card S/E bar" shows totals per user.');
    helpWin.paraEst(
      "Normally you first enter an estimate as in 0/2 (S:blank, E:2) and later spend it with 2/0 (S:2, E:blank)"
    );
    helpWin.paraEst("If you didn't estimate it previously, enter 2/2 which estimates and spends it on the same entry.");
    helpWin.paraEst(
      "Plus automatically pre-fills E when you type <b>S</b> that causes Remain to be negative (S sum bigger than E sum)."
    );
    helpWin.paraEst(
      "Plus considers your card finished when your <b>S sum</b> equals <b>E sum</b> thus R (Remain) is zero."
    );
    helpWin.paraEst(
      "You dont have to spend all the estimate right away. Maybe you enter 0/5, then 3/0 then 2/0. The sum is 5/5."
    );
    helpWin.paraEst(
      "Your first S/E row per card is the 1ˢᵗ estimate (E 1ˢᵗ) to compare with the current estimate <b>E sum</b>."
    );
    helpWin.paraSE(
      'When you enter S/E for another user (not "me") Plus generates a special note in that S/E row: "[by user]."'
    );
    helpWin.paraSE(
      "All special notes that Plus generates with [brackets] are secure and cannot be faked or removed by other users making Plus actions fully traceable."
    );
    ...
    helpWin.paraSE(
      'To track time spent in lists automatically, see <A href="http://www.plusfortrello.com/p/automated-time-tracking-with-butler-plus.html" target="_blank">using Plus & Butler</A>.'
    );
    helpWin.paraSE("&nbsp");
    helpWin.paraSE(
      'As you or others enter Estimates, Spent, Transfers or Modify, Plus makes special card comments (unless using "Stealth" sync mode).'
    );
    helpWin.paraEst("Here is an example of entering S/E, starting from the bottom (oldest) card comment:");
    helpWin.paraEst('<img src="' + chrome.runtime.getURL("images/s1.png") + '"/>');
    helpWin.paraSE("&nbsp");
    helpWin.paraSE(
      "You do not need to see those card comments to figure out totals or history. Plus has special reports and charts for that purpose."
    );
    helpWin.paraSE(
      '&bull; <b>Do not delete or edit a card S/E comment.</b> Instead use "<u>modify</u>" in the card front report.'
    );
    helpWin.paraEst('&bull; You can also add a "global" estimate or S/E for other users.');
    helpWin.paraSE(
      '&bull; Use "modify" if you prefer to work with total S/E ("sum") instead of adding or substracting with the "card S/E bar".'
    );
    helpWin.paraSE('&nbsp;&nbsp;&nbsp;"modify" will do the math for you and enter the needed S/E row.');
    helpWin.paraSE(
      '&nbsp;&nbsp;&nbsp;Example: if you entered a Spent of 3 and modify it to zero, "modify" will enter a new row of "-3/0".'
    );
    helpWin.paraSE("&bull; Enter S/E back in time by clicking on 'now' and pick how many days ago it happened.");
    helpWin.paraSE(
      '&bull; Keyboard: Use TAB or SHIFT+TAB to move between fields. Enter with the "Enter" key or button.'
    );
    helpWin.paraSE(
      '<b>More:</b> <A target="_blank" href="http://www.plusfortrello.com/p/s-e-entry-methods.html">Which S/E entry method should you use?</A>'
    );
    helpWin.paraSE("&nbsp");
    helpWin.paraSE('<hr class="agile_hr_help"><br>');
    ...
    helpWin.para(
      "<b>Pick your team's sync method. It defines whether Plus stores information inside or outside Trello.</b>"
    );
    helpWin
      .para(
        'Enable sync even if you wont use Spent or Estimates. Our <A target="_blank" href="">Plus help board</A> has more information.'
      )
      .children("A")
      .click(function (e) {
        window.open("https://trello.com/b/0jHOl1As/plus-for-trello-help", "_blank");
    ...
    comboSync = helpWin
      .para('<select id="agile_idComboSync" style="width:auto;height:2em;" class="agile_combo_input">')
      .children("select");
    comboSync.append($(new Option("Sync off", SYNCMETHOD.disabled)).addClass("agile_box_input_hilite_red"));
    comboSync.append(
      $(
        new Option("Recommended - Store inside Trello (S/E in Trello card comments)", SYNCMETHOD.trelloComments)
      ).addClass("agile_normalBackground")
    );
    comboSync.append(
      $(
        new Option("Stealth - Store outside Trello (S/E in Google spreadsheet)", SYNCMETHOD.googleSheetStealth)
      ).addClass("agile_normalBackground")
    );
    comboSync.append(
      $(new Option("Legacy - Store outside Trello (S/E in Google spreadsheet)", SYNCMETHOD.googleSheetLegacy)).addClass(
        "agile_normalBackground"
      )
    );
    var syncSectionsMap = {};
    ...
    if (helpWin.hasLegacyRows) {
      helpWin.para(
        '<A target="_blank" href="http://www.plusfortrello.com/2014/11/plus-for-trello-upgrade-from-legacy.html">Legacy "Google spreadsheet sync" users read here</A>.'
      );
      bDisplayedLegacyNote = true;
    }
    var paraFirstSync = helpWin.para(
      "<b>Your first sync will start after you close help</b>.\nKeep using Trello normally or close it, it will not affect sync."
    );
    helpWin.para(
      'If you switch sync methods or change keywords, "Reset Sync" from <A href="#agile_help_utilities">Utilities</A>.'
    );
    ...
    helpWin.paraSE(
      "&bull; Enter S/E using the card plus bar, mobile app, power-up or as a manual card comment.",
      divCur
    );
    helpWin.para(
      '&bull; This is the only method compatible with Butler for Trello to automatically <A target="_blank" href="http://www.plusfortrello.com/p/automated-time-tracking-with-butler-plus.html">track time spent in lists</A>.',
      divCur
    );
    if (g_strServiceUrl)
      helpWin.para(
        "Plus will no longer use the Google spreadsheet or rename card titles. You can also remove existing S/E inside card titles from Utilities.",
        divCur
      );
    var txtSEByCardComments =
      '<br>Use the default single keyword "plus!" to store and read S/E from card comments. Customize it here:<br><input style="display:inline;text-transform: lowercase;" type="text" spellcheck="false" maxlength="150" />&nbsp;<input type="button" value="Save keywords" /> Separate <A target="_blank" href="http://www.plusfortrello.com/p/faq.html#use_keywords">multiple keywords</A> with comma.';
    txtSEByCardComments =
      txtSEByCardComments +
      "<br>Your team should use the same keyword unless you want to further categorize or separate multiple subteams.";
    txtSEByCardComments =
      txtSEByCardComments +
      "<br>Home charts and the weekly report in the header can be filtered by keywords, see Preferences.";
    txtSEByCardComments =
      txtSEByCardComments +
      "<br>See <A href='http://www.plusfortrello.com/p/spent-estimate-card-comment-format.html' target='_blank'>card comment format help</A> for advanced features and keyword configuration ideas.";
    txtSEByCardComments =
      txtSEByCardComments +
      "<br><br>If your team entered S/E in Plus before 2015, also add 'plus s/e' as your last keyword. <A target='_blank' href='http://www.plusfortrello.com/2014/11/plus-for-trello-upgrade-from-legacy.html'>More</A>";

    var buttonshowNonMemberBoardsDialog = null;
    var txtSEByCardCommentsLast = "";
    if (!bAddFirstSyncNote)
      txtSEByCardCommentsLast =
        '<br>Find all boards in which you are not a member (Plus only syncs on boards with your direct membership):<br><input type="button" value="Find boards" />';
    ...
    helpWin.para(
      "Use this option when you must completely hide S/E from others that have access to your boards (like clients).",
      divCur
    );
    helpWin.para(
      "Stores S/E only in a private Google spreadsheet. S/E is not recorded anywhere inside Trello.",
      divCur
    );
    helpWin.para("The other sync modes make a card comment each time you enter S/E.", divCur);
    helpWin.para(
      "Only those that use the same sync spreadsheet will see the team S/E, regardless of Trello board permissions.",
      divCur
    );
    helpWin.para("&nbsp;", divCur);
    helpWin.para(
      'If you only want to prevent your S/E from appearing other user\'s reports and do not mind S/E appearing in card comments, you should instead use the 1ˢᵗ sync option and use a different "keyword".',
      divCur
    );
    helpWin.para("&nbsp;", divCur);
    helpWin.para('How is this mode different from "Trello card comments" sync:', divCur);
    helpWin.para(
      '&bull; Requires you to be <A target="_blank" href="https://support.google.com/chrome/answer/185277">signed-into Chrome</A>',
      divCur
    );
    helpWin.para(
      '&bull; Enter S/E using the "card S/E bar", not as card comments nor from mobile, power-up or other browsers.',
      divCur
    );
    helpWin.para(
      '&bull; No <A href="http://www.plusfortrello.com/p/faq.html#use_keywords" target="_blank">multiple keywords</A> feature.',
      divCur
    );
    ...
    function showCurrentSpreadsheetLink() {
      if (g_strServiceUrl == "") helpWin.para("Spreadsheet not yet configured.", divCur);
      else {
    ...
    if (!bDisplayedLegacyNote)
      helpWin.para(
        'Legacy "Google spreadsheet sync" users <A target="_blank" href="http://www.plusfortrello.com/2014/11/plus-for-trello-upgrade-from-legacy.html">read here</A>.',
        divCur
      );
    helpWin.para(
      'This legacy mode was how Plus initially worked, before the other two modes existed. Choose it if your team still hasn\'t <A target="_blank" href="http://www.plusfortrello.com/2014/11/plus-for-trello-upgrade-from-legacy.html">upgraded</A>.',
      divCur
    );
    helpWin.para(
    ...
    helpWin.para(
      "Advantages: Permission is based on the spreadsheet's permissions, regardless of board membership.",
      divCur
    );
    ...
    helpWin.para(
      "To prevent adding card comments or renaming card titles but still use spreadsheets, see the Stealth sync mode.",
      divCur
    );
    helpWin.para(
      'In this mode you must enter all S/E using the "card S/E bar" from Chrome, never directly as comments nor from mobile.',
      divCur
    );
    ...
    function onComboSyncChange() {
      ...
      for (var sMethod in syncSectionsMap) {
        ...
        } else syncSectionsMap[sMethod].hide();
      }
      ...
      if (valCombo != valComboOld) {
        ...
        if (valCombo == SYNCMETHOD.disabled) {
          ...
          if (bAddFirstSyncNote) paraFirstSync.show();
          else paraFirstSync.hide();
          if (valCombo == SYNCMETHOD.trelloComments) {
            ...
            if (g_strServiceUrl) {
              //if url not set yet, these two will be set during in plusconfig
              bEnableTrelloSyncNew = true;
        ...
        if (
          bDisableSyncNew != g_bDisableSync ||
          bEnableTrelloSyncNew != g_bEnableTrelloSync ||
          bSyncByCommentsNew != g_optEnterSEByComment.bEnabled ||
          bStealthSEModeNew != g_bStealthSEMode
        ) {
          setEnableTrelloSyncValue(bEnableTrelloSyncNew, bSyncByCommentsNew, bDisableSyncNew, bStealthSEModeNew);
        }

        if (bHilite) comboSync.addClass("agile_box_input_hilite_red");
        else comboSync.removeClass("agile_box_input_hilite_red");
      }

      if (elemShow) elemShow.show();
    }
    ...
    if (g_bDisableSync || (g_strServiceUrl == "" && !g_optEnterSEByComment.IsEnabled()))
      //same as !helpWin.isSyncEnabled()
      valComboNew = SYNCMETHOD.disabled;
    else if (g_optEnterSEByComment.IsEnabled()) {
      valComboNew = SYNCMETHOD.trelloComments;
    } else {
      if (g_bStealthSEMode) valComboNew = SYNCMETHOD.googleSheetStealth;
      else valComboNew = SYNCMETHOD.googleSheetLegacy;
    }
    ...
    function doSaveKeywords(bShowSavedMessage) {
      ...
      if (rgNew.length == 0) rgNew.push(SEKEYWORD_DEFAULT);
      ...
      chrome.storage.sync.set({ rgKWFCC: JSON.stringify(rgNew) }, function () {
        ...
        var bChanged =
          JSON.stringify(g_optEnterSEByComment.rgKeywords).toLowerCase() != JSON.stringify(rgNew).toLowerCase();
        g_optEnterSEByComment.rgKeywords = rgNew;
        getKeywordsViewList(); //trigger refresh
        refreshCardTableStats();
        if (bShowSavedMessage)
          alert(
            bChanged ? "Saved. If your new keywords were used in the past, 'Reset Sync' from utilities." : "Saved."
          );
      });
    }
    ...
    function setEnableTrelloSyncValue(bValue, bValueSyncByComments, bDisabled, bStealthSEMode) {
      ...
      function worker() {
        ...
        if (bValue) pair["bEnabledTrelloSyncBETA"] = true; //only way to turn it off is by doing a reset which will erase this sync property. review zig: later use a local property to detect if device was converted out of beta
          ...
          if (g_bEnableTrelloSync && !g_bDisableSync) {
            if (bValueSyncByComments) {
              if (!g_optEnterSEByComment.hasLegacyKeyword() && helpWin.hasLegacyRows) {
                ...
                alert(
                  "the legacy keyword 'plus s/e' was added because you have legacy history rows (before dec. 2014).\nThis allows you to later Reset Sync without missing legacy card comments."
                );
              }
              inputKeywords.focus();
            }
    ...
    helpWin.paraEst(
      "Plus supports two levels of pre-estimation to help your team figure out card first estimates (E 1<sup>st</sup>)."
    );
    helpWin.paraEst(
      "Add pre-estimates to checklist items or card titles. Neither show in reports, only in the card front and board dimensions."
    );
    helpWin.paraEst(
      'Read <A href="http://www.plusfortrello.com/p/how-plus-tracks-spent-and-estimate-in.html#plus_preestimates" target="_blank">how to use pre-estimates</A>'
    );
    ...
    helpWin.paraSE(
      '<b>★</b> <b>Do not edit or delete a card S/E comment</b>. Instead use "modify" to make S/E changes.'
    );
    helpWin.paraSE(
      '<b>★</b> If you do modify or delete S/E card comments see help about the card "^resetsync" command.'
    );
    ...
    helpWin.paraEst(
      '<b>★</b> When a user finishes a card but has <b>R</b>emaining, use "modify" and blank or zero <b>R</b> (which reduces E).'
    );
    helpWin.paraEst("<b>★</b> Similarly if S goes over E, increase E so R is not negative.");
    helpWin.paraEst(
      '<b>★</b> The "card S/E bar" and "modify" automatically pre-fill increased E to help you prevent negative R.'
    );
    helpWin.paraEst(
      '<b>★</b> Prevent accidental E increases with the preference to "Prevent me from increasing existing E".'
    );
    helpWin.paraSE(
      '<b>★</b> You may use the units:subunits <b>colon format</b> to enter S/E. (as in hours:minutes when using "hour" units)'
    );
    helpWin.paraSE(
      "&nbsp;&nbsp;&nbsp;1:25 in hour units = 1 hour 25 minutes = 1.42 hours. Note one uses a <i>colon:</i> and the other uses a <i>period.</i>"
    );
    helpWin.paraSE('&nbsp;&nbsp;&nbsp;Plus always converts to "decimal format".');
    helpWin.paraSE(
      '<b>★</b> Add <b>[exclude]</b> to list names to exclude them from board sums on the Trello board page.<br>\
&nbsp;&nbsp;&nbsp;To exclude those also in reports set the list filter to "![exclude]".'
    );
    helpWin.paraSE(
      '<b>★</b> Renaming a user is not renamed in Plus. It will appear as a new user until "Reset sync". <a href="https://trello.com/c/N59rMJbk/" target="_blank">More</a>.'
    );
    helpWin.paraSE("&nbsp");
    helpWin.paraSE('<hr class="agile_hr_help"><br>');
    ...
    helpWin.paraSE(
      '<A href="http://www.plusfortrello.com/p/mobile-plus-for-trello.html" target="_blank">More information</A>.'
    );
    helpWin.paraSE("&nbsp");
    helpWin.paraSE('<hr class="agile_hr_help"><br>');

    helpWin.para('<b><h2 id="agile_help_plusmenu">Plus menu</h2></b>');
    helpWin.para('<img src="' + chrome.runtime.getURL("images/plusmenu.png") + '"/>');
    helpWin.para(
      '<A href="https://www.youtube.com/watch?v=gbAZXtaRi5o" target="_blank">Don\'t see the Plus menu icon?</A>'
    );
    helpWin.paraSE(
      'The icon changes to <img src="' +
        chrome.runtime.getURL("images/icon19new.png") +
        '"/> (green dot top-left) when there are <b>new S/E</b> rows from your team.'
    );
    ...
    helpWin.para(
      'Use pattern matching with <b>*</b> for "any characters" and <b>?</b> for "single character" (<a target="_blank" href="http://en.wikipedia.org/wiki/Glob_(programming)#Syntax">GLOB syntax</a>).'
    );
    ...
    helpWin.para(
      '&bull; Include <A href="http://www.plusfortrello.com/p/trello-custom-fields-in-plus-reports.html" target="_blank">Trello custom fields and many more options</A> ("Pro" version).'
    );
    helpWin.paraSE('&bull; Pick the groups, pivot, sort, filters and options, then "Query".');
    helpWin.para(
      '&bull; Use "Copy" <IMG border="none" align="top" src="' +
        chrome.runtime.getURL("images/copy.png") +
        '"></IMG> on the top-right to copy to the clipboard. Paste on a spreadsheet or email.'
    );
    helpWin.para("&bull; Drill-down on Trello home chart bars or pivot cells.");
    helpWin.para(
      "&bull; Reports and burndowns work offline from the Chrome Plus menu. Bookmark, email the report URL or copy & paste the report anywhere."
    );
    helpWin.paraEst(
      "&bull; The <b>E. type</b> column tells if the row Estimate is new, increases (+E) or decreases (-E) the card estimate per user."
    );
    helpWin.para(
      '<A target="_blank" href="http://www.plusfortrello.com/p/report-documentation-and-examples.html"><b>More reports help</b></A>'
    );
    helpWin.para("&nbsp");
    helpWin.para('<hr class="agile_hr_help"><br>');

    helpWin.paraSE('<b><h2 id="agile_help_timers">Card Timers</h2></b>');
    helpWin.paraSE('<img src="' + chrome.runtime.getURL("images/timer.png") + '"/>');
    helpWin.paraSE(
      "&bull; Start a timer from any card. The last active timer is always visible in the Chrome Plus icon and menu."
    );
    helpWin.paraSE("&bull; Timers measure time in your units from Preferences.");
    helpWin.paraSE(
      "&bull; Timers always fill Spent using 'decimal format' and not 'colon format'. See more under 'Best practices'."
    );
    helpWin.paraSE("&bull; Minimize a timer by clicking on '↓'");
    helpWin.paraSE(
      "&bull; Use timers started from another device when you are <A target='_blank' href='https://support.google.com/chrome/answer/185277'>signed-into Chrome</A>."
    );
    helpWin.paraSE("&bull; If you forgot to start a timer, type the spent so far in the 'S' box and start the timer.");
    helpWin.paraSE(
      "&bull; Pause the timer to pre-fill the 'S' box.<span class='onlyPlusEst'> Add an optional estimate or note and press ENTER.</span>"
    );
    helpWin.paraSE("&bull; If you dont press ENTER right away, Plus will remind you next time you open the card.");
    helpWin.paraSE(
      "&bull; Cards with active (running) timers have a hourglass in Board view and show in the Chrome Plus menu."
    );
    helpWin.paraSE("&nbsp");
    helpWin.paraSE('<hr class="agile_hr_help"><br>');
    ...
    helpWin.para('<img src="' + chrome.runtime.getURL("images/cardht.png") + '"/>');
    helpWin.para('Tags containing "!" are highlighted in yellow.');
    helpWin.para("&nbsp");
    helpWin.para('<hr class="agile_hr_help"><br>');

    helpWin.paraEst('<b><h2 id="agile_help_burndowns">Burndown charts</h2></b>');
    helpWin.paraEst('<img src="' + chrome.runtime.getURL("images/s4.png") + '"/>');
    ...
    helpWin.paraEst(
      '&bull; Remain <span style="color:green;">(green)</span> goes down to zero at the end of each milestone.'
    );
    helpWin.paraEst(
      '&bull; Plus supports <a href="" target="_blank">end-date projections</a> (dotted green line) based on past spent rates.'
    );
    helpWin.paraEst("&bull; Due dates are automatically added as annotations, or add your own.");
    helpWin.paraEst(
      '&bull; Add an annotation with a card S/E row\'s <A href="http://www.plusfortrello.com/p/spent-estimate-card-comment-format.html" target="_blank">note starting with "!"</A>.'
    );
    helpWin.paraEst("&bull; Include multiple boards and customize filters.");
    helpWin.paraEst(
      '&bull; <A href="https://trello.com/c/WkUNDkBq/35-burn-downs-timeline-of-due-dates" target="_blank">More</A>.'
    );
    helpWin.paraEst("&nbsp");
    helpWin.paraEst('<hr class="agile_hr_help"><br>');
    ...
    helpWin.paraSE(
      'Useful when using <A target="_blank" href="http://www.plusfortrello.com/p/faq.html#use_keywords">multiple keywords</A> or "card title S/E" (Scrum for Trello and such).'
    );
    helpWin.paraSE('<img width="300" src="' + chrome.runtime.getURL("images/dimensions.png") + '"/>');
    helpWin.paraSE(
      '<A href="http://www.plusfortrello.com/p/board-dimensions.html" target="_blank">More about "dimensions"</A>'
    );
    helpWin.paraSE("&nbsp");
    helpWin.paraSE('<hr class="agile_hr_help"><br>');
    ...
    helpWin.para(
      '&bull; <A target="_blank" href="http://help.trello.com/article/820-card-aging">Enable the Card Aging power-up</A> on each board to hide cards.'
    );
    ...
    if (true) {
      ...
      pComboUnits
        ...
        .append(
          $(
            "<span> Card timers measure time in your units. When changing units, S/E already entered is assumed in the new units so set your units here before entering any S/E.</span>"
          )
        );
    ...
    if (true) {
      var checkPointUnits = helpWin
        .paraSE(
          '<input style="vertical-align:middle;margin-bottom:0px;" type="checkbox" class="agile_checkHelp" value="checkedPointUnits" \
>Call units "Points" instead of the selection above.</input>'
        )
        .children("input:checkbox:first");

      if (g_bDisplayPointUnits) checkPointUnits[0].checked = true;
      ...
      checkPointUnits.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bDisplayPointUnits = bValue;
    ...
    function updateEffectiveDow() {
      ...
      if (delta != 0) {
        ...
        var strPrefix = "";
        if (delta > 0) strPrefix = "the following ";
        else strPrefix = "the previous ";
        strEffectiveDow =
          " <b>Week effectively starts on " +
          strPrefix +
          getWeekdayName(DowMapper.dowFromPosWeek(0), true).toLowerCase() +
          "</b>. Set shift to zero above to undo. ";
      }
      var dateNowTemp = new Date();
      strEffectiveDow +=
        "<br>Today's week is " + getCurrentWeekNum(dateNowTemp, dowStart, delta) + " with these settings.<br><br>";
      if (delta != 0)
    ...
    if (true) {
      ...
      comboDowStart.change(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          ...
          if (strError) sendDesktopNotification(strError, 6000, idNotificationWeekUpgrade);
          if (bError) {
          ...
          openPlusDb(
            ...
            function (response) {
              if (response.status != STATUS_OK) strError += response.status;
              else {
    ...
    if (true) {
      var pComboWeekDelta = helpWin
        .raw("<p><span>Use any start day by shifting the week -back or +forward: </span></p>")
        .addClass("agile_help_indent1");
    ...
    if (true) {
      helpWin.paraSE(
        '&bull; Customize your <b>S</b>pent, <b>E</b>stimate, <b>R</b>emaining board views: ("Pro" version only)'
      );
      var strTable =
        '<table style="width:auto !important;margin-left:1em !important;">\
...
<td align="middle"><input style="margin-bottom:0px;" type="checkbox" class="agile_checkHelp"></input></td></tr>';
          ...
          check.click(function () {
            if (!g_bProVersion)
              sendDesktopNotification(
                'To use custom board views, enable the "Pro" version at the top of this help pane.'
              );
            g_serViews[type][prop] = check.is(":checked");
            if (timeoutChangeSERView) clearTimeout(timeoutChangeSERView);
            setTimeout(updateBoardUI, 100);
    ...
    if (true) {
      var paraPreventEstMod = helpWin.paraEst(
        '<input style="vertical-align:middle;margin-bottom:0px;" type="checkbox" class="agile_checkHelp" value="checkedPreventEstMod" \
>Prevent me (mostly) from increasing existing <b>E</b>stimates. Your manager does it for you or wants to prevent entry mistakes. </input> <a href="">Tell me more</a>'
      ...
      paraPreventEstMod.children("a").click(function (ev) {
        helpTooltip(
          ev,
          "Check to prevent users from accidentally increasing E (cause a +E) in the S/E bar and 'modify'.\
...
Managers can always easily find new S/E rows with 'E. type: +E' in the Plus Chrome menu 'New S/E rows' report or estimate change charts."
      ...
      if (g_bPreventIncreasedE) checkPreventEstMod[0].checked = true;
      ...
      checkPreventEstMod.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bPreventIncreasedE = bValue;
    ...
    if (true) {
      var checkIgnoreZeroEst = helpWin
        .paraEst(
          '<input style="vertical-align:middle;margin-bottom:0px;" type="checkbox" class="agile_checkHelp" value="checkedIgnoreZeroEstimates" \
...
. You will lose ability to measure remaining effort and "E" will not autocomplete as you type "S".</input>'
        )
        .children("input:checkbox:first");

      if (g_bAllowNegativeRemaining) checkIgnoreZeroEst[0].checked = true;
      ...
      checkIgnoreZeroEst.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bAllowNegativeRemaining = bValue;
    ...
    if (true) {
      var checkHidePending = helpWin
        .paraEst(
          '<input style="vertical-align:middle;" type="checkbox" class="agile_checkHelp" value="checkedHidePending">\
Hide "Remaining balance cards" section in Trello home.</input>'
        )
        .children("input:checkbox:first");
      if (g_bHidePendingCards) checkHidePending[0].checked = true;
      ...
      checkHidePending.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bHidePendingCards = bValue;
    ...
    if (true) {
      var checkAlwaysShowSEBar = helpWin
        .paraSE(
          '<input style="vertical-align:middle;" type="checkbox" class="agile_checkHelp" value="checkedAlwaysShowSEBar">\
Always show the "card S/E bar".</input>',
          null,
          "If you leave unchecked, you need to click the card front 'S/E & More' menu to show the Plus S/E bar."
        )
        .children("input:checkbox:first");
      if (g_bAlwaysShowSEBar) checkAlwaysShowSEBar[0].checked = true;
      ...
      checkAlwaysShowSEBar.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bAlwaysShowSEBar = bValue;
    ...
    if (true) {
      var checkUseLastSEBarUser = helpWin
        .paraSE(
          '<input style="vertical-align:middle;" type="checkbox" class="agile_checkHelp" value="checkedUseLastSEBarUser">\
Always default to the last user selected in the card S/E bar and transfer "from" field.</input>'
        )
        .children("input:checkbox:first");
      if (g_bUseLastSEBarUser) checkUseLastSEBarUser[0].checked = true;
      ...
      checkUseLastSEBarUser.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bUseLastSEBarUser = bValue;
    ...
    if (true) {
      var checkHideLessMore = helpWin
        .para(
          '<input style="vertical-align:middle;" type="checkbox" class="agile_checkHelp" value="checkedHideLessMore">\
Hide "Less - More" at the top of Trello pages.</input>',
          null,
          "This feature shows in all Trello page headers. Uncheck if you dont need it."
        )
        .children("input:checkbox:first");
      if (g_bHideLessMore) checkHideLessMore[0].checked = true;
      ...
      checkHideLessMore.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bHideLessMore = bValue;
          checkHideLessMore[0].checked = g_bHideLessMore;
          if (g_bHideLessMore) {
            chrome.storage.sync.set({ bShowAllItems: true }, function () {
              if (chrome.runtime.lastError === undefined) g_bShowAllItems = true;
            });
          }
        });
      });
    ...
    if (true) {
      var checkSyncOutsideTrello = helpWin
        .para(
          '<input style="vertical-align:middle;" type="checkbox" class="agile_checkHelp" value="checkedSyncOutsideTrello">\
Background sync every 10 minutes while Chrome is open even if Trello is not open.</input>',
          null,
          "Check this so the Chrome Plus menu, reports and charts are up-to-date even if you dont open a Trello page."
        )
        .children("input:checkbox:first");
      if (g_bSyncOutsideTrello) checkSyncOutsideTrello[0].checked = true;
      ...
      checkSyncOutsideTrello.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bSyncOutsideTrello = bValue;
    ...
    if (true) {
      var checkDontWarnParallelTimers = helpWin
        .paraSE(
          '<input style="vertical-align:middle;" type="checkbox" class="agile_checkHelp" value="checkedDontWarnParallelTimers">\
Do not warn when starting a timer when another timer is active.</input>'
        )
        .children("input:checkbox:first");
      if (g_bDontWarnParallelTimers) checkDontWarnParallelTimers[0].checked = true;
      ...
      checkDontWarnParallelTimers.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bDontWarnParallelTimers = bValue;
    ...
    if (true) {
      //always show Spent in the Chrome icon, even when a timer is active.
      var comboSpentOnAppIcon = helpWin
        .paraSE(
          'Show your weekly spent on the Chrome Plus icon? <select style="width:auto" class="agile_combo_input"></select>'
        )
      ...
      comboSpentOnAppIcon.change(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_optAlwaysShowSpentChromeIcon = val;
          comboSpentOnAppIcon.val(g_optAlwaysShowSpentChromeIcon);
          sendExtensionMessage(
            { method: "updatePlusIcon", bOnlyTimer: false, bAnimate: true, bSetSpentBadge: true },
            function (response) {}
          );
        });
      });
    ...
    if (true) {
      var checkDontShowSpentPopups = helpWin
        .paraSE(
          '<input style="vertical-align:middle;" type="checkbox" class="agile_checkHelp" value="checkedDontShowSpentPopups">\
Do not show daily spent total popup notifications every time you enter spent.</input>'
        )
        .children("input:checkbox:first");
      if (g_bDontShowSpentPopups) checkDontShowSpentPopups[0].checked = true;
      ...
      checkDontShowSpentPopups.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bDontShowSpentPopups = bValue;
    ...
    if (true) {
      var comboCardPopupStyle = helpWin
        .para(
          'Use <A href="https://trello.com/c/ZduasRWD/138-mini-me-card-popups" target="_blank">mini-me card popups</A> in reports and Chrome menu? <select style="width:auto" class="agile_combo_input"></select>'
        )
        .children("select:first");
      comboCardPopupStyle.append(
        $(new Option("Yes, as small card popups (limited funcionality)", CARDPOPUPTYPE.POPUP_NOACTIONS))
      );
      comboCardPopupStyle.append(
        $(new Option("Yes, as medium card popups (more functionality)", CARDPOPUPTYPE.POPUP_SOMEACTIONS))
      );
      ...
      comboCardPopupStyle.change(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) helpWin.cardPopupType = val;
          comboCardPopupStyle.val(helpWin.cardPopupType);
        });
      });
    }
    ...
    if (true) {
      var checkDontShowTimerPopups = helpWin
        .paraSE(
          '<input style="vertical-align:middle;" type="checkbox" class="agile_checkHelp" value="checkedDontShowTimerPopups">\
Do not show floating timers (just show them inside the Chrome Plus menu).</input>'
        )
        .children("input:checkbox:first");
      if (g_bDontShowTimerPopups) checkDontShowTimerPopups[0].checked = true;
      ...
      checkDontShowTimerPopups.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bDontShowTimerPopups = bValue;
    ...
    if (true) {
      var checkCardColor = helpWin
        .para(
          '<input style="vertical-align:middle;" type="checkbox" class="agile_checkHelp" value="checkedCardColor">\
Set the card background based on its first label color.</input>'
        )
        .children("input:checkbox:first");
      if (g_bChangeCardColor) checkCardColor[0].checked = true;
      ...
      checkCardColor.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bChangeCardColor = bValue;
    ...
    //option to hide tour
    if (true) {
      var checkHideTour = helpWin
        .para(
          '<input style="vertical-align:middle;" type="checkbox" class="agile_checkHelp" value="checkedHideTour">\
Hide the "tour" from the top of Trello pages.</input>'
        )
        .children("input:checkbox:first");
      if (g_bHideTour) checkHideTour[0].checked = true;
      ...
      checkHideTour.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bHideTour = bValue;
    ...
    helpWin.paraSE(
      "<br>&bull; The next two settings let Plus read S/E from card and checklist titles for board dimensions, pre-estimation and those migrating from other scrum tools."
    );
    helpWin.paraSE("Plus uses card title S/E in board dimensions only when the card has no S/E rows entered yet.");
    helpWin.paraSE(
      "All users should have the same setting. S/E entered this way will only appear in the board and checklists, not in reports or burndowns."
    );
    helpWin.paraSE(
      "See more about <A target='_blank' href='http://www.plusfortrello.com/p/notes-for-users-of-scrum-for-trello.html'>S/E in card titles</A> and <A target='_blank' href='http://www.plusfortrello.com/p/how-plus-tracks-spent-and-estimate-in.html#plus_preestimates'>pre-estimations</A>."
    );
    //checkAcceptPFTLegacy
    if (true) {
      var checkAcceptPFTLegacy = helpWin
        .rawSE(
          '<span style="vertical-align:middle;margin-bottom:0px;"><input style="vertical-align:middle;margin-bottom:0px;" type="checkbox"  value="checkAcceptPFTLegacy">\
Accept the "(S/E) title" format in card titles.</input></span><br>'
        )
        .children("input:checkbox:first");
      if (g_bAcceptPFTLegacy) checkAcceptPFTLegacy[0].checked = true;
      ...
      checkAcceptPFTLegacy.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bAcceptPFTLegacy = bValue;
    ...
    if (true) {
      var checkAcceptScrumForTrello = helpWin
        .rawSE(
          '<span style="vertical-align:middle;margin-bottom:0px;"><input style="vertical-align:middle;margin-bottom:0px;" type="checkbox"  value="checkedAcceptSFT">\
Accept the "Scrum for Trello" format in card titles: <i>(Estimate) card title [Spent]</i>.</input></span>'
        )
        .children("input:checkbox:first");
      if (g_bAcceptSFT) checkAcceptScrumForTrello[0].checked = true;
      ...
      checkAcceptScrumForTrello.click(function () {
        ...
        chrome.storage.sync.set(pair, function () {
          if (chrome.runtime.lastError == undefined) g_bAcceptSFT = bValue;
    ...
    //global user
    if (true) {
      var paraGlobalUser = helpWin.paraEst(
        '&bull; Global estimates name (not a real Trello user): <input style="display:inline;width:15em;" type="text" spellcheck="false" maxlength="20"/>&nbsp;<input type="button" value="Save"/>'
      );
      ...
      buttonSaveGlobalUser.click(function () {
        ...
        function doSave(bShowSavedMessage) {
          ...
          chrome.storage.sync.set(objSave, function () {
            ...
            g_globalUser = val;
            if (bShowSavedMessage) alert("Saved.");
          });
        }
      });
    }
    ...
    if (true) {
      var paraExcludeUsers = helpWin.paraSE(
        '&bull; Exclude these users from the card bar. Separate users with comma:<br><input style="display:inline;width:40em;" type="text" spellcheck="false" maxlength="500"/>&nbsp;<input type="button" value="Save list"/>'
      );
    ...
    if (true) {
      var paraKWHome = helpWin.paraSE(
        '&bull; Only include these <A target="_blank" href="http://www.plusfortrello.com/p/faq.html#use_keywords">keywords</A> (separated by comma) in the Trello header report and home charts:<br><input style="display:inline;width:40em;" type="text" spellcheck="false" maxlength="200"/>&nbsp;<input type="button" value="Save list"/>'
      );
      ...
      buttonKWHome.click(function () {
        ...
        function doSave(bShowSavedMessage) {
          ...
          chrome.storage.sync.set(objSave, function () {
            ...
            doWeeklyReport(g_configData, getCurrentTrelloUser(), true, false);
            setTimeout(function () {
              //timeout allows the charts UI to update
              if (bShowSavedMessage) alert("Saved.");
            }, 200);
          });
        }
    ...
    helpWin
      .para(
        'NOTE: many preferences are hidden because you checked "do not use Timers, Spent, Estimates or Points" at the top of this help.'
      )
    ...
    var paraReset = helpWin.para(
      '&bull; Re-read all your S/E data: <input type="button" value="Reset sync"/><br />Close other trello tabs before reset. Useful if you changed keywords, edited or deleted many card S/E comments.'
    );
    helpWin.paraSE(
      'If you only mofified a few card comments, read about the <A href="http://www.plusfortrello.com/p/spent-estimate-card-comment-format.html#resetsynccommand" target="_blank">card "^resetsync" command</A>.<br /><br />'
    );
    ...
    if (g_optEnterSEByComment.IsEnabled() || IsStealthMode()) {
      var paraRenameCards = helpWin.paraSE(
        '&bull; Remove S/E from card titles in Trello. Useful if you switch from the Legacy sync method:<br><input type="button" value="Rename cards with S/E history"/>&nbsp;&nbsp;&nbsp;<input type="button" value="Rename all cards"/>'
      );
      ...
      function handleButtonRename(bOnlyCardsWithHistory) {
        sendExtensionMessage(
          { method: "queueRenameAllCards", bOnlyCardsWithHistory: bOnlyCardsWithHistory },
          function (response) {
            alert(
              "Renaming will happen at the end of the next sync\nafter you close help.\nHover the Chrome Plus icon (top-right) to see sync progress."
            );
      ...
      buttonRenameCardsWithSE.click(function () {
        if (
          !confirm(
            "Are you sure you want to rename all cards with existing S/E rows?\nThey will be permanently renamed in Trello."
          )
        )
          return;
      ...
      buttonRenameCardsAll.click(function () {
        if (
          !confirm(
            "Are you sure you want to rename all cards, even those without S/E history rows?\nThey will be permanently renamed in Trello."
          )
        )
          return;
      ...
      var paraUndoRenameCards = helpWin.paraSE(
        '<br />&bull; Undo a "Remove S/E from card titles" command using the created backup file from the "Rename" buttons above.<br /><input type="button" value="Undo Rename cards"/>'
      );
      var buttonUndoRenameCards = paraUndoRenameCards.children("input:button:first");
      buttonUndoRenameCards.click(function () {
        sendExtensionMessage({ method: "undoRenameCards" }, function (response) {
          if (response.status == STATUS_OK) {
            if (response.totalCards != 0) {
              alert(
                "Undo of " +
                  response.totalCards +
                  " card titles will happen during the next sync\nafter you close help.\nHover the Chrome Plus icon (top-right) to see sync progress."
              );
              helpWin.bStartSyncOnClose = true;
            } else alert("No cards to process.");
          } else {
      ...
      helpWin.paraSE(
        'Removal of S/E from card titles is only allowed in "Trello card comments" or "stealth" sync mode.'
      );
    }

    helpWin.para("&nbsp");
    helpWin.para(
      '&bull; To find all boards where you are not a member, see the <A href="#agile_help_trellosync">"Sync" section</A> above (only if using the "card comments" sync mode)'
    );
    helpWin.para("&nbsp");
    helpWin.para('<hr class="agile_hr_help"><br>');
    ...
    helpWin.paraSE(
      "Plus can read S/E from card titles. If so, the S/E boxes in the card back are gray instead of white."
    );
    helpWin.paraSE(
      '<A target="_blank" href="http://www.plusfortrello.com/p/notes-for-users-of-scrum-for-trello.html">Read migration instructions</A> and see <b>Preferences</b> to "Accept the Scrum for Trello format".'
    );
    helpWin.paraSE("&nbsp");
    helpWin.paraSE('<hr class="agile_hr_help"><br>');
    ...
    helpWin.para(
      'Read the most <A target="_blank" href="http://www.plusfortrello.com/p/faq.html">Frequenty Asked Questions</a>.'
    );
    ...
    helpWin.para(
      'Plus secures all your data inside your browser, does not use servers to store Trello data (except your license information) and does not have access to your data outside your browser. <A target="_blank" href="http://www.plusfortrello.com/p/privacy-policy.html">More</A>.'
    );
    helpWin.para(
      'By using this software, you agree to our <A target="_blank" href="http://www.plusfortrello.com/p/eula-plus-for-trello-end-user-license.html">End-user license agreement (EULA)</A>.'
    );
    helpWin.para("&nbsp");
    helpWin.para('<hr class="agile_hr_help"><br>');
      ...
      if (id && id != "agile_pro_section") {
        ...
        while (pParent && cSafe >= 0) {
          if (!pParent.hasClass("onlyPlusSE") && !pParent.hasClass("onlyPlusEst")) {
            ...
            if (pParent.hasClass("onlyPlusSE")) li.addClass("onlyPlusSE");
            if (pParent.hasClass("onlyPlusEst")) li.addClass("onlyPlusEst");
            break;
          }
        }
        toc.append(li);
  ...
  close: function (bRestarting) {
    ...
    sendExtensionMessage({ method: "endPauseSync" }, function (response) {
      while (objHelp.m_extraElems && objHelp.m_extraElems.length > 0) objHelp.m_extraElems.pop().remove();
      ...
      objHelp.m_container.fadeOut("fast", function () {
        ...
        if (objHelp.bStartSyncOnClose) {
          ...
          if (g_bNeedStartTourBubble) showTourBubble();
        }
      });
    });
  },
...
<button style="float:right;" id="agile_dialog_showNonMemberBoards_OK">OK</button> \
  ...
  divDialog
    .find("#agile_dialog_showNonMemberBoards_OK")
    .off("click.plusForTrello")
    .on("click.plusForTrello", function (e) {
      divDialog[0].close();
    });
  ...
  sendExtensionMessage({ method: "getBoardsWithoutMe" }, function (response) {
    if (cProtect != g_cProtectMultiNonMemberBoardsDialog) return;
    div.empty();
    ...
    response.boards.forEach(function (board) {
      ...
      var text = " ";
      if (board.dateLastActivity) text += makeDateCustomString(new Date(board.dateLastActivity)) + " ";

      if (board.closed) text += "[Closed] ";
      ...
      a.click(function (ev) {
        ...
        if ((ev && (ev.ctrlKey || ev.shiftKey)) || window.name == targetWin) targetWin = "_blank"; //force many windows
