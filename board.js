...
function updateBoardUI() {
  ...
  if (boardCur) updateCards(boardCur, null, true, false);
}
...
function updateCardsWorker(boardCur, responseParam, bShowBoardTotals, defaultSE, idBoardParam, mapIdCardToSE) {
  ...
  function response() {
    if (bResetHtmlLast) g_strPageHtmlLast = "";
    else g_strPageHtmlLast = document.body.innerHTML; //reset page detector
    if (responseParam) responseParam();
  }
  ...
  function forEachList(i, el) {
    ...
    function forEachCardWorker(k, card) {
      ...
      function bUpdateTitle() {
        ...
        if (!updateTotals) bFilteredCard = true;

        if (!bCardIsVisible) bHasHiddenCard = true;

        if (g_bChangeCardColor) LabelsManager.update($(card));

        var nodeTitle = originalTitleTag.contents().filter(function () {
          //this method tries to cover cases when trello changes their html
          return this.nodeType == 3;
        });
        if (nodeTitle && nodeTitle.length > 0) title = nodeTitle[0].nodeValue;
        else {
      ...
      function updateSE() {
        if (title == "" || idCardCur == null) {
          //special case when couldnt parse it
          estimation = 0;
          spent = 0;
          hashtags = [];
          return;
        ...
        if (seFromDb) {
          ...
          se.estimate = 0;
          se.spent = 0;
        } else if (se.spent != 0 || se.estimate != 0) bSEFromTitle = true;
        ...
        if (bRecurring) cleanTitle = replaceString(cleanTitle, g_regexRecurringTitleReplace, "");

        var ctlContents = cloneTitleTag.contents();
        var ctlUpdate = null;

        if (ctlContents && ctlContents.length >= 2) ctlUpdate = ctlContents[1];

        if (!ctlUpdate)
          //trello might have changed their page. try with a regular <A> without children
      ...
      if (bNoBadges && bTourRunning) bNoBadges = false;
      ...
      if (remainBadge.size() > 0) remainBadge.contents().last()[0].textContent = remain;
      if (bSEFromTitle) remainBadge.addClass(szClassSEFromTitle);
      else remainBadge.removeClass(szClassSEFromTitle);
      ...
      if (estimateBadge.size() > 0) estimateBadge.contents().last()[0].textContent = estimation;
      if (bSEFromTitle) estimateBadge.addClass(szClassSEFromTitle);
      else estimateBadge.removeClass(szClassSEFromTitle);
      ...
      if (spentBadge.size() > 0) spentBadge.contents().last()[0].textContent = spent;

      if (bSEFromTitle) spentBadge.addClass(szClassSEFromTitle);
      else spentBadge.removeClass(szClassSEFromTitle);
      ...
      if (true) {
        ...
        if (elRecurring.length == 0) {
          var imgRecurring = $("<img class='agile_image_recurring_back'>").attr(
            "src",
            chrome.runtime.getURL("images/recurring.png")
          );
        ...
        if (bRecurring) elRecurring.show();
        else elRecurring.hide();
      }
      ...
      if (hashtagsJq.length == 0) {
        ...
        if (htChildren.length == hashtags.length) {
          ...
          for (iChildren = 0; iChildren < htChildren.length; iChildren++) {
            if (htChildren[iChildren].textContent != hashtags[iChildren]) break;
          }
          if (iChildren == htChildren.length) bSameHashtags = true;
        }
        if (!bSameHashtags) hashtagsJq.html("");
      }
        ...
        for (var i = 0; i < hashtags.length; i++) {
          ...
          hashtagsJq.append(
            spanLoop
              .addClass(
                i == 0
                  ? "badge agile_badge agile_badge_hashtag agile_badge_hashtag_primary"
                  : "badge agile_badge agile_badge_hashtag agile_badge_hashtag_secondary"
              )
              .html(hashLoop)
          );
          if (hashLoop.indexOf("!") >= 0) spanLoop.addClass("agile_badge_hashtag_shout");
        }
      }
    ...
    if (totalEstimation == 0 && totalSpent == 0 && !bTourRunning) divSE.hide();
    else {
  ...
  if (!bShowHeaderStuff && globalTotalEstimation == 0 && globalTotalSpent == 0) {
    ...
    if (defaultSE && !bShowBoardTotals && !bFilteredCard) {
      //use the cache
    ...
    elemShowHide(
      g_estimationTotal,
      g_bNoSE || g_bNoEst ? false : bShowHeaderStuff || !g_bProVersion ? true : g_serViews.board.e
    );
    elemShowHide(
      g_remainingTotal,
      g_bNoSE || g_bNoEst ? false : bShowHeaderStuff || !g_bProVersion ? true : g_serViews.board.r
    );
  ...
  if (
    g_globalTotalSpent != null &&
    (g_globalTotalSpent != globalTotalSpent || g_globalTotalEstimation != globalTotalEstimation)
  ) {
    if (g_bSkipUpdateSsLinks) g_bSkipUpdateSsLinks = false;
    else bSetTimeout = true;
  }
  ...
  if (bSetTimeout)
    setTimeout(function () {
      updateSsLinksDetector(globalTotalSpent, globalTotalEstimation);
    }, 300); //let it breathe.
...
function updateWorker(bShowBoardTotals) {
  ...
  if (!g_bForceUpdate && isTimerRunningOnScreen()) return;
...
function showSFTDialog() {
  ...
  chrome.storage.sync.get([PROP_SFTDontWarnAgain], function (obj) {
    if (chrome.runtime.lastError == undefined && obj && obj[PROP_SFTDontWarnAgain]) return;
    var divDialog = $("#agile_dialog_SFTWarning");

    if (divDialog.length > 0) return; //show at most once per page cold load
...
<br /><input style="vertical-align:middle;margin-bottom:0px;"  type="checkbox"  id="agile_check_SFTDontWarnAgain"><label style="display: inline-block;font-weight:500;"  for="agile_check_SFTDontWarnAgain">Dont show me again</label></input>\
    ...
    divDialog
      .find("#agile_dialog_SFTWarning_OK")
      ...
      .on("click.plusForTrello", function (e) {
        ...
        setTimeout(function () {
          divDialog[0].close();
        }, 300 + 10); //wait for animation to complete
      });
...
function updateCards(boardCur, responseParam, bShowBoardTotals, bRecalcAgedCards) {
  if (isPlusDisplayDisabled()) return;
  if (bRecalcAgedCards === undefined) bRecalcAgedCards = true;

  function response() {
    if (bRecalcAgedCards) doShowAgedCards(g_bShowAllItems);
  ...
  if (boardCur == null || g_remainingTotal == null) {
    ...
    if (boardCur == null && idCardParsed) {
      ...
      if (elemDetect.length > 0) {
        ...
        if (elemCardAny.length == 0) removeTimerForCard(idCardParsed, true);
      }
    }
  ...
  chrome.storage.local.get([keySE, keyId], function (obj) {
    ...
    if (!idBoard && valueId && valueId.idBoard) idBoard = valueId.idBoard;
    ...
    function doIt() {
      if (true) {
        var sql =
          "select CB.idCard, SUM(CB.spent) as sumSpent, SUM(CB.est) as sumEst FROM CARDBALANCE AS CB join CARDS AS C ON CB.idCard=C.idCard WHERE C.idBoard=? \
                            group by CB.idCard";
        params = [idBoard];
        ...
        if (dimension != VAL_COMBOVIEWKW_ALL && dimension != VAL_COMBOVIEWKW_KWONLY) {
          ...
          sql =
            "select H.idCard, SUM(H.spent) as sumSpent, SUM(H.est) as sumEst FROM HISTORY AS H WHERE H.idBoard=? AND H.keyword=? \
                            group by H.idCard";
...
function setUpdatingGlobalSums(boardCur, bUpdating) {
  if (g_bUpdatingGlobalSums !== null && g_bUpdatingGlobalSums == bUpdating) return;
  ...
  if (!bUpdating) {
    ...
    setTimeout(function () {
      if (g_bUpdatingGlobalSums) updateCards(boardCur, null, true);
    }, 3000);
  }
}
...
function processCardTimerIcon(stored, container, idCard, stateLoop) {
  ...
  if (stored !== undefined && stored.msEnd == null) {
    ...
    if (imgTimer.length == 0) {
      imgTimer = $("<img>")
        .attr("src", chrome.runtime.getURL("images/iconspent.png"))
        .addClass("agile_timer_icon_small");
...
function getCurrentBoard() {
  if (getIdBoardFromUrl(document.URL) == null && getIdCardFromUrl(document.URL) == null) return null;
  var elemBoard = $(".board-main-content");
  if (elemBoard.length == 0) return null;

  elemBoard = elemBoard.find('[data-testid="board-name-display"]');
  if (elemBoard.length == 0) return null;

  var ret = elemBoard.text().trim();
  if (ret == "") ret = null;
  return ret;
}
...
var InfoBoxManager = {
  update: function () {
    ...
    if (boardHeader.length != 1) return;

    if (g_remainingTotal.parent()[0] === boardHeader[0])
      //optimize
      return;
    ...
    if (burndownLink.length != 0) boardHeader.append(burndownLink);

    var reportLink = $(".agile_plus_report_link");
    if (reportLink.length != 0) boardHeader.append(reportLink);

    var viewKW = g_bheader.comboSEView;
    if (viewKW && viewKW.length != 0) boardHeader.append(viewKW);

    var spanFilter = $(".agile_plus_filter_span");
    if (spanFilter.length != 0) boardHeader.append(spanFilter);
  },
};
...
var InfoBoxFactory = {
  ...
  makeTotalInfoBox: function (type, bBoardTotals) {
    ...
    if (bBoardTotals) box.addClass("agileBox_right");
    else box.addClass("agileBox_home");
    if (type == ESTIMATION) {
