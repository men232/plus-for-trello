...
function redrawCharts(bDontRedrawUser) {
  if (!bDontRedrawUser) drawChartUser();
  if (g_tl.crosshair) g_tl.crosshair.hide(); //could reposition (like annotations) but not worth it

  if (g_tl.projectionLine) g_tl.projectionLine.hide();

  if (g_tl.container) {
    //add in case it was removed by a previous resize (in checkHideTimelineSpark)
    if (g_tl.chartBottom) g_tl.container.add(g_tl.chartBottom, 2, 1);

    if (g_tl.xAxisBottom) g_tl.container.add(g_tl.xAxisBottom, 3, 1);
    ...
    if (g_tl.redrawAnnotations) g_tl.redrawAnnotations();
  }
}
...
function setSql(sql, values, elemsParam) {
  ...
  if (elems["orderBy"] == "date")
    //review default sort
    delete elems["orderBy"];

  if (elems["archived"] == "0") delete elems["archived"];

  if (elems["deleted"] == "0") delete elems["deleted"];
  ...
  function fixField(field) {
    ...
    if (elems[field] != "true") delete elems[field];
  }
  ...
  if (!g_bDontQuery) configBoardBurndownData(null, elems);
  else {
    g_bDontQuery = false;
  ...
  openPlusDb(function (response) {
    ...
    getSQLReport(sql, values, function (response) {
      ...
      try {
        ...
        if (bUniqueBoard) {
          if (rows && rows.length > 0) updateTitle(rows[0].nameBoard);
          else {
            getSQLReport("select name from BOARDS where idBoard=?", [elems["idBoard"]], function (response) {
              rows = response.rows;
              if (rows && rows.length > 0) updateTitle(rows[0].name);
              else updateTitle("board not synced");
            });
          }
        } else {
...
function updateBoardLinks(params) {
  ...
  $("#projectionLink").show();
  $("#saveBurndown").show();
  if (paramIdBoard) idBoard = decodeURIComponent(paramIdBoard);

  if (idBoard) {
    delete params["board"]; //dont pass it back to the report. it was historically just for title display purposes (june  2016)
    $("#reportLink")
      .attr("href", chrome.runtime.getURL("report.html?chartView=s&idBoard=") + paramIdBoard + "&weekStartRecent=true")
      .show();
    $("#boardLink")
      .attr("href", "https://trello.com/b/" + paramIdBoard)
      .show();
  } else {
    $("#reportLink").hide();
    $("#boardLink").hide();
...
function loadBurndown() {
  ...
  if (paramNameBoard) boardName = decodeURIComponent(paramNameBoard);
  ...
  elemHideAnnotations.click(function () {
    if (!g_sql) return; //too early
    params = getUrlParams();
    if (elemHideAnnotations[0].checked) params["checkHideAnnotationTexts"] = "true";
    else delete params["checkHideAnnotationTexts"];
  ...
  elemHideZoom.click(function () {
    if (!g_sql) return; //too early
    params = getUrlParams();
    if (elemHideZoom[0].checked) params["checkHideZoomArea"] = "true";
    else delete params["checkHideZoomArea"];
  ...
  chrome.storage.local.get([PROP_TRELLOUSER, PROP_SHOWBOARDMARKERS], function (obj) {
    ...
    if (bDontQuery) setTimeout(doFilterClick, 250); //without timeout it wont work. likely from timing of resizeMe. did not investigate further
  });
}
...
function makeRowMarkerParams(rowOrig, totalByUser) {
  ...
  if (estEnd > estStart || spentEnd > estStart) colors.seEnd = true;
...
function loadBoardMarkers(idBoard, totalByUser) {
  var sql =
    "SELECT datetime(dateStart,'unixepoch','localtime') as dateStartStr, datetime(dateEnd,'unixepoch','localtime') as dateEndStr, dateStart, dateEnd, userMarked, userMarking, spentStart, estStart, spentEnd, estEnd, nameMarker \
...
function addRowMarkerData(table, rowData, colors, bHeader) {
  ...
  for (i in rowData) {
    ...
    if (colors[i]) elem.css("background", "lightcoral");
    row.append(elem);
  }
  table.append(row);
}
...
function resetChartline() {
  if (g_tl.pointer) {
    if (g_tl.plot) g_tl.pointer.detachFrom(g_tl.plot);
    g_tl.pointer = null;
  }
  g_tl.plot = null;
  g_tl.crosshair = null;
  ...
  if (g_tl.container) g_tl.container.destroy();
  g_tl.container = null;
...
function loadTimeline(series, params) {
  ...
  plotAnnotations.attr("title", function (d) {
    return (
      ...
      d.tooltip +
      "</div><div>" +
      makeDateCustomString(d.x) +
      " (" +
      getCurrentWeekNum(d.x) +
      ")</div><div>Total S:" +
      d.sumSpent +
      "&nbsp;&nbsp;E:" +
      d.y +
  ...
  function redrawAnnotations() {
    if (!g_paramsLast) g_paramsLast = getUrlParams();
  ...
  function onUpdateXScale() {
    ...
    if (g_tl.crosshair) g_tl.crosshair.hide();
  ...
  var colorScale = new Plottable.Scales.Color()
    .range(g_TimelineColors)
    .domain(["Spent", "Estimate", "Remain", "Annotation"]);
  ...
  g_tl.container = new Plottable.Components.Table([
    //ALERT: resize code assumes table positions
  ...
  pointer.onClick(function (p) {
    ...
    if (!datum) return; //for future
    var d = datum.drill;
    var html = getHtmlBurndownTooltip(
      d.user,
      d.card,
      d.date,
      d.spent,
...
function createProjectionLine(plot, xFormatter, xScale, yScale) {
  ...
  projection.lineDom = container
    .append("line")
    .attr("stroke", g_colorRemaining)
    .attr("stroke-width", 1)
    .attr("stroke-dasharray", "5,5");
  projection.circleStart = container
    .append("circle")
    .attr("stroke", g_colorRemaining)
    .attr("fill", "white")
    .attr("r", 6);
  projection.circleMid = container.append("circle").attr("stroke", g_colorRemaining).attr("fill", "black").attr("r", 3);
  projection.circleEnd = container
    .append("circle")
    .attr("stroke", g_colorRemaining)
    .attr("fill", g_colorRemaining)
    .attr("r", 6)
    .style("visibility", "hidden");
  projection.labelBackground = container
    .append("rect")
    .attr("width", 0)
    .attr("height", 0)
    .attr("fill", "white")
    .attr("rx", 3)
    .attr("ry", 3)
    .attr("stroke", g_colorRemainingDark)
    .attr("stroke-width", 1)
    .style("visibility", "hidden");
  projection.labelEnd = container
    .append("text")
    .attr("stroke", g_colorRemainingDark)
    .attr("stroke-width", 1)
  ...
  projection.drawAt = function (p) {
    ...
    if (projection.bProjectionFirstClick) {
      ...
      attr.x2 =
        ((p.x - g_projectionData.x1) / (p.y - g_projectionData.y1)) * (attr.y2 - g_projectionData.y1) +
        g_projectionData.x1;
      ...
      var widthLabel = 150;
      if (attr.x2 + widthLabel > plot.width()) projection.labelEnd.attr({ x: attr.x2 - widthLabel, y: attr.y2 - 13 });
...
function createCrosshair(plot, yScale) {
  ...
  crosshair.vLine = crosshairContainer
    .append("line")
    .attr("stroke", g_colorTrelloBlack)
...
function setChartData(rows, idBoard, params) {
  ...
  for (; i < lengthRows; i++) {
    ...
    //rowid -1
    if (
      cSkipElemInTimeline == 0 &&
      row.rowid != ROWID_REPORT_CARD &&
      row.comment &&
      row.comment.indexOf(g_prefixCommentTransferTo) >= 0 &&
      i + 1 < lengthRows &&
      spent == 0
    ) {
      ...
      if (
        rowNext.rowid &&
        row.rowid + 1 == rowNext.rowid &&
        rowNext.comment &&
        rowNext.comment.indexOf(g_prefixCommentTransferFrom) >= 0 &&
        rowNext.spent == 0 &&
    ...
    var objHtml;
    if (user) {
      //!user when row is a card due date
      if (totalByUser[user] === undefined) totalByUser[user] = { s: 0, e: 0, sNotR: 0, eNotR: 0, data: [] };
      ...
      if (iAnnotation == 0 || comment.indexOf("] !") > 0)
        //needs to start with ! (] happens when Spent autoinserts markers like [+E] in the comment
      ...
      objHtml = {
        ...
        card: card,
        date: date,
        spent: parseFixedFloat(spent),
        est: parseFixedFloat(est),
        spentSum: spentTotalDisplay,
        estSum: estTotalDisplay,
        remainSum: remainTotalDisplay,
        idCard: idCard,
        note: comment,
      };
      ...
      if (annotation)
        seriesTimeline.annotation.push({
          x: date,
          y: estTotalDisplay,
          stroke: g_TimelineColors[3],
          text: shortenString(annotation),
          tooltip: annotation,
          sumSpent: spentTotalDisplay,
          sumR: remainTotalDisplay,
        });
    } else {
      if (row.dateDue) {
        ...
        if (weekStart || weekEnd) {
          ...
          if ((weekStart && week < weekStart) || (weekEnd && week > weekEnd)) bSkip = true;
        }
        if (!bSkip) {
          objHtml = {
            user: user,
  ...
  if (rows.length == 0) {
    ...
    if (g_bShowBoardMarkers && idBoard != null) loadBoardMarkers(idBoard, totalByUser);
    drawChartUser();
  }
}
...
function getHtmlBurndownTooltipByUser(rowsParam, bReverse, colExclude, selection) {
  ...
  if (bUniqueBoard) header.splice(1, 1);
  ...
  function callbackRowData(row) {
    ...
    if (!bUniqueBoard) {
      ...
      rgRet.push({
        name: "<A target='_blank' href='" + urlBoard + "'>" + escapeHtml(strTruncate(row.nameBoard)) + "</A>",
        bNoTruncate: true,
      });
    }
    ...
    rgRet.push({
      name: "<A target='_blank' href='" + urlCard + "'>" + escapeHtml(strTruncate(row.nameCard)) + "</A>",
      bNoTruncate: true,
    });
    ...
    if (bRemain) {
      if (rPush === 0) return null;
    } else {
      if (sPush === 0) return null;
    }
