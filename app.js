...
var g_bheader = {
  ...
  comboSEView: null,
  hide: function () {
    if (this.comboSEView) this.comboSEView.hide();
  },
};
...
var g_serViews = {
  board: { s: true, e: true, r: true },
  list: { s: true, e: true, r: false },
  card: { s: true, e: true, r: false },
};
...
document.addEventListener("visibilitychange", function () {
  ...
  testExtension(function () {
    if (g_bDidInitialIntervalsSetup && document.body)
      //once we got here without a body, so do some sanity checks for that timing issue
      doAllUpdates(true);
  });
});
...
function insertCardTimer(containerBar) {
  ...
  function tryInsert() {
    if (!inserted()) setTimeout(tryInsert, 200);
  }
  ...
  function inserted() {
    ...
    if (divInsert.find($("#agile_timer")).size() != 0) return true;
...
function showExtensionUpgradedError(e, bNeedsBackgroundUpgrade) {
  ...
  if (e && e.message && !bIgnoreError(e.message)) message = e.message;
...
<a title="Ignore to keep working on this page.\nSome Plus features may not work until you Reload." href="" class="button-link agile_dialog_Postit_button" id="agile_dialog_ExtensionUpgraded_Ignore">Ignore</a> \
    ...
    var imgReload = $("<img>")
      .attr("src", chrome.runtime.getURL("images/reloadchrome.png"))
      .addClass("agile_reload_ext_button_img");
    ...
    reload.off("click.plusForTrello").on("click.plusForTrello", function (e) {
      e.preventDefault();
      if (bOnReload) return; //extra reentry safety
      bOnReload = true;
      setTimeout(function () {
        //timeout so the button reacts to the click uxwise
    ...
    divDialog
      .find("#agile_dialog_ExtensionUpgraded_Ignore")
      .off("click.plusForTrello")
      .on("click.plusForTrello", function (e) {
        if (bNeedsBackgroundUpgrade) localStorage.setItem(PROP_LS_MSLAST_IGNORE_EXTUPGRADE, Date.now());
...
<span style="float:right;margin-right:1em;"><A href="http://www.plusfortrello.com/p/plus-for-trello-pro-version.html" target="_blank">Help</A></span>\
  ...
  function initDialog() {
    ...
    divDialog
      .find("#agile_dialog_FirstLic_OKWebstore")
      .off("click.plusForTrello")
      .on("click.plusForTrello", function (e) {
        e.preventDefault();
    ...
    divDialog
      .find("#agile_dialog_FirstLic_Cancel")
      .off("click.plusForTrello")
      .on("click.plusForTrello", function (e) {
        e.preventDefault(); //link click would navigate otherwise
        doCloseDialog(function () {
          if (bOvertime)
            setTimeout(function () {
              sendDesktopNotification(
                "If you do not wish to activate please turn off 'Pro' from the Plus Help pane.",
                10000
              );
            }, 1000);
          callback("cancel");
...
<img style="padding:3px;margin-left:0.5em;margin-bottom:-7px;" src="' +
  ...
  if (!g_bForceShowTry) {
    setTimeout(function () {
      if (divDialog.hasClass("agile_dialog_Postit_Anim_ShiftToShow")) {
        //class indicates we havent started anim&close process
...
<a style="float:right;" href="" class="button-link agile_dialog_Postit_button" id="agile_dialog_FatalError_Ignore">Ignore</a> \
    ...
    divDialog.find("#agile_dialog_FatalError_ViewLog").prop("href", chrome.runtime.getURL("plusmessages.html"));
    divDialog
      .find("#agile_dialog_FatalError_Ignore")
      .off("click.plusForTrello")
      .on("click.plusForTrello", function (e) {
        e.preventDefault(); //link click would navigate otherwise
        divDialog[0].close();
...
function testExtension(callback) {
  ...
  try {
    ...
    sendExtensionMessage(
      { method: "testBackgroundPage", logMessages: rgLog },
      function (response) {
  ...
  if (g_verStore && g_verStore != g_manifestVersion) {
    ...
    if (partsStore.length != partsCurrent.length) {
      ...
      for (var iPartsVersion = 0; iPartsVersion < partsStore.length; iPartsVersion++) {
        ...
        if (vCur > vStore) break;
        if (vStore > vCur) bNeedUpgrade = true;
...
function handleIframeLoad() {
  ...
  if (url.indexOf("https://trello.com/embed/card?") == 0) {
    ...
    function checkTitle() {
      ...
      if (!title) setInterval(checkTitle, 200);
      else {
...
$(function () {
  loadExtensionVersion(function () {
    setTimeout(function () {
      //in timeout so we can safely reference globals and give a little time for trello load itself since we  "run_at": "document_start"
...
function setTrelloAuth(callback, bInFrame) {
  ...
  if (dscNew && dscNew != g_dscTrello) {
    ...
    sendExtensionMessage({ method: "setTrelloAuthData", dsc: dscNew, bInFrame: bInFrame }, function () {
      if (callback) setTimeout(callback, 1); //prevent recursing from message callback
    });
  } else if (callback) {
    callback();
  }
...
function loadOptions(callback) {
  ...
  chrome.storage.sync.get(
    [
      ...
      SYNCPROP_NO_SE,
      SYNCPROP_SERVIEWS,
      SYNCPROP_KEYWORDS_HOME,
      keyDisplayPointUnits,
      SYNCPROP_GLOBALUSER,
      SYNCPROP_BOARD_DIMENSION,
      SYNCPROP_bStealthSEMode,
      SYNCPROP_language,
      keyServiceUrl,
      keybDontShowTimerPopups,
      keybDontShowSpentPopups,
      keyClosePlusHomeSection,
      keyDontWarnParallelTimers,
      keyUnits,
      keyrgExcludedUsers,
      ...
      keyAllowNegativeRemaining,
      keyPreventIncreasedE,
      keyAlreadyDonated,
      keybEnableTrelloSync,
      keyCheckedTrelloSyncEnable,
      keyHidePendingCards,
      keyAlwaysShowSEBar,
      keyUseLastSEBarUser,
      keyDowStart,
      ...
      keySyncOutsideTrello,
      keybHideTour,
      keybChangeCardColor,
      keyPropbSumFilteredCardsOnly,
      keybDisabledSync,
    ],
    function (objSync) {
      if (BLastErrorDetected()) return;
      try {
      ...
      chrome.storage.local.get(
        [LOCALPROP_PRO_VERSION, LOCALPROP_PRO_MSDATEENABLED, LOCALPROP_EXTENSION_VERSIONSTORE],
        function (obj) {
          if (BLastErrorDetected()) return;
          ...
          if (!g_bProVersion || g_msStartPro > 0) callback();
          else {
            //fix g_msStartPro
...
function doAllUpdates(bFromInterval) {
  if (bFromInterval) {
    var msNow = Date.now();
    if (msNow - g_msLastAllUpdatesInterval < UPDATE_STEP * 0.8) return; //prevent frequent calls (we pretend bFromInterval in some situations)
  ...
  if (idCard) sendExtensionMessage({ method: "notifyCardTab", idCard: idCard }, function (response) {});
  else {
        ...
        if (boardmenu.length > 0) {
          ...
          if (settingsIcon.length > 0 && settingsIcon.is(":visible")) {
            ...
            if (boardmenumav.length > 0) {
              ...
              pftSettings = $(
                "<A class='agile-pft-boardsettings board-menu-navigation-item-link' href=''>'Plus' Help & Settings</A>"
              );
  ...
  if (bFromInterval && !isTabVisible()) return;

  markForUpdate();
  if (isPlusDisplayDisabled()) return;

  if (g_delayedActions.bPendingWeeklyReport) doWeeklyReport(g_configData, getCurrentTrelloUser(), false, true);
...
function updateSsLinksDetector(globalTotalSpent, globalTotalEstimation) {
  ...
  if (user != null && globalTotalSpent == g_globalTotalSpent && globalTotalEstimation == g_globalTotalEstimation)
    ...
    setTimeout(function () {
      updateSsLinksDetector(gTSLocal, gTELocal);
    }, 500); //try later until it stabilizes
  }
}
    ...
    sendExtensionMessage({ method: "detectLegacyHistoryRows" }, function (response) {
      if (response.hasLegacyRows) {
        if (g_optEnterSEByComment.IsEnabled() && !g_optEnterSEByComment.hasLegacyKeyword()) {
          if (
            !confirm(
              "The legacy keyword 'Plus S/E' is missing from your keywords, thus Plus will not be able to read your legacy rows.\nAre you sure you want to Reset?"
            )
          )
            return;
        }
      ...
      sendExtensionMessage({ method: "isSyncing" }, function (response) {
        if (response.status != STATUS_OK) {
        ...
        sendExtensionMessage({ method: "getTotalDBRowsNotSync" }, function (response) {
          ...
          if (response.cRowsTotal > 0 && (g_bDisableSync || !g_optEnterSEByComment.IsEnabled())) {
            //review newsync
            if (!g_optEnterSEByComment.IsEnabled() && g_strServiceUrl && g_strServiceUrl.length > 0) {
              if (
                !confirm(
                  "You have pending S/E rows that havent synced yet to the spreadsheet. Are you sure you want to lose those rows?"
                )
              )
                return;
            } else if (
              !confirm(
                "Sync is not enabled. S/E rows wont come back until you do so.'\nAre you sure you want to reset now?"
              )
            )
              return;
          }
