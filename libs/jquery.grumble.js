...
(function ($, window) {
  var defaults = {
    ...
    textTemplate:
      '<div class="grumble-text" style="display:none;"><div class="outer"><div class="inner">{text}</div></div></div>',
    context: null,
  };
  ...
  window.GrumbleBubble.prototype = {
    create: function () {
      ...
      this.bubble = $(tmpl(this.options.template)).css(
        "background-image",
        "url(" + chrome.runtime.getURL("images/bubble-sprite.png") + ")"
      );
...
(function ($, Bubble) {
  ...
  $(document).bind("keyup.bubble", function (event) {
    // Pressing the escape key will stop all bubbles
    if (event.keyCode === 27) {
  ...
  function getOffsets($me, context, angle) {
    ...
    if (context) {
      ...
      if (oParent.top > 0) offset.top += 10; //hack
    }
  ...
  function calculateTextHeight(defaultSize, range, text, bFindOptimal) {
    var el = $(
        '<div style="position:absolute;visibility:hidden;width:' + defaultSize + 'px;">' + text + "</div>"
      ).appendTo($(document.body)),
      height = el.outerHeight() * 2 + defaultSize * 0.2 /*the 20% is approx padding: could be more clever*/,
