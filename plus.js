...
function isSpecialPayTestUser() {
  ...
  if (!g_userTrelloCurrent) return false;
  if (g_userTrelloCurrent == "zmandel") return true;
  ...
  } else randUser = parseInt(randUser, 10);

  return true;
}

var g_waiterLi = CreateWaiter(2, function () {
  if (document.URL.indexOf("/trello.com/" + URLPART_PLUSLICENSE + "/") >= 0) return;

  assert(g_userTrelloCurrent); //waiters ensure user and db are loaded at this point
  checkFirstTimeUse(function (bShowedDialog) {
    if (bShowedDialog) return;
    checkNeedsExtensionUpdate(function (bShowedDialog) {
      //review: should also check regularly for users that never close the trello tab
      if (bShowedDialog) return;
...
function checkNeedsExtensionUpdate(callback) {
  sendExtensionMessage({ method: "getDateUpdateNotificationReceived" }, function (response) {
    if (response && response.status == STATUS_OK) {
      ...
      var bUrgent = false;
      if (response.msDate > 0 && msNow - response.msDate > 1000 * 60 * 60 * 8) {
        //8 hours of pending update. so we dont bother users right away (give a chance for chrome to autoupdate)
        bShowUpdateNeeded = true;
        if (msNow - response.msDate > 1000 * 60 * 60 * 24 * 3)
          // 3 days old
          bUrgent = true;
      }
      ...
      if (msDateLastWarn > 0) {
        ...
        if (msDelta < msLimit) bShowUpdateNeeded = false;
      }
      if (bShowUpdateNeeded) showExtensionUpgradedError(null, true);
    }
  });
}

function resizeHelp(container) {
  if (!container) container = $("#agile_help_container");
...
function isPlusDisplayDisabled() {
  if (g_optIsPlusDisplayDisabled === null) g_optIsPlusDisplayDisabled = localStorage.getItem(g_lsKeyDisablePlus); //we want to read it once so its consistent for future calls. user must refresh page if changed

  if (g_optIsPlusDisplayDisabled == "true") return true;
  return false;
}
...
function isBackendMode(configData) {
  if (configData === undefined) {
    if (!g_bReadGlobalConfig) return false;
...
<a style="float:right;margin-top:2em;" target="_blank" href="http://www.plusfortrello.com/p/plus-for-trello-pro-version.html">Read more</a>.\
  ...
  divDialog
    .find("#agile_dialog_showAprovePro_Cancel")
    .off("click.plusForTrello")
    .on("click.plusForTrello", function (e) {
      doFinish(false);
    });
  ...
  divDialog
    .find("#agile_dialog_showAprovePro_OK")
    .off("click.plusForTrello")
    .on("click.plusForTrello", function (e) {
      doFinish(true);
    });
  ...
  showModalDialog(divDialog[0]);
  setTimeout(function () {
    if (!elementInViewport($("#agile_dialog_showAprovePro_OK")[0])) $("#agile-scrolldown-alert-pro").slideDown();
  }, 200);
}
...
function configureSsLinks(bParam) {
  if (g_strServiceUrl != null) {
    ...
    chrome.storage.sync.get("serviceUrl", function (obj) {
      ...
      if (strUrlNew === undefined || strUrlNew == null) strUrlNew = ""; //means simple trello
      strUrlNew = strUrlNew.trim();
      var keyUrlLast = "serviceUrlLast";
      chrome.storage.local.get(keyUrlLast, function (obj) {
        //must be local as we are comparing local configuration. each devices does the same
        var strUrlOld = obj[keyUrlLast];
        if (strUrlOld) strUrlOld = strUrlOld.trim();
        else strUrlOld = "";
    ...
    if (xhr.readyState == 4) {
      ...
      function handleFinishRequest() {
        ...
        if (xhr.status == 200) {
          ...
          if (bHandledDeletedOrNoAccess(xhr.status, objRet, "error: permission error or deleted")) {
...
function getCurrentTrelloUser() {
  ...
  if (user) return user;
  if (g_bMakingUserApiCall) return null;
  var header = $("#header");
...
function getCurrentTrelloUserWorker() {
  if (g_userTrelloCurrent != null) return g_userTrelloCurrent;
  var header = $("#header");
  ...
  if (elemUserMenu.length == 0) return null;
  ...
  if (iParenOpen < 0) return null;
  userElem = userElem.substring(iParenOpen + 1);
  var iParenClose = userElem.lastIndexOf(")");
  if (iParenClose < 0 || iParenClose != userElem.length - 1) return null;
  userElem = userElem.substring(0, iParenClose);
  if (userElem.length == 0) return null; //not needed, but might help when trello suddenly changes DOM
  ...
  chrome.storage.local.get([PROP_TRELLOUSER], function (obj) {
    ...
    if (userTrelloLast != userElem) {
      if (userTrelloLast) sendDesktopNotification("Warning: Trello user changed. Reset Sync from Plus help", 30000);
      var pairUser = {};
      pairUser[PROP_TRELLOUSER] = userElem;
      chrome.storage.local.set(pairUser, function () {
        if (chrome.runtime.lastError != undefined) alert("Plus for Trello:" + chrome.runtime.lastError.message);
      });
    }
  });
  return userElem;
...
function setWeekSummaryProgress(elem) {
  ...
  if (elemTable.hasClass(strClass)) return false;
...
function configureSsLinksWorker(b, url, bSkipConfigCache) {
  ...
  if (userElem == null) {
    ...
    setTimeout(function () {
      configureSsLinksWorker(b, url, bSkipConfigCache);
    }, 500);
    return;
  }
  ...
  if (urlUserElem.length == 0) {
    ...
    if (!isPlusDisplayDisabled()) {
      if (g_bNoSE) urlUserElem.hide();
      getRecentWeeksList().appendTo(b);
    }
  }

  if (!isPlusDisplayDisabled()) checkCreateRecentFilter(b);
  ...
  function part2() {
    sendExtensionMessage(
      { method: "getConfigData", userTrello: userElem, urlService: url, bSkipCache: bSkipConfigCache },
      function (respConfig) {
  ...
  if (!g_bCheckedTrelloSyncEnable && !g_bEnableTrelloSync) {
    ...
    if (!g_bDisableSync) {
      ...
      chrome.storage.sync.set(pairTrelloSync, function () {
        if (chrome.runtime.lastError != undefined) alert("Plus for Trello:" + chrome.runtime.lastError.message);
        else g_bEnableTrelloSync = true; //note: this is a safe place to init this global. be careful if init code changes.
        part2();
      });
...
function initialIntervalsSetup() {
  ...
  if (isPlusDisplayDisabled()) return;
  ...
  setInterval(function () {
    ...
    sendExtensionMessage({ method: "queryTrelloDetectionCount" }, function (response) {
      if (response.status != STATUS_OK) return;

      var msNow = Date.now();
      if (response.count == 0) {
        if (msLastDetectedActivity == 0) return;
        if (msNow - msLastDetectedActivity > 500) {
          //this prevents many consecutive change detections from triggering many syncs
...
function onReadGlobalConfig(configData, user) {
  ...
  if (isBackendMode(configData)) g_bShowBoardMarkers = true;
  var pair = {};
...
function setSyncErrorStatus(urlUser, status, statusLastTrelloSync) {
  ...
  if (statusLastTrelloSync == "busy") statusLastTrelloSync = STATUS_OK; //dont bother the user when busy
  ...
  if (status == STATUS_OK && statusLastTrelloSync == STATUS_OK) {
    ...
    if (g_bDisableSync || (g_strServiceUrl == "" && !g_optEnterSEByComment.IsEnabled())) strTip = "";
  ...
  if (status != STATUS_OK) {
    if (status && status.indexOf("error:") >= 0) statusSet = status;
    else statusSet = "error: " + (status ? status : "unknown error");
  }

  if (statusLastTrelloSync != STATUS_OK) {
    if (statusSet != "") statusSet = statusSet + "\n";
...
function startOpenDB(config, user) {
  ...
  openPlusDb(
    function (response) {
      ...
      if (!isPlusDisplayDisabled()) {
        g_intervalSync = setInterval(function () {
          doSyncDB(user, true, false, false);
        }, g_msSyncPeriod);
...
function programUnZoom(userElem) {
  if (g_idTimeoutReportHover == null) {
    //note: !=null can actually happen in rare cases involving switching windows while on the hover timeout wait
...
function checkFirstTimeUse(callback) {
  ...
  sendExtensionMessage({ method: "getTotalDBRows" }, function (response) {
    ...
    chrome.storage.local.get(
      [LOCALPROP_NEEDSHOWPRO, keyDateLastSetupCheck, LOCALPROP_DONTSHOWSYNCWARN],
      function (obj) {
        ...
        if (bSyncNotEnabled && !g_bDisableSync) {
          //sync not set up ever
          if (msDateLastSetupCheck !== undefined) {
            if (totalDbRowsHistory > 0) {
              if (msDateNow - msDateLastSetupCheck > 1000 * 60 * 60 * 24)
                //nag once a day
                bShowHelp = true;
            }
          } else {
        ...
        if (bShowHelp) {
          ...
          if (!boardCurrent || boardCurrent.toLowerCase().indexOf("plus for trello") < 0) {
            //skip the plus for trello help board
            var msNow = Date.now();
            ...
            if (msNow - msLastHiliteNoSync > 1000 * 60 * 5) {
              //can be negative
              bShowedDialog = true;
...
function updateCRowsTotalState(cRowsTotal, config, user) {
  ...
  if (bNewRows || cRowsTotal == 0) {
    //cRowsTotal==0 is a hack so the "first sync" status text gets updated after a first sync with no rows
    g_bForceUpdate = true;
    g_seCardCur = null; //mark as uninitialized. will be set on the refresh below
    if (!isTabVisible()) g_delayedActions.bPendingWeeklyReport = true;
    else doWeeklyReport(config, user, false, true);
  }
}
...
function onDbOpened() {
  ...
  if (g_portBackground == null) {
    ...
    g_portBackground.onMessage.addListener(function (msg) {
      var urlUser = $("#urlUser");
      if (msg.status) setSyncErrorStatus(urlUser, msg.status, msg.statusLastTrelloSync);
      ...
      if (msg.event == EVENTS.FIRST_SYNC_RUNNING) {
        ...
        if (!msg.bNewHistoryRows) {
          //NEW_ROWS is already handled above
          var user = getCurrentTrelloUser();
          if (user) {
            g_bForceUpdate = true;
            if (!isTabVisible()) g_delayedActions.bPendingWeeklyReport = true;
            else doWeeklyReport(g_configData, user, false, true);
          }
        }
...
function doSyncDB(userUnusedParam, bFromAuto, bOnlyTrelloSync, bRetry, bForce) {
  if (Help.isVisible() || isPlusDisplayDisabled()) return;
  bOnlyTrelloSync = bOnlyTrelloSync || false;
  if (bRetry === undefined) bRetry = true;
  ...
  sendExtensionMessage({ method: "queryTrelloToken" }, function (response) {
    tokenTrello = response.cookie_value;
    if (bFromAuto && !bForce) {
      if (!isTabVisible()) {
        return;
      }
  ...
  function worker() {
    if (g_cRetryingSync > 0) return;
    urlUser = $("#urlUser"); //set/refresh
    if (!setWeekSummaryProgress(urlUser)) return;
    ...
    if (g_bEnableTrelloSync && !bDidTrelloSync && !bEnterSEByComments) {
      AuthAndsendExtensionMessage(
        { method: "trelloSyncBoards", tokenTrello: tokenTrello, bUserInitiated: !bFromAuto },
        function (response) {
          ...
          if (bOnlyTrelloSync && !bSetStatus) {
            //when status is set, we do want to continue to google sync
            setSyncErrorStatus(urlUser, response.status);
            return;
          }
          doGoogleSync();
  ...
  function doRetry() {
    if (!bSetStatus) setSyncErrorStatus(urlUser, STATUS_OK);
    g_cRetryingSync++;
    setTimeout(function () {
      g_cRetryingSync--;
      worker();
  ...
  function doGoogleSync() {
    ...
    bSetStatus = false;
    AuthAndsendExtensionMessage(
      { method: "syncDB", config: config, bUserInitiated: !bFromAuto, tokenTrello: tokenTrello },
      function (response) {
        statusSync = response.status;
        if (statusSync == "busy") doRetry();
...
function doWeeklyReport(config, user, bUpdateErrorState, bReuseCharts, bRefreshCardReport) {
  if (!user) return; //safety in case trello DOM structure breaks
  ...
  if (bUpdateErrorState === undefined) bUpdateErrorState = true;

  if (bRefreshCardReport === undefined) bRefreshCardReport = true;

  bReuseCharts = bReuseCharts || false;
  if (bRefreshCardReport) refreshCardTableStats(); //review zig: register for refresh notifications instead of hardcoding here all these. also this causes two reports when card page refreshed
  var dateToday = new Date();
  var weekCur = getCurrentWeekNum();
  var dowToday = dateToday.getDay();
  var sToday = 0;
  if (weekCur != getCurrentWeekNum(dateToday)) sToday = null; //means we are not tracking "today" because the week selection is not the current week.

  var sql =
    "select H.idCard, H.user,H.spent,H.est,H.comment,C.name as nameCard, strftime('%w',H.date,'unixepoch','localtime') as dow, H.date, B.name as nameBoard,B.idBoard, H.eType from HISTORY H JOIN BOARDS B ON H.idBoard=B.idBoard JOIN CARDS C ON H.idCard=C.idCard AND C.bDeleted=0 WHERE week=? ";
  var values = [weekCur];
  ...
  g_cRowsWeekByUser = 0;
  getSQLReport(sql, values, function (response2) {
    ...
    if (response2.status == STATUS_OK) {
      for (; i < response2.rows.length; i++) {
        ...
        if (row.user == null) continue; //table empty
        if (ordered.length == 0 || ordered[ordered.length - 1][0] != row.user) {
          //note must be ordered by user
          ordered.push([row.user, 0, 0, 0, 0, 0, 0, 0]);
          drilldownData.push([row.user, [], [], [], [], [], [], []]);
          if (iCurrentUserOrder < 0 && row.user == curUser) iCurrentUserOrder = ordered.length - 1;
        }
      ...
      for (i = 0; i < ordered.length; i++) {
        row = ordered[i];
        var c = 1;
        for (; c < row.length; c++) row[c] = parseFixedFloat(row[c]); //reformat so charts dont have to
      }
    }
    ...
    if (iCurrentUserOrder >= 0) {
      ...
      for (; k < rowUser.length; k++) {
        if (rowUser[k] == 0) continue;
        sumDays += rowUser[k];
        var dow = DowMapper.dowFromPosWeek(k - 1);
        if (sToday !== null && dow == dowToday) sToday += rowUser[k];
        dataWeek.weekSummary.rows.push({ day: getWeekdayName(dow), total: rowUser[k] });
      }

      dataWeek.sWeek = sumDays;
    }
    if (sToday === null) dataWeek.sToday = null;
    else dataWeek.sToday = parseFixedFloat(sToday);
...
function addWeekDataByBoard(dataWeek, weekCur, response, callback) {
  ...
  if (response.status == STATUS_OK) {
    ...
    if (response.status == STATUS_OK) {
      for (; i < rows.length; i++) {
        var row = rows[i];
        if (row.user == null) continue; //review zig when does this happen, no data? (sql response status row)
      ...
      for (i = 0; i < ordered.length; i++) {
        rowOrder = ordered[i];
        var iCol = 1;
        for (; iCol < rowOrder.length; iCol++) rowOrder[iCol] = parseFixedFloat(rowOrder[iCol]); //format it here so charts dont have to
        while (rowOrder.length != colUserLast + 1) rowOrder.push(0);
      }
    }
  }
  dataWeek.byBoard = {
    ...
    users: users,
    drilldownData: drilldownData,
    bReuseCharts: dataWeek.bReuseCharts,
  };
  callback();
}
...
function insertHistoryRowFromUI(rows, callback) {
  sendExtensionMessage({ method: "insertHistoryRowFromUI", rows: rows }, function (response) {
    if (response.status != STATUS_OK) alert("Insert error: " + response.status);
...
function adjustSelectWidthToContent(combo) {
  ...
  var cch = val.length;
  if (cch > 4) combo.css("width", "auto");
  else combo.css("width", cch + "em");
}

function getRecentWeeksList() {
  var combo = $('<select id="spentRecentWeeks" />').addClass(
    "agile_title_combo agile_combo_input agile_boldfont agile_title_combo_input"
  );
  combo.css("cursor", "pointer");
  combo.attr("title", "click to change the week being viewed.");
  if (g_bNoSE) combo.hide();
...
function getKeywordsViewList() {
  ...
  if (combo.length == 0 && g_bheader.comboSEView) combo = g_bheader.comboSEView;

  if (combo.length == 0) {
    combo = $('<select id="agile_globalkeywordlist"/>').addClass(
      "agile_title_combo agile_combo_input agile_title_combo_input"
    );
    combo.css("cursor", "pointer");
  ...
  if (bUseKeywords || g_bDisableSync) {
    //if g_bDisableSync we want to at least show something, so show default keywords
  ...
  if (g_bAcceptSFT || g_bAcceptPFTLegacy) {
    if (bUseKeywords)
      rgItems.push({
        str: "Keywords only",
        val: VAL_COMBOVIEWKW_KWONLY,
        title: "S/E only from keywords (exclude card title s/e)",
      });
  ...
  if (rgItems.length >= 1) {
    ...
    if (bMultipleKeywords) rgItems.push({ str: "↗ Report by keyword", val: VAL_COMBOVIEWKW_REPORTKW });
    ...
    if (combo.val() != g_dimension) {
      //keyword no longer in use, default to all
      ...
      if (val == VAL_COMBOVIEWKW_REPORTKW) {
        ...
        if (idBoardCur) {
          var url =
            ...
            "?chartView=s&groupBy=keyword&orderBy=date&sortList=%5B%5B%22Keyword%22%2C0%5D%5D&idBoard=" +
            encodeURIComponent(idBoardCur);
...
function getAllUsersList() {
  ...
  chrome.storage.local.get("allUsersSpent", function (obj) {
    ...
    combo.append(
      $(new Option("Users", ""))
        .addClass("agile_users_combo_element agile_users_combo_element_disabled")
...
function configureSsLinksWorkerPostOauth(resp, b, user, bUpdateErrorState) {
  ...
  var nameSsLink = "";
  if (bUpdateErrorState) setSyncErrorStatus(urlUserElem, resp.status);

  if (!resp) {
    nameSsLink = "<tr><td>Plus disabled</td></tr>";
  } else {
    processUserSENotifications(resp.sToday, resp.sWeek);
    if (resp.weekSummary.rows.length == 0) nameSsLink = "<tr><td>Ø</td></tr>";
    else {
      var row1 = "";
      var row2 = "";
...
function checkTrelloLogo() {
  ...
  if (trelloLogo.length == 0 || !trelloLogo.is(":visible")) trelloLogo = $(".header-logo-pride");
...
function setupBurnDown(bShowHeaderStuff, bShowSumFilter) {
  ...
  if (board == null || g_remainingTotal === undefined) return false;

  if (!bShowHeaderStuff) bShowSumFilter = false;
  ...
  if (idBoard == null) return false;

  if (burndownLink.length == 0) {
    burndownLink = $("<img title='Plus - Board Burndown & Projections' style='display:none;'>")
      .attr("src", chrome.runtime.getURL("images/chart-sm.png"))
      .addClass("agile_img_boardheader agile_plus_burndown_link");
    burndownLink.insertAfter(g_spentTotal);
    burndownLink.click(function () {
      var idBoardCur = getIdBoardFromUrl(document.URL);
      if (idBoardCur == null) return false;
      var url = chrome.runtime.getURL("dashboard.html") + "?idBoard=" + encodeURIComponent(idBoardCur);
      window.open(url, "_blank");
      return false;
    });

    reportLink = $("<img title='Plus - Board Reports & Charts' style='display:none;'>")
      .attr("src", chrome.runtime.getURL("images/report-sm.png"))
      .addClass("agile_img_boardheader agile_plus_report_link");
    reportLink.insertAfter(burndownLink);
    reportLink.click(function () {
      var idBoardCur = getIdBoardFromUrl(document.URL);
      if (idBoardCur == null) return false;
      var url = chrome.runtime.getURL("report.html") + "?groupBy=idCardH&idBoard=" + encodeURIComponent(idBoardCur);
      window.open(url, "_blank");
      return false;
    });
    ...
    var filterSumCheck = $(
      '<input  type="checkbox" value="checked" id="agile_plus_filter_check"  style="margin-bottom:0px;margin-top:0.55em;float:right;min-height:0.5em;line-height:0.5em;height:1em;margin-bottom:0px;cursor:pointer;">'
    );
    var labelSumCheck = $(
      '<label for="agile_plus_filter_check" style="float:right;padding-right:0.4em;margin-top:0.40em;margin-bottom:0px;font-weight:normal;cursor:pointer;">'
    ).text("Sum filtered");
  ...
  if (!g_bNoSE && !g_bNoEst) burndownLink.show();
  reportLink.show();
  ...
  if (bShowSumFilter) spanFilter.show();
  else spanFilter.hide();

  return true;
}

function processUserSENotifications(sToday, sWeek) {
  if (sToday === null) return;
  try {
    var factor = 10;
    if (sWeek > 100) factor = 1;
    else if (sWeek < 10) factor = 100;
    var sBadge = Math.round(sWeek * factor) / factor;
    if (sBadge >= 1000) sBadge = "+999";

    if (sBadge == 0) sBadge = ""; //dont show badge when zero spent, its annoying to those not using S/E (even thou it can be turned off from preferences)
    ...
    chrome.storage.local.get(key, function (obj) {
      var value = obj[key];
      if (value != null) {
        if (strToday == value.strToday && sToday == value.sToday) return;
      }
      var pair = {};
...
function bAtTrelloHome() {
  ...
  if (url.lastIndexOf("/#") == url.length - 2) url = url.substring(0, url.length - 2);

  if (url != "https://trello.com/" && url != "https://trello.com/u/" + user + "/boards") return false;
  return true;
}
...
function insertFrontpageChartsWorker(mainDiv, dataWeek, user) {
  ...
  if (!dataWeek.bReuseCharts) g_chartsCache = {};
  ...
  if (divInsertAfter.length == 0) {
    if (divPrepend.length == 0) {
      setTimeout(function () {
        insertFrontpageChartsWorker(mainDiv, dataWeek, user);
      }, 50); //wait until trello loads that div
      return false;
    }
  }
  ...
  if (divSpentItems.length == 0) {
    ...
    var waiter = CreateWaiter(4, function () {
      ...
      function refreshAll() {
        ...
        divSpentItems.show();
        if (g_bPreventChartDraw) g_bPreventChartDraw = false;
    ...
    if (bNewTrelloHome) tableSpentItems.find("tbody").addClass("newTrelloHomeBackground");
    ...
    divItemDashboardRecent
      .addClass("agile_spent_item_title  agile_spent_item_combo agile_combo_input")
      .attr("title", "Your recent S/E.\n\nTip: control+click items to open in a new tab");
    var divItemDashboardUnspent = addModuleSection(true, false, cellA, "", idPendingModule, true, "left", false);
    divItemDashboardUnspent
      .addClass("agile_spent_item_combo agile_combo_input")
      .attr("title", "Your remaining S/E.\n\nTip: control+click items to open in a new tab");
    if (bNewTrelloHome) {
    ...
    chartModuleLoader(
      ...
      cellC,
      "Week by user",
      idChartModuleSpentWeekUsers,
      idChartModuleSpentWeekUsers + strPostfixStatus,
      dataWeek,
      loadChartSpentWeekUser,
      "left",
      true
    );
    chartModuleLoader(
      ...
      cellD,
      "Week by board",
      idChartModuleSpentWeekBoard,
      idChartModuleSpentWeekBoard + strPostfixStatus,
      dataWeek.byBoard,
      loadChartSpentWeekBoard,
      "left",
      true
    );
...
function chartModuleLoader(
  ...
  chrome.storage.local.get(nameLocal, function (obj) {
    ...
    if (heightLast === undefined) heightLast = "50px";
...
function addModuleSection(bCombobox, bEnableZoom, div, name, id, bHidden, strFloat, bLastRow) {
  if (bHidden === undefined) bHidden = false;
  ...
  if (true) {
    ...
    if (strFloat) divModule.css("float", strFloat);
  }
  ...
  if (bLastRow) divItem.addClass("agile_spent_item_lastRow");
  divModule.append(divItem);
  if (bHidden) divModule.hide();
...
function checkNotUsingSE() {
  ...
  getSQLReport(sql, [], function (response) {
    if (response && response.status == STATUS_OK && response.rows) {
      ...
      if (bNoSE) {
        setTimeout(function () {
          //wait a little for page to finish
          showTryNoSEDialog();
        }, 2000);
      }
    }
...
function doRecentReport(waiter, elemRecent, user) {
  ...
  var sql =
    "select count(*) as cGrouped, max(date*1000) as msDate, max(dateLocal) as dateLocal, nameBoard, nameCard, SUM(spent) as spent, sum(est) as est, coalesce(GROUP_CONCAT(comment,'\n'),'') as comment, idCard from \
  ...
  getSQLReport(sql, values, function (response) {
    ...
    if (g_msStartPlusUsage && g_cRowsHistoryLast < 20 && !g_bNoSE && !g_bNoEst) {
      ...
      if (msDelta > 1000 * 60 * 60 * 2) {
        //2 days
        if ((localStorage.getItem(PROP_LS_ASKEDNOTUSINGSE) || "") != "1") {
          sendExtensionMessage({ method: "completedFirstSync" }, function (response) {
            if (response && response.status == STATUS_OK && response.bCompletedFirstSync) {
              localStorage.setItem(PROP_LS_ASKEDNOTUSINGSE, "1");
              checkNotUsingSE();
            }
          });
        }
...
function doPendingReport(waiter, elemPending, user) {
  ...
  var sql =
    "select CB.user, CB.spent, CB.est, CB.diff, datetime(CB.date,'unixepoch','localtime') as dateLocal, B.name as nameBoard, C.name as nameCard, C.idCard, \
					...
					WHERE CB.user=? AND C.bArchived=0 AND C.bDeleted=0 AND (" +
    sqlNegativeDiff +
    " CB.diff>0.005 OR CB.spent<-0.005 OR CB.est<-0.005) \
...
function addDashboardListItem(list, name, url, tooltip) {
  ...
  if (tooltip !== undefined) option.attr("title", tooltip);
  return option;
}
...
function handleLoadRecent(combo, data, user) {
  ...
  for (; i < data.length; i++) {
    ...
    if (cDays == 0) tooltip += "today ";
    else if (cDays == 1) tooltip += cDays + " day ago ";
    else tooltip += cDays + " days ago ";

    tooltip = tooltip + row.dateLocal + "\n" + prefixSE + "S: " + row.spent + "   E: " + row.est;
    if (comment) tooltip = tooltip + "\n" + (row.cGrouped > 1 ? "Comments:\n" : "Comment: ") + comment;
  ...
  combo.off("keydown").on("keydown", function (ev) {
    ...
    if (ev && ev.ctrlKey) msDateLastCtrlClick = performance.now();
  });
  ...
  combo.off("change").on("change", function (ev) {
    ...
    if (id) {
      if (id == VAL_COMBO_OPENREPORT) {
        ...
        window.open(chrome.runtime.getURL("report.html") + params, "_blank");
      } else {
...
function handleLoadPending(combo, data, user) {
  ...
  for (; i < data.length; i++) {
    ...
    if (cDays == 1) tooltip += " day ago.";
    else tooltip += " days ago.";
    ...
    var span = addDashboardListItem(
      combo,
      parseFixedFloat(row.diff) +
        " \u00A0\u00A0\u00A0" +
        strTruncate(row.nameBoard) +
        " - " +
        strTruncate(row.nameCard),
      row.idCard,
      tooltip
    );
    if (bError) span.addClass("agile_card_error");
  }
  ...
  combo.off("change").on("change", function (ev) {
    ...
    if (id) {
      if (id == VAL_COMBO_OPENREPORT) {
        ...
        if (g_bProVersion) params += "&customColumns=r%2Ccard%2Cboard%2Clabels%2CdateDue%2CdateString";
...
function getHtmlDrillDownTooltip(rows, bReverse, colExclude) {
  var headerBase = [
    ...
    { name: "S" },
    { name: "E" },
    { name: "Note", bExtend: true },
    { name: COLUMNNAME_ETYPE },
  ];

  var header = [];
  var iHeader = 0;
  for (; iHeader < headerBase.length; iHeader++) {
    if (headerBase[iHeader].name != colExclude) header.push(headerBase[iHeader]);
  }
  ...
  function callbackRowData(row) {
    ...
    if (colExclude != "User") rgRet.push({ name: row.user, bNoTruncate: false });
    if (colExclude != "Board") {
      if (row.idBoard) {
    ...
    else urlCard = "https://trello.com/c/" + row.idCard;
...
function loadChartSpentWeekBoard(waiter, divContainer, idElem, idElemStatusChart, response) {
  ...
  for (; iUser < response.users.length; iUser++) data.addColumn("number", response.users[iUser]);
...
function finishSpentChartConfig(
  waiter,
  idElem,
  elem,
  data,
  ...
  if (posLegend == "top" || posLegend == "bottom") height += g_marginLabelChart;
  ...
  if (drilldowns) {
    chartParams.chart.setAction({
      ...
      action: function () {
        // When clicked, the following runs.
  ...
  if (!g_bPreventChartDraw) drawSpentWeekChart(chartParams);
  if (waiter) waiter.Decrease();
  var pair = {};
...
function updateUsersList(users) {
  users.sort(function (aParam, bParam) {
    var a = aParam[0];
    var b = bParam[0];
    if (a > b) return 1;
    if (a < b) return -1;
...
function drawSpentWeekChart(chartParams) {
  if (!g_bShowHomePlusSections) return;
  var bNewTrelloHome = bAtNewTrelloHome();
  ...
  if (posLegend == "top") top = g_marginLabelChart;
  else if (posLegend == "bottom") bottom = g_marginLabelChart;
  ...
  var style = {
    chartArea: {
      ...
      bottom: bottom,
      right: 0,
      height: data.getNumberOfRows() * g_heightBarUser,
      width: "100%",
    },
...
function remapTextElements(value, postfix, svg, mapRows, mapDone) {
  ...
  var elem = svg
    ...
    .filter(function () {
      ...
      if (valElem == value) return true;
      if (postfix.length == 0 || this.innerHTML == valElem) return false;
...
function setSmallFont(elem, percent) {
  ...
  if (percent === undefined) percent = 0.7;
  if (g_current_fontSize == null) g_current_fontSize = parseInt($("body").css("font-size"), 10);
...
function doShowAgedCards(bShow) {
  ...
  if (bTrelloFilter)
    //REVIEW zig: not enough. ideally all hidden cards (by "less") shoul be shown again as soon at the trello filter cards pane comes up
    return;

  if (bShow) elems.removeClass("hide");
  else elems.addClass("hide");
}

var g_bShowAllItems = true; //show all items, or recent only (cards and boards)

function checkCreateRecentFilter(header) {
  if (g_bHideLessMore) return;

  var elemFilter = header.find($("#toggleAll"));
  if (elemFilter.length > 0) return;
  var elem = $('<a id="toggleAll" href="">...</a>')
    .css("margin-left", "5px")
    .addClass("agile_plus_header_link")
    .appendTo(header);
...
function showHiddenCardsAlert() {
  if (g_cShownHiddenCardsAlert > 0) return;
...
function updateShowAllButtonState(elem, bFirstTime) {
  ...
  chrome.storage.sync.get("bShowAllItems", function (obj) {
    ...
    if (bShow) {
      ...
      if (bFirstTime) hiliteOnce(elem, 0, "agile_box_more_hilite");
...
function testExtensionAndcommitPendingPlusMessages() {
  if (!g_bErrorExtension) testExtension(); //this attempts commit of pending queue
}
...
function checkTryPro() {
  const PROP_LS_MSLASTTRYPRO = "plus_ms_last_try_pro";
  const PROP_LS_DATELASTTRYPROCHECK = "plus_date_last_try_pro_check";
  const PROP_LS_CTIMESSHOWNTRYPRO = "plus_times_shown_try_pro";
  const MSDELTA_TRYPRO = 1000 * 60 * 60 * 24 * 40; //40 days
  const CTIMES_FINAL = 3;
  const msNow = Date.now();

  if (!g_bForceShowTry) {
    if (g_msStartPlusUsage === null || g_bShowedTryPro) return;

    var cDaysUsingPlus = Math.floor((msNow - g_msStartPlusUsage) / 1000 / 60 / 60 / 24);
    if (cDaysUsingPlus < 15) return;

    var msLast = parseInt(localStorage.getItem(PROP_LS_MSLASTTRYPRO) || "0", 10) || 0;

    if (msNow - msLast < MSDELTA_TRYPRO) return;
  }
  var strDateLast = localStorage.getItem(PROP_LS_DATELASTTRYPROCHECK) || "";
  var strDateNow = makeDateCustomString(new Date(msNow));
  var cTimesToday = 0;
  if (strDateNow == strDateLast)
    cTimesToday = parseInt(localStorage.getItem(PROP_LS_CTIMESSHOWNTRYPRO) || "0", 10) || 0;
  else localStorage.setItem(PROP_LS_DATELASTTRYPROCHECK, strDateNow);

  cTimesToday++;

  if (!g_bForceShowTry) {
    if (cTimesToday > CTIMES_FINAL) return;

    if (cDaysUsingPlus < 20 && cTimesToday > 1) return;
  }

  localStorage.setItem(PROP_LS_CTIMESSHOWNTRYPRO, cTimesToday.toString());
  ...
  showTryProDialog(cTimesToday == CTIMES_FINAL, function (bOK) {
    ...
    localStorage.setItem(PROP_LS_MSLASTTRYPRO, msNow.toString());
  });
}

function insertStripeDialog(liDataStripe) {
  var userTrello = getCurrentTrelloUser();
  if (!userTrello) return null;
...
<form method="post" id="agile-stripe-payment-form">\
    ...
    <label style="display:inline-block;">License owner:</label>&nbsp;<span id="agile_userTrello_stripe">' +
      userTrello +
      '</span>\
    <br><br>\
    <label>\
...
<p>&nbsp;</p>\
<div><img src="' +
      chrome.runtime.getURL("images/dotswait.gif") +
      '" /></div></div>\
</dialog>'
  );
  ...
  $("#agile_cancel_stripe_buy,#agile_stripe_ok").click(function (ev) {
    ...
    ev.preventDefault();
    if (ev.target.id == "agile_cancel_stripe_buy") hitAnalytics("LicActivation", "CANCELED-STRIPE");
    return false;
  });
  ...
  if (liDataStripe) {
    ...
    if (liDataStripe.emailOwner) elemEmail.val(liDataStripe.emailOwner);
    quantityOrig = 0;
    ...
    if (liDataStripe.nameCardOwner) divStripeForm.find("#agile_cardholdername_stripe").val(liDataStripe.nameCardOwner);
  }
...
function handleStripePay(callbackIn) {
  ...
  chrome.storage.sync.get([SYNCPROP_LIDATA_STRIPE], function (objLiStripe) {
    ...
    function insertWrap() {
      ...
      script.src = chrome.runtime.getURL("stripe-wrap.js");
    ...
    function addCustomEventListener() {
      ...
      function callbackEvent(event) {
        if (event && event.isTrusted && event.data && event.data.type && event.data.type == "agile_stripe_data") {
          ...
          if (license && license.quantity > 0) hitAnalytics("LicActivation", "OK-ACTIVATED-STRIPE");
          var objNew = {};
...
function checkLi(bForce, bExpanded, callbackIn) {
  ...
  chrome.storage.local.get([LOCALPROP_PRO_VERSION], function (obj) {
    ...
    chrome.storage.sync.get([SYNCPROP_LIDATA, SYNCPROP_LIDATA_STRIPE], function (obj) {
      ...
      var msNow = Date.now();
      if (!bForce && (liData.li || (liDataStripe.li && liDataStripe.quantity > 0))) return; //review zig expiration etc

      var msLast = Math.max(liData.msLastCheck || 0, liDataStripe.msLastCheck || 0);
      var bWaitCheck = !bForce;
      if (bWaitCheck && msNow - msLast < 1000 * 60 * 60 * 4)
        //4 hours
        return;
      ...
      function updateLiIfNewer(liData, bStripeMode) {
        ...
        chrome.storage.sync.get([propLi], function (obj) {
          ...
          if (liDataFromSync && liData.li && liData.li != liDataFromSync.li) {
            ...
            logPlusError(
              "unexpected: received license in saveLiIfNewer. license:" +
                liData.li +
                ". previous retained:" +
                liDataFromSync.li
            );
        ...
        showFirstLicDialog(bExpanded, liData.li == "", function (status, bStripeMode) {
          ...
          if (status == STATUS_OK) {
            ...
            sendExtensionMessage({ method: "checkLi" }, function (response) {
              ...
              if (response.status == STATUS_OK) sendDesktopNotification("Successful activation. Enjoy!");
              else if (response.status == "hasLicense")
                sendDesktopNotification(
                  "Existing license found (was already activated). Enjoy!",
                  12000
                ); //user not using Chrome sign-in (storage sync) or crashed just after payment and before saving to storage.
              else if (response.status == "TOKEN_MISSING_ERROR")
                sendDesktopNotification(
                  "Please sign-in to Chrome before buying. In the top right of Chrome, click the button with your name or 'People'.",
                  12000
                );
              else {
                ...
                else sendDesktopNotification("An error happened. Please try later. " + response.status, 12000);
...
function handlePlusLicenseUrl(userOwner, idSub) {
  ...
  if (divPlusContent.length > 0) return;
  var userTrello = getCurrentTrelloUser();

  if (!userTrello) return;
  ...
  chrome.storage.sync.get([SYNCPROP_LIDATA_STRIPE], function (obj) {
    ...
    var elemBottom = $(
      '<p style="margin-top:4em;font-size:100%;color:#909090;"><A href="http://www.plusfortrello.com" target="_blank" style="color:#909090;">Plus for Trello</A> is not associated with Trello or Atlassian.</p>'
    );
      ...
      xhr.onreadystatechange = function (event) {
        if (xhr.readyState == 4) {
          ...
          if (!val || !iColon || val.indexOf(strOk) != 0) {
            if (iColon > 0) val = val.substr(iColon + 1, val.length) + " (" + val.substr(0, iColon) + ")";
            elemText.text("Error");
            elemP.text(val);
            elemP2
              .html(
                "<p>Visit our <a href='http://www.plusfortrello.com/p/support.html' target='_blank'>Plus support page</a></p>"
              )
              .show();
          ...
          chrome.storage.sync.set(objNew, function () {
            ...
            if (bSyncNotEnabled) {
              elemP2
                .html("<b><span style='color:red;'>You have not yet enabled sync.</span> <a href=''>Do it now</a><b/>")
                .show();
            ...
            elemP.html(
              "<b>Thank you!</b><br><br><br>Love Plus? <a href='https://chrome.google.com/webstore/detail/plus-for-trello-time-trac/gjjpophepkbhejnglcmkdnncmaanojkf/reviews' target='_blank'>Rate us!</a><br><br>\
...
function setProVersionOption(bValue, callback) {
  ...
  chrome.storage.local.set(pair, function () {
    if (chrome.runtime.lastError == undefined) {
      ...
      if (bSetDate) g_msStartPro = msNow;
    }
    if (callback) callback();
  });
}

function showEnableSyncDialog(callback) {
  //callback: STATUS_OK, dontask, cancel (later)
...
<dialog id="agile_dialog_EnableSync" style="cursor:pointer;text-align: center;width:24em;padding-top:0.5em;" class="agile_dialog_DefaultStyle agile_dialog_Postit agile_dialog_Postit_Anim_EnableSync">\
  ...
  function initDialog() {
    ...
    divDialog
      .find("#agile_dialog_EnableSync_OK")
      .off("click.plusForTrello")
      .on("click.plusForTrello", function (e) {
        e.preventDefault();
    ...
    divDialog
      .find("#agile_dialog_EnableSync_Cancel")
      .off("click.plusForTrello")
      .on("click.plusForTrello", function (e) {
    ...
    divDialog
      .find("#agile_dialog_EnableSync_DontAsk")
      .off("click.plusForTrello")
      .on("click.plusForTrello", function (e) {
        e.preventDefault(); //link click would navigate otherwise
        doCloseDialog(function () {
  ...
  setTimeout(function () {
    divDialog.addClass("agile_dialog_Postit_Anim_ShiftToShow");
  }, 200); //some dialog conflict prevents animation from working without timeout
}
