...
<td align="left" class="agile-etransfer-tcel-header" style="white-space: nowrap;">E ' +
        UNITS.getLongFormat(UNITS.current, g_bDisplayPointUnits) +
        '</td> \
</tr> \
<tr> \
...
<label class="agile_unselectable" style="vertical-align: middle;font-weight:normal;line-height:1em;"><input type="checkbox" class="agile_transfere_alsospend" style="vertical-align: middle;margin-bottom:4px;margin-top:4px;" /> Immediately spend the transferred ' +
        UNITS.getLongFormat(UNITS.current, g_bDisplayPointUnits) +
        '.</label>\
...
<td class="agile-etransfer-cell agile-etransfer-tcel-to-r"></td>\
  ...
  setTimeout(() => {
    comboUserTo.focus();
  }); // for convenience, and so ESC wont kill the card behind
  var celSel = {
    from: {
      user: ".agile-etransfer-tcel-from-user",
      s: ".agile-etransfer-tcel-from-s",
      e: ".agile-etransfer-tcel-from-e",
      r: ".agile-etransfer-tcel-from-r",
    },
    to: {
      user: ".agile-etransfer-tcel-to-user",
      s: ".agile-etransfer-tcel-to-s",
      e: ".agile-etransfer-tcel-to-e",
      r: ".agile-etransfer-tcel-to-r",
    },
  };
  ...
  for (user in seData) {
    ...
    if (data && data.e && data.e > 0) {
      ...
      if (r <= 0) continue;
      if (user == userCurrent) user = g_strUserMeOption;

      //userLast, if exists, has precedence, then the global user
      if (!userSelected || userSelected != userLast) {
        if (user == userLast || user == g_globalUser || userSelected != g_globalUser) userSelected = user;
      }
  ...
  if (bHideComboKeyword) {
    ...
    var href = chrome.runtime.getURL(
      "report.html?chartView=s&idCard=" +
        encodeURIComponent(idCard) +
        '&groupBy=user-keyword&orderBy=keyword&archived=-1&deleted=-1&sortList=%5B%5B"User"%2C0%5D%2C%5B"Keyword"%2C0%5D%5D'
    );
    if (g_bProVersion) href += "&customColumns=user%2Ckeyword%2Cs%2Ce1st%2Ce%2Cr";
  ...
  function setMessageText(text) {
    if (!text) elemMessage.html("&nbsp;");
    else elemMessage.text(text);
  }
  ...
  function getData(bSilent, callback) {
    ...
    function fillTableRow(balData, selData, prefixFrom) {
      ...
      if (user) {
        ...
        if (balData.s < 0) elemS.addClass("agile_plus_header_error");
        elemE.text(parseFixedFloat(balData.e) + " (" + parseFixedFloat(balData.e1) + ")");
        if (balData.e < 0) elemE.addClass("agile_plus_header_error");
        var rCalc = balData.e - balData.s;
        elemR.text(parseFixedFloat(rCalc));
        if (rCalc < 0) elemR.addClass("agile_plus_header_error");
        else if (rCalc > 0) elemR.addClass("agile_remaining_background");
      } else {
        elemS.empty();
        elemE.empty();
        elemR.empty();
    ...
    getSQLReport(sqlStmt, sqlParams, function (response) {
      ...
      for (var i = 0; i < response.rows.length; i++) {
        ...
        elem.s += row.spent;
        elem.e += row.est;
        if (row.eType == ETYPE_NEW) elem.e1 += row.est;
      }
      ...
      bal.to.e += eNewNum;
      bal.to.e1 += eNewNum;
      if (bAlsoSpend) bal.to.s += eNewNum;
  ...
  function doEnter() {
    getData(false, function (status, data) {
      //data: // { keyword: kw, userFrom: userFrom, userTo: userTo, e: eNewNum, note: note, bAlsoSpend: bAlsoSpend }
      if (status != STATUS_OK || !data) return;
      assert(data.e > 0);
      ...
      function onFinishedFinal(bOk, bCloseEvenOnFailure) {
        enableUI(true);
        if (bOk || bCloseEvenOnFailure) divDialog[0].close();
      }
