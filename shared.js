...
globalThis.IDTEAM_UNKNOWN = "//"; //reserved idTeam. note that idTeam can also be null for boards without team
globalThis.IDBOARD_UNKNOWN = "//"; //board shortLink/idLong reserved for unknown boards. saved into db. for cases where user does not have permission for a board (for example when moving a card there)
globalThis.IDLIST_UNKNOWN = "//"; //list idLong. deals with missing idList in convertToCardFromCheckItem board action. saved to db
...
globalThis.IDLABEL_DUMMY = "//"; //the one dummy label cards share.
globalThis.g_bDummyLabel = false; //not used. just kept to keep the code as it inserts itself in interesting spots
globalThis.ROWID_REPORT_CARD = -1; // "fake" rowid on reports that join card with history. use -1 as rowid so when doing a "new s/e rows" report and a group is used, this union wont appear.
globalThis.PREFIX_COMMAND_SE_RESETCOMMAND = "[^resetsync";
globalThis.g_msFetchTimeout = 15000; //ms to wait on urlFetches. update copy on plus.js
globalThis.g_cchTruncateDefault = 50;
globalThis.g_cchTruncateShort = 20;
globalThis.g_cchTruncateChartDlabel = 35;
globalThis.g_regexpHashtags = /#([\S-]+)/g;
globalThis.g_colorTrelloBlack = "#4D4D4D";
globalThis.IDNOTIFICATION_FIRSTSYNCPRORESS = "firstSyncProgress";
globalThis.OPT_SHOWSPENTINICON_NORMAL = 0;
globalThis.OPT_SHOWSPENTINICON_ALWAYS = 1;
globalThis.OPT_SHOWSPENTINICON_NEVER = 2;
globalThis.g_optAlwaysShowSpentChromeIcon = OPT_SHOWSPENTINICON_NORMAL; //review zig these 3 need initialization.
globalThis.g_bDontShowTimerPopups = false;
globalThis.g_bIncreaseLogging = false;
globalThis.g_lsKeyDisablePlus = "plus_agile_disable_page_changes"; //in page localStorage (of trello.com content script) so it survives plus reset sync
globalThis.g_language = "en";
globalThis.g_bNoSE = false; //true -> hide S/E features
globalThis.g_bNoEst = false; //true -> hide E features
globalThis.g_bProVersion = false;
globalThis.g_msStartPro = 0; //together with g_bProVersion
globalThis.g_bFromBackground = false;
globalThis.g_msStartPlusUsage = null; //ms of date when plus started being used.
globalThis.g_bReportsUseSystemNumberFormat = true; //review: should parametrize but wasnt easy. Callers must set this before calling report format functions.

globalThis.URLPART_PLUSLICENSE = "plus-license";
globalThis.LOCALPROP_NEEDSHOWPRO = "keyNeedShowProInfo";
globalThis.LOCALPROP_DONTSHOWSYNCWARN = "bDontShowAgainSyncWarn";
globalThis.LOCALPROP_NEEDSHOWHELPPANE = "keyNeedShowHelpPane";

globalThis.ID_PLUSBOARDCOMMAND = "/PLUSCOMMAND"; //review zig: remnant from undocumented boardmarkers feature. newer commands do not use this.

//thanks http://stackoverflow.com/a/12034334
globalThis.g_entityMap = {
  "&": "&amp;",
  "<": "&lt;",
  ">": "&gt;",
  '"': "&quot;",
...
globalThis.numToPlusLocaleString = function numToPlusLocaleString(val) {
  if (g_bReportsUseSystemNumberFormat && typeof val == "number") val = val.toLocaleString();
  return val;
};

//replace can be a string, or a map function
//for performace, returns same string when there is no match
globalThis.replaceString = function replaceString(string, regex, replace) {
...
globalThis.cmpString = function cmpString(a, b) {
  return a > b ? 1 : a < b ? -1 : 0;
};

globalThis.commonBuildUrlFromParams = function commonBuildUrlFromParams(params, doc) {
  var url = chrome.runtime.getURL(doc);
  var val = null;
...
globalThis.g_regexEscapeHtml = /[&<>"'\/]/g;

globalThis.escapeHtml = function escapeHtml(string) {
  if (!string) return "";
...
globalThis.getDialogParent = function getDialogParent(bForceBody) {
  if (!bForceBody) {
    var par = $('[data-testid="card-back-name"]');
    if (par.length > 0) return par;
  }
  return $("body");
};

globalThis.bHandledDeletedOrNoAccess = function bHandledDeletedOrNoAccess(status, objRet, statusRetCustom) {
  var bDeleted = bStatusXhrDeleted(status);
  if (bStatusXhrNoAccess(status) || bDeleted) {
    if (typeof statusRetCustom == "undefined") statusRetCustom = STATUS_OK;
    objRet.hasPermission = false;
    objRet.status = statusRetCustom;
    if (bDeleted) objRet.bDeleted = true;
    return true;
  }
  return false;
};

globalThis.bStatusXhrNoAccess = function bStatusXhrNoAccess(status) {
  if (status == 400 || status == 401 || status == 403) return true;
  return false;
};

globalThis.bStatusXhrDeleted = function bStatusXhrDeleted(status) {
  return status == 404;
};

globalThis.g_regexParserFormat = /<.*?>/;

globalThis.addTableSorterParsers = function addTableSorterParsers() {
  ...
  $.tablesorter.addParser({
    ...
    format: function (s) {
      // format your data for normalization
      if (s.length > 2)
        //hackish: empty dates show as a spasish char. this detects it.
        return s;
...
globalThis.isTestVersion = function isTestVersion() {
  //return false;
  return chrome.runtime.id != "gjjpophepkbhejnglcmkdnncmaanojkf";
};

globalThis.getHashtagsFromTitle = function getHashtagsFromTitle(title, bFirstOnly) {
  var hashtags = [];
...
globalThis.setOptAlwaysShowSpentChromeIcon = function setOptAlwaysShowSpentChromeIcon(opt) {
  //note old versions used to store boolean
  if (typeof opt === "undefined") opt = OPT_SHOWSPENTINICON_NORMAL;
  else if (opt === true) opt = OPT_SHOWSPENTINICON_ALWAYS;
  else if (opt === false) opt = OPT_SHOWSPENTINICON_NORMAL;
  g_optAlwaysShowSpentChromeIcon = opt;
};

globalThis.STATUS_OK = "OK"; //for messaging status with background page. all responses should have response.status=STATUS_OK when ok
globalThis.STATUS_CANCEL = "cancelled";

globalThis.COLOR_ERROR = "#D16C6C";
globalThis.MS_TRELLOAPI_WAIT = 1000 / 2; //its really 10 per second but we dont abuse https://help.trello.com/article/838-api-rate-limits
globalThis.CMAX_THREADS = 4;
globalThis.g_callbackOnAssert = null;
globalThis.g_bDebuggingInfo = false;

globalThis.g_bAcceptSFT = true;
globalThis.g_bAcceptPFTLegacy = true;

globalThis.g_regexExcludeList = /\[\s*exclude\s*\]/;
globalThis.g_userTrelloBackground = null;

globalThis.g_regexWords = /\S+/g; //parse words from an s/e comment. kept global in hopes of increasing perf by not having to parse the regex every time

globalThis.PROP_TRELLOUSER = "plustrellouser";
globalThis.PROP_SHOWBOARDMARKERS = "showboardmarkers";
globalThis.COLUMNNAME_ETYPE = "E. type";
globalThis.g_bPopupMode = false; //running as popup? (chrome browse action popup) REVIEW zig: cleanup, only reports need this?

globalThis.SYNCPROP_CARDPOPUPTYPE = "cardPopupType"; //one of CARDPOPUPTYPE
globalThis.SYNCPROP_ACTIVETIMER = "cardTimerActive";
globalThis.SYNCPROP_optAlwaysShowSpentChromeIcon = "bAlwaysShowSpentChromeIcon"; //"b" because it used to be a boolean
globalThis.SYNCPROP_bShowedFeatureSEButton = "bShowedFeatureSEButton";
globalThis.SYNCPROP_bStealthSEMode = "bStealthSEMode";
globalThis.SYNCPROP_language = "language";
globalThis.SYNCPROP_BOARD_DIMENSION = "board_dimension";
globalThis.SYNCPROP_GLOBALUSER = "global_user";
globalThis.SYNCPROP_KEYWORDS_HOME = "keywords_home";
globalThis.LOCALPROP_PRO_VERSION = "pro_enabled";
globalThis.LOCALPROP_PRO_MSDATEENABLED = "pro_datestart"; //only used for prompting, not the actual date the user enabled it, only the local date as of v5.1.2
globalThis.SYNCPROP_MSLICHECK = "msLiCheck";
globalThis.SYNCPROP_LIDATA = "LiData"; //for chrome store {  msLastCheck, msCreated, li}
globalThis.SYNCPROP_LIDATA_STRIPE = "striLiData"; //for stripe { msLastCheck, msCreated, li, userTrello, emailOwner, quantity, nameCardOwner}
globalThis.SYNCPROP_SERVIEWS = "SERViews"; // see g_serViews
globalThis.SYNCPROP_MSSTARTPLUSUSAGE = "msStartPlusUsage";
globalThis.SYNCPROP_USERSEBAR_LAST = "userSEBarLast";
globalThis.SYNCPROP_NO_SE = "dontUseSE";
globalThis.SYNCPROP_NO_EST = "dontUseEst";
globalThis.LOCALPROP_EXTENSION_VERSIONSTORE = "chromeStoreExtensionVersion";
globalThis.LOCALPROP_bHiliteTimerOnOpenCard = "bHiliteTimerOnOpenCard";
globalThis.g_bStealthSEMode = false; //stealth mode. Only applies when using google spreadsheet sync. use IsStealthMode()
globalThis.g_strServiceUrl = null; //null while not loaded. set to empty string or url NOTE initialized separately in content vs background
globalThis.SEKEYWORD_DEFAULT = "plus!";
globalThis.SEKEYWORD_LEGACY = "plus s/e";
globalThis.g_bEnableTrelloSync = false; //review zig this and g_bDisableSync must be initialized by caller (like loadSharedOptions)
globalThis.g_bDisableSync = false; // 'bDisabledSync' sync prop. note this takes precedence over bEnableTrelloSync or g_strServiceUrl 'serviceUrl'
globalThis.g_bCheckedTrelloSyncEnable = false; //review zig must be initialized by caller
globalThis.g_hackPaddingTableSorter = "&nbsp;&nbsp;"; //because we remove many tablesorter css styles, appending spaces to header text was the easiest way to avoid overlap with sort arrow
globalThis.g_dateMinCommentSELegacy = new Date(2014, 11, 12);

globalThis.IsStealthMode = function IsStealthMode() {
  return !g_bDisableSync && g_bStealthSEMode && !g_optEnterSEByComment.IsEnabled() && g_strServiceUrl;
};

globalThis.MsWaitRetry = function MsWaitRetry() {
  return Math.floor(12000 + Math.random() * 8000);
};

globalThis.g_msWaitMax = 200000;

globalThis.g_optEnterSEByComment = {
  bInitialized: false,
  IsEnabled: function () {
    //note this doesnt take into account g_bDisableSync
  ...
  getAllKeywordsExceptLegacy: function () {
    ...
    for (var i = 0; i < iMax; i++) {
      ...
      if (kw.toLowerCase() == SEKEYWORD_LEGACY) continue;
      ret.push(kw);
    }
    if (ret.length == 0) ret.push(SEKEYWORD_DEFAULT);
    return ret;
  },
  getDefaultKeyword: function () {
    assert(this.bInitialized);
    var ret = this.rgKeywords[0];
    if (!ret) ret = SEKEYWORD_DEFAULT;
  ...
  loadFromStrings: function (bEnabled, strKW) {
    ...
    if (this.rgKeywords.constructor !== Array || this.rgKeywords.length == 0) this.rgKeywords = [SEKEYWORD_DEFAULT]; //force always the default. array cant be empty.
...
globalThis.g_rgiDayName = [null, null, null, null, null, null, null];
globalThis.g_rgiDayNameLong = [null, null, null, null, null, null, null];
globalThis.getWeekdayName = function getWeekdayName(num, bLong) {
  var dateKnown;
  if (bLong) {
 ...
 *
 **/
globalThis.DowMapper = {
  //public:
  DOWSTART_DEFAULT: 0, //sunday default
  getDowStart: function () {
    return this.m_dowStart;
  }, //set dow with position 0
  getDowDelta: function () {
    return this.m_dowDelta;
  },
  ...
  posWeekFromDow: function (dow) {
    //position for a given dow
    var pos = dow - this.m_dowStart - this.m_dowDelta;
    if (pos < 0) pos = 14 + pos;
    pos = pos % 7;
    return pos;
  },
  dowFromPosWeek: function (pos) {
    //dow in given position
    var dowNew = pos + this.m_dowStart + this.m_dowDelta;
    if (dowNew < 0) dowNew = 14 + dowNew;
    dowNew = dowNew % 7;
    return dowNew;
  },
...
globalThis.loadSharedOptions = function loadSharedOptions(callback) {
  ...
  chrome.storage.sync.get(
    [
      SYNCPROP_NO_EST,
      SYNCPROP_NO_SE,
      SYNCPROP_MSSTARTPLUSUSAGE,
      keyServiceUrl,
      SYNCPROP_bStealthSEMode,
      SYNCPROP_language,
      keybDontShowTimerPopups,
      keyUnits,
      SYNCPROP_optAlwaysShowSpentChromeIcon,
      keyAcceptSFT,
      keybEnableTrelloSync,
    ...
    function (objSync) {
      ...
      if (g_strServiceUrl === undefined || g_strServiceUrl == null) g_strServiceUrl = ""; //means simple trello. (do the same as in content script)
      ...
      if (g_bAcceptSFT === undefined) g_bAcceptSFT = true;

      g_bAcceptPFTLegacy = objSync[keyAcceptPFTLegacy];
      if (g_bAcceptPFTLegacy === undefined) g_bAcceptPFTLegacy = true; //defaults to true to not break legacy users
...
globalThis.errFromXhr = function errFromXhr(xhr) {
  var errText = "error: " + xhr.status;
  if (xhr.statusText || xhr.responseText) {
    g_bIncreaseLogging = true;
    errText = errText + "\n" + xhr.statusText + "\n" + xhr.responseText;
  } else if (xhr.status == 0) errText = Language.NOINTERNETCONNECTION;
  console.log(errText);
  return errText;
};

globalThis.assert = function assert(condition, message) {
  if (!condition) {
    g_bIncreaseLogging = true;
    ...
    logPlusError(log);
    debugger;
    if (g_callbackOnAssert) g_callbackOnAssert(log);
    throw new Error(log);
  }
};

globalThis.saveAsFile = function saveAsFile(data, filename, bForceSave) {
  if (!bForceSave && !g_bDebuggingInfo) return;
...
globalThis.ETYPE_NONE = 0;
globalThis.ETYPE_INCR = 1;
globalThis.ETYPE_DECR = -1;
globalThis.ETYPE_NEW = 2;

globalThis.nameFromEType = function nameFromEType(eType) {
  if (eType == ETYPE_NONE) return "";

  if (eType == ETYPE_INCR) return "+E";

  if (eType == ETYPE_DECR) return "-E";

  if (eType == ETYPE_NEW) return "NEW";
};

globalThis.buildSyncErrorTooltip = function buildSyncErrorTooltip(status) {
  var ret = "";
  if (status) {
...
globalThis.openPlusDb = function openPlusDb(sendResponse, options) {
  sendExtensionMessage({ method: "openDB", options: options }, function (response) {
...
globalThis.CreateWaiter = function CreateWaiter(cTasks, callback) {
  var waiter = {
    ...
    Increase: function (prop) {
      //prop: optional to prevent multiple calls with the same prop from increasing
      if (prop) {
        if (this.mapProps[prop]) return;
    ...
    Decrease: function (prop) {
      //prop: optional to prevent multiple calls with the same prop from decreasing
      if (prop) {
        if (this.mapProps[prop]) return;
        this.mapProps[prop] = true;
      }
      this.cTasksPending--;
      if (this.bWaiting && this.cTasksPending == 0) this.OnFinish();
    },
...
globalThis.getUrlParams = function getUrlParams(href) {
  if (!href) href = window.location.href;
  var iParams = href.indexOf("?");
  var objRet = {};
  if (iParams < 0) return objRet;
 ...
 **/
globalThis.strTruncate = function strTruncate(str, length) {
  if (length === undefined) length = g_cchTruncateDefault;
  if (typeof str != "string") str = "" + str;
  if (str.length > length) str = str.substr(0, length) + "...";
  return str;
};

globalThis.sendExtensionMessage = function sendExtensionMessage(obj, responseParam, bRethrow) {
  function preResponse(response) {
    try {
      if (response && response.bExtensionNotLoadedOK) return; //safer to not respond

      if (chrome.runtime.lastError)
        //could happen on error connecting to the extension. that case response can even be undefined https://developer.chrome.com/extensions/runtime#method-sendMessage
        throw new Error(chrome.runtime.lastError.message);

      if (responseParam) responseParam(response);
    } catch (e) {
      logException(e);
    }
  }
...
globalThis.logException = function logException(e, str) {
  if (str && str != e.message) str = str + "," + e.message;
  else str = e.message;
  logPlusError(str + " :: " + e.stack, false);
};

globalThis.sendDesktopNotification = function sendDesktopNotification(strNotif, timeout, idUse) {
  if (timeout === undefined) timeout = 7000;

  sendExtensionMessage(
    { method: "showDesktopNotification", notification: strNotif, timeout: timeout, idUse: idUse },
    function (response) {}
  );
};

globalThis.g_plusLogMessages = []; //queues an error log which is regularly purged
globalThis.g_lastLogPush = null;
globalThis.g_lastLogError = "";

globalThis.bIgnoreError = function bIgnoreError(str) {
...
//
globalThis.logPlusError = function logPlusError(str, bAddStackTrace) {
  str = str || ""; //handle possible undefined
  if (bIgnoreError(str)) return;

  if (str.indexOf("disconnected port") >= 0 || str.indexOf("port closed") >= 0) return; //sometimes we dont return from a received message. that seems ok

  g_bIncreaseLogging = true;
  var strStack = null;
  var date = new Date();
  if (bAddStackTrace === undefined) bAddStackTrace = true;
  if (bAddStackTrace) {
    try {
      throw new Error();
    } catch (e) {
  ...
  if (typeof g_userTrelloCurrent != "undefined") {
    if (g_userTrelloCurrent && (g_userTrelloCurrent == "zmandel" || g_userTrelloCurrent == "zigmandel")) {
      if (typeof document != "undefined" && typeof PLUS_BACKGROUND_CALLER == "undefined") console.dir(document.body);
      //hi to me
      if (str) alert(str);
    }
  }
  ...
  if (
    !(
      g_lastLogPush != null &&
      pushData.date - g_lastLogPush.date < 1000 * 60 &&
      pushData.message == g_lastLogPush.message
    )
  ) {
    g_lastLogPush = pushData;
    g_plusLogMessages.push(pushData);
    setTimeout(function () {
      //get out of the current callstack which could be inside a db transaction etc
      if (g_callbackPostLogMessage) g_callbackPostLogMessage();
    }, 2000);
  }
};
 ...
 **/
globalThis.g_intervalCallbackPostLogMessage = null;
globalThis.g_callbackPostLogMessage = null;

globalThis.setCallbackPostLogMessage = function setCallbackPostLogMessage(callback) {
  g_callbackPostLogMessage = callback;
  if (g_intervalCallbackPostLogMessage) clearInterval(g_intervalCallbackPostLogMessage);
 ...
 **/
globalThis.getSQLReportShared = function getSQLReportShared(sql, values, okCallback, errorCallback) {
  var promise = null;
  ...
  function sendResponse(response) {
    if (response.status != STATUS_OK) {
      ...
      if (errorCallback && !resolveFn) errorCallback(response.status);
...
globalThis.selectElementContents = function selectElementContents(el) {
...
globalThis.removeSelection = function removeSelection() {
  if (window.getSelection && document.createRange) {
    var sel = window.getSelection();
    sel.removeAllRanges();
  }
...
globalThis.prependZero = function prependZero(num) {
  assert(typeof num == "number");
  return num < 10 ? "0" + num : num;
};

globalThis.parseColonFormatSE = function parseColonFormatSE(val, bExact) {
  assert(val.indexOf(":") >= 0);
  if (val.indexOf(".") >= 0) return null; //invalid

  var rg = val.split(":");
  if (rg.length != 2) return null; //invalid
...
//returns a float
globalThis.parseFixedFloat = function parseFixedFloat(text, bDontZeroNan, bOneDecimal) {
  var val = null;

  if (typeof text == "number") val = text;
  else {
...
globalThis.addSumToRows = function addSumToRows(bModifyRows, rows, prefix) {
  prefix = prefix || "";
  var mapRet = {};
  var iRow = 0;
  for (; iRow < rows.length; iRow++) {
    ...
    var row = rows[iRow];
    var iCol = 1;
    for (; iCol < row.length; iCol++) sum += row[iCol];
    sum = parseFixedFloat(sum);
    if (bModifyRows) row[0] = prefix + sum + " " + row[0];
    else mapRet[row[0]] = sum;
  }
  return mapRet;
};

globalThis.getDrilldownTopButtons = function getDrilldownTopButtons(bNoClose, title) {
  bNoClose = bNoClose || false;

  var ret =
    '<div style="padding-bottom:3px;"><p class="agile_drilldown_h"><span>' +
    title +
    "</span><span class='agile_selection_totals'></span></p>";
  if (!bNoClose) ret += '<img class="agile_help_close_drilldown"></img>';
  ret += '<img class="agile_drilldown_select"></img></div>';
  return ret;
};

globalThis.getHtmlBurndownTooltipFromRows = function getHtmlBurndownTooltipFromRows(
  bShowTotals,
  rows,
  bReverse,
  header,
  ...
  if (title === undefined) title = "Plus Drill-down";
  ...
  function htmlRow(row) {
    function td(val, bNoTruncate, type) {
      ...
      if (val === "") val = "&nbsp;"; //prevent tables with all empty fields (Bad height)
      if (typeof val == "number") val = numToPlusLocaleString(val);
    ...
    var strPost = "";
    if (tds.title && tds.title != "") strPost = " title='" + tds.title + "'";
    var strClassRow = "agile-drilldown-row";

    if (row.bSelectedRow) strClassRow = strClassRow + " agile-drilldown-row-toggle";

    if (iColComment >= 0 && tds.length > iColComment) {
      var commentLow = tds[iColComment].name.toLowerCase();
      if (commentLow.indexOf(PREFIX_ERROR_SE_COMMENT) >= 0) strClassRow = strClassRow + " agile-drilldown-row-error";
  ...
  var html = "";
  if (bOnlyTable) html = '<div class="agile_tooltip_scroller" tabindex="0">';
  else html = '<div class="agile_tooltip_scroller agile_tooltip_scroller_Short" tabindex="0">';
  ...
  for (; iHeader < header.length; iHeader++) {
    ...
    if (bExtendCur) bExtended = true;
    if (!bExtended && iHeader == header.length - 1) bExtendCur = true;
    var nameCol = header[iHeader].name;
    if (nameCol.toLowerCase() == "note") iColComment = iHeader;
  ...
  if (bShowTotals) {
    ...
    if (bSEColumns && !g_bNoSE) {
      ...
      if (!g_bNoEst) {
        title +=
          (bUseEFirst
            ? "&nbsp;&nbsp;&nbsp;&nbspE 1ˢᵗ" + sep + numToPlusLocaleString(parseFixedFloat(eFirstTotal))
            : "") +
          "&nbsp;&nbsp;&nbsp;&nbspE" +
          sep +
          numToPlusLocaleString(parseFixedFloat(eTotal)) +
          "&nbsp;&nbsp;";
...
globalThis.setScrollerHeight = function setScrollerHeight(heightWindow, scroller, elemTop, dyTop, bAdjustBody) {
  if (elemTop.length == 0) return;
  ...
  if ($("#reportBottomMessage").is(":visible")) height = height - 40;
  if (height < 100)
    //minimum size
    height = 100;

  if (!bAdjustBody) scroller.css("height", height - 7); //7 is to prevent scrollbar in case some plattform calcs are off by a few pixels
};

globalThis.updateSelectedReportTotals = function updateSelectedReportTotals() {
  ...
  if (charSeparator == "0") charSeparator = null; //none

  function getReportColumnNumber(text) {
    if (charSeparator) text = text.replace(charSeparator, "");
    if (charDecimal && charDecimal != ".") text = text.replace(charDecimal, ".");
  ...
  if (selected.length > 0)
    $(".agile_selection_totals").html(
      (g_bPopupMode ? "<br />" : "") +
        "&nbsp;" +
        selected.length +
        " Selected " +
        (g_bNoSE ? "" : "&nbsp;&nbsp;S:" + numToPlusLocaleString(parseFixedFloat(sCur)) + "&nbsp;&nbsp;") +
        (g_bNoSE || g_bNoEst
          ? ""
          : "&nbsp;&nbsp;E:" +
            numToPlusLocaleString(parseFixedFloat(eCur)) +
            "&nbsp;&nbsp;&nbsp;&nbsp;R:" +
            numToPlusLocaleString(parseFixedFloat(eCur - sCur)))
    );
  else $(".agile_selection_totals").empty();
};

globalThis.makeReportContainer = function makeReportContainer(html, widthWindow, bOnlyTable, elemParent, bNoScroll) {
  bNoScroll = bNoScroll || bOnlyTable;
  var container = $(".agile_topLevelTooltipContainer");
  bOnlyTable = bOnlyTable || false;

  if (container.length == 0) container = $("<div class='agile_topLevelTooltipContainer notranslate'></div>");
  container.empty();
  ...
  if (!bOnlyTable) {
    ...
    var marginLeft = 0;
    if (widthWindow < $(window).width()) marginLeft = -Math.round(widthWindow / 2);
    else {
    ...
    function (ev) {
      ...
      if (t.is("a") && (t.prop("target") == "_blank" || ev.ctrlKey || ev.shiftKey)) {
        ...
        if (idCard) {
          sendExtensionMessage(
            {
              method: "openCardWindow",
              idCard: idCard,
              position: { x: ev.screenX, y: ev.screenY },
              bForceTab: ev.ctrlKey || ev.shiftKey,
            },
            function (response) {}
          );
          ev.preventDefault();
      ...
      if (elemThis.children("th").length == 0) {
        if (elemThis.hasClass("agile-drilldown-row-toggle")) elemThis.removeClass("agile-drilldown-row-toggle");
        else elemThis.addClass("agile-drilldown-row-toggle");
  ...
  container.hide();
  if (!elemParent) elemParent = $("body");
  elemParent.append(container);
  container.show();
  if (true) {
    if (!bOnlyTable) scroller.focus();
    ...
    if (tooltip) tooltip.keyup(checkRemoveContainer);
  }
  ...
  if (!bOnlyTable) {
    ...
    if (attrTitle) return container;
    btnClose.attr("src", chrome.runtime.getURL("images/close.png"));
  ...
  if (!bOnlyTable) scroller.scrollview();
  return container;
};

globalThis.handleDrilldownWindow = function handleDrilldownWindow(
  chart,
  drilldowns,
  htmlFromRows,
  colExclude,
...
globalThis.handleSectionSlide = function handleSectionSlide(section, content, widthOpen, elemShowHide, callbackDone) {
  var step = 250;
  var bOpened = section.hasClass("agile_arrow_opened");
  if (!bOpened && widthOpen) {
    //doing width before the toggle looks better and avoids a chrome paint bug
    section.css("width", widthOpen);
    if (elemShowHide) elemShowHide.show();
  }

  if (!bOpened) content.show();
  else content.hide();
  content.css("opacity", bOpened ? "0" : "1");
  if (bOpened) {
    section.removeClass("agile_arrow_opened");
    section.addClass("agile_arrow_closed");
    if (elemShowHide) elemShowHide.hide();
    if (widthOpen) section.css("width", "auto");
  } else {
        ...
        setTimeout(function () {
          self.i
            ...
            .dblclick(function () {
              ...
              _m.animate({ scrollLeft: 0, scrollTop: dy }, "normal", "swing");
            });
          //self.centering();
        }, 50);
      },
...
globalThis.setPopupClickHandler = function setPopupClickHandler(elem, url) {
  function create(ev) {
    var idCard = getIdCardFromUrl(url);
    if (idCard) {
      sendExtensionMessage(
        { method: "openCardWindow", idCard: idCard, bForceTab: ev && (ev.ctrlKey || ev.shiftKey) },
        function (response) {}
      );
      return;
    } else {
...
globalThis.hiliteOnce = function hiliteOnce(elem, msTime, strClass, count) {
 ...
 **/
globalThis.cloneObject = function cloneObject(obj) {
  return JSON.parse(JSON.stringify(obj));
};

globalThis.getCurrentMonthFormatted = function getCurrentMonthFormatted(date) {
  return date.getFullYear() + "-" + getWithZeroPrefix(date.getMonth() + 1);
};

globalThis.g_weekNumUse = null; //"2015-W05"; //set for testing only

globalThis.getCurrentWeekNum = function getCurrentWeekNum(date, dowStartOpt, dowDeltaOpt) {
  if (date === undefined) {
    if (g_weekNumUse != null) return g_weekNumUse; //default week num, from plus header week selector
    date = new Date();
  }

  if (dowStartOpt === undefined) dowStartOpt = DowMapper.getDowStart();
  if (dowDeltaOpt === undefined) dowDeltaOpt = DowMapper.getDowDelta();
  if (dowDeltaOpt != 0) {
  ...
  if (weeknum == 1 && month == 11) year++; //week  belongs to next year
  else if (month == 0 && weeknum >= 50) year--;
...
globalThis.getWeekNumCalc = function getWeekNumCalc(dateIn, dowOffset) {
  ...
  if (false) {
    if (dowOffset == 6) dateIn.setDate(dateIn.getDate() + 7);
  }
  ...
  var daynum =
    Math.floor(
      (dateIn.getTime() - newYear.getTime() - (dateIn.getTimezoneOffset() - newYear.getTimezoneOffset()) * 60000) /
        86400000
    ) + 1;
  var weeknum;
  ...
  if (weeknum == 0)
    //these 2 lines by Zig (case d=6 but left here just in case)
    weeknum = 53;
  return weeknum;
};

globalThis.getWithZeroPrefix = function getWithZeroPrefix(number) {
  var ret = (number < 10 ? "0" : "") + number;
  return ret;
};

//YYYY-MM-DD
globalThis.makeDateCustomString = function makeDateCustomString(date, bWithTime) {
...
globalThis.getDeltaDates = function getDeltaDates(dateA, dateB) {
  var delta = 0;
  if (dateA) {
    ...
    if (date1 < date2) delta = delta * -1;
  }
  return delta;
};

globalThis.setBusy = function setBusy(bBusy, elem) {
...
globalThis.getCardTimerSyncHash = function getCardTimerSyncHash(idCard) {
  return "timer:" + idCard;
};

globalThis.getCardTimerData = function getCardTimerData(hash, resp) {
...
globalThis.verifyActiveTimer = function verifyActiveTimer(idCard) {
  chrome.storage.sync.get([SYNCPROP_ACTIVETIMER], function (obj) {
    if (obj[SYNCPROP_ACTIVETIMER] && obj[SYNCPROP_ACTIVETIMER] == idCard) return;
    var objNew = {};
...
globalThis.updateTimerChromeIcon = function updateTimerChromeIcon(bAnimate) {
  sendExtensionMessage(
    { method: "updatePlusIcon", bOnlyTimer: true, bAnimate: bAnimate || false },
    function (response) {}
  );
};

globalThis.findNextActiveTimer = function findNextActiveTimer() {
  chrome.storage.sync.get(null, function (items) {
    ...
    if (idCardFoundLast) verifyActiveTimer(idCardFoundLast);
  });
};

globalThis.findAllActiveTimers = function findAllActiveTimers(callback) {
  chrome.storage.sync.get(null, function (items) {
    var rgIdCards = [];
    var szTimerPattern = "timer:";
    var idCardFoundLast = null;
...
globalThis.removeTimerForCard = function removeTimerForCard(idCardParsed, bWarn) {
  ...
  chrome.storage.sync.get([SYNCPROP_ACTIVETIMER, hash], function (obj) {
    ...
    if (obj[SYNCPROP_ACTIVETIMER] !== undefined && obj[SYNCPROP_ACTIVETIMER] == idCardParsed) bDeleteActive = true;
    var rgPropsRemove = [];
    if (obj[hash]) rgPropsRemove.push(hash);
    if (bDeleteActive) rgPropsRemove.push(SYNCPROP_ACTIVETIMER);

    if (rgPropsRemove.length == 0) return;

    if (bWarn && !confirm("You no longer have access to the card. Remove the timer?")) return;
    ...
    chrome.storage.sync.remove(rgPropsRemove, function () {
      ...
      updateTimerChromeIcon();
      if (bDeleteActive) findNextActiveTimer();
...
globalThis.getTimerElemText = function getTimerElemText(msStart, msEnd, bValues, bNoSeconds) {
  bNoSeconds = bNoSeconds || false;
  var txt = "";
  var ms = 0;
  if (msStart != null) ms = msEnd - msStart;
  ...
  txt =
    (days == 0 ? "" : "" + days + "d ") +
    (unit == UNITS.minutes ? "" : getWithZeroPrefix(hours) + ":") +
    getWithZeroPrefix(minutes) +
...
globalThis.showTimerPopup = function showTimerPopup(idCard, ptClick) {
  sendExtensionMessage({ method: "showTimerWindow", idCard: idCard, ptClick: ptClick }, function (response) {});
};

globalThis.g_cTimesZeroTimeout = 0;
globalThis.g_cTimesContinuousZeroTimeout = 0;
 ...
 *
 **/
globalThis.processThreadedItems = function processThreadedItems(
  tokenTrello,
  items,
  onPreProcessItem,
  onProcessItem,
  ...
  function onFinishedInternal(status) {
    if (bReturned)
      //allow multiple calls, simplifiers error handling in callers
      return;
    bReturned = true;
  ...
  function startProcess() {
    ...
    for (var iitem = 0; iitem < items.length; iitem++) {
      ...
      if (!onPreProcessItem || onPreProcessItem(itemCur)) {
        cNeedProcess++;
        if (cNeedProcess <= CMAX_THREADS) queueCall(itemCur, iitem, (cNeedProcess - 1) * MS_TRELLOAPI_WAIT);
      } else {
    ...
    } else cReadyToWait = true; //this might not be necessary, but it doesnt hurt
  }

  function queueCall(item, iitem, ms) {
    if (bReturned)
      //a previous error could have already returned
      return;
    ...
    if (needsProcessItemDelay && !needsProcessItemDelay(item, iitem)) {
      ms = 0;
      g_cTimesZeroTimeout++;
      if (g_cTimesZeroTimeout % 50 == 0) ms = 50; //breath in case of consecutive zero waits. doesnt hurt when not consecutive
    }
    if (ms == 0) ms = undefined; //faster, specially in Canary (2016 last checked)

    if (ms === undefined) g_cTimesContinuousZeroTimeout++;
  ...
  function postProcessItem(status, item, iitem) {
    ...
    if (onFinishedEach) onFinishedEach(status);
    ...
    for (var i = iitem + 1; i < items.length; i++) {
      ...
      if (itemCur.bFinishedQuery || itemCur.bPendingQuery) continue;
...
globalThis.g_regexParseSE = null;
globalThis.g_regexRemoveHashtags = /#[\S-]+/g;
 ...
 *
 */
globalThis.parseSE = function parseSE(title, bKeepHashTags) {
  var bAcceptSFT = g_bAcceptSFT;
  var se = { bParsed: false, bSFTFormat: false };

  if (bAcceptSFT) se = parseSE_SFT(title);
  ...
  if (se.bParsed) {
    ...
    if (!g_bAcceptPFTLegacy || rgResults == null || rgResults[2] === undefined || rgResults[3] === undefined) {
      ...
      if (bAcceptSFT) se.bSFTFormat = true;
    } else {
  ...
  if (bKeepHashTags === undefined || bKeepHashTags == false)
    //review zig cleanup by initializing to bKeepHashTags = bKeepHashTags || false and testing !bKeepHashTags
    se.titleNoSE = replaceString(se.titleNoSE, g_regexRemoveHashtags, "");
  return se;
};

globalThis.g_regexSFT1 = null;
globalThis.g_regexSFT2 = null;
globalThis.parseSE_SFT = function parseSE_SFT(title) {
  ...
  if (g_regexSFT1 === null) g_regexSFT1 = new RegExp("^.*?" + makePatt(leftDelim, rightDelim) + ".*$"); //*? means non-greedy match so find first
  var patt = g_regexSFT1;
  var rgResults = patt.exec(title);

  if (rgResults == null || rgResults[1] === undefined) se.estimate = 0;
  else {
  ...
  leftDelim = "[";
  rightDelim = "]";
  if (!g_regexSFT2) g_regexSFT2 = new RegExp("^.*" + makePatt(leftDelim, rightDelim) + ".*$"); //normal (greedy) match so it finds last;
  patt = g_regexSFT2;
  rgResults = patt.exec(title);
  if (rgResults == null || rgResults[1] === undefined) se.spent = 0;
  else {
...
globalThis.getCardData = function getCardData(tokenTrello, idCardLong, fields, bBoardShortLink, callback, waitRetry) {
  ...
  if (bBoardShortLink) url = url + "&board=true&board_fields=shortLink";
    ...
    if (xhr.readyState == 4) {
      ...
      function handleFinishRequest() {
        ...
        if (xhr.status == 200) {
          ...
          if (bHandledDeletedOrNoAccess(xhr.status, objRet)) {
            //no permission to the board, or card deleted already
            null; //happy lint
          } else if (xhr.status == 429) {
            //too many request, reached quota.
...
globalThis.renameCard = function renameCard(tokenTrello, idCard, title, callback, statusNoAccessCustom, waitRetry) {
  if (!statusNoAccessCustom) statusNoAccessCustom = "error: permission error or deleted";
  //https://trello.com/docs/api/card/index.html
    ...
    if (xhr.readyState == 4) {
      ...
      function handleFinishRequest() {
        ...
        if (xhr.status == 200) {
          ...
          if (bHandledDeletedOrNoAccess(xhr.status, objRet, statusNoAccessCustom)) {
            //no permission or deleted
            null; //happy lint
          } else if (xhr.status == 429) {
            //too many request, reached quota.
            var waitNew = (waitRetry || MsWaitRetry()) * 2;
...
globalThis.g_regexBracketOpen = /\[/g;
globalThis.g_regexBracketClose = /\]/g;

globalThis.replaceBrackets = function replaceBrackets(str) {
...
globalThis.getTimeDifferenceAsString = function getTimeDifferenceAsString(msDateParam) {
  assert(msDateParam);
...
globalThis.elementInViewport = function elementInViewport(el) {
  //thanks to http://stackoverflow.com/a/125106/2213940
...
globalThis.removeBracketsInNote = function removeBracketsInNote(note) {
  var iFindBracket = note.lastIndexOf("]");
  if (iFindBracket >= 0) {
    iFindBracket++;
    if (iFindBracket >= note.length) note = "";
    else note = note.substring(iFindBracket);
  }
  return note.trim();
};

globalThis.groupRows = function groupRows(rowsOrig, propertyGroup, propertySort, bCountCards, extraFieldsData) {
  var ret = [];
  var i = 0;
  ...
  if (extraFieldsData) {
    bHasExtraData =
      Object.keys(extraFieldsData.cardData).length > 0 || Object.keys(extraFieldsData.cfmetaData).length > 0;
    if (bHasExtraData) {
      ...
      for (i = 0; i < cMax; i++) {
          ...
          if (cardCFCur) {
            if (cfmetaData) {
              ...
              for (var idCF in cf) {
                ...
                var idCFUse = idCF;
                if (cfmetaDataCur.idMaster) idCFUse = cfmetaDataCur.idMaster;
                var cfCurField = cf[idCF];
                var valCF = "";
                if (cfCurField) valCF = cfCurField[cfmetaDataCur.type];
                if (cfmetaDataCur.type == "number") valCF = parseFloat(valCF || "0.0") || 0.0;
                else if (cfmetaDataCur.type == "date") valCF = makeDateCustomString(new Date(valCF), true);
                if (!row.cfData) {
                  row.cfData = {};
        ...
        if (bHasGroup) {
          for (; iProp < pGroups.length; iProp++) {
            ...
            var valCur = "";
            if (propname == "dowName") {
              //review zig: not yet in UI
              if (!row.dowName) row.dowName = getWeekdayName(new Date(row.date * 1000).getDay(), false); //NOTE we modify the row here. needed by reports
              valCur = row.dowName;
            } else if (propname == "hashtagFirst" || propname == "hashtags") {
              if (row.hashtagFirst === undefined) {
                //splitRows might have already loaded them
                var rgHash = getHashtagsFromTitle(row.nameCard || "", true);
                if (rgHash.length > 0) valCur = rgHash[0];
                else valCur = "";
          ...
          var group = map[key];
          if (group === undefined) group = cloneObject(row);
          else {
            ...
            if (
              row.rowid !== undefined &&
              row.rowid != ROWID_REPORT_CARD &&
              (group.rowid === undefined || row.rowid > group.rowid)
            ) {
  ...
  if (propertySort == "dow") {
    ...
    for (i = 0; i < ret.length; i++) {
      ...
      if (row.bSkip) continue;
      if (!row.dow) row.dow = new Date(row.date * 1000).getDay(); //NOTE we modify the row here. needed by reports
    }
  }
  ...
  if (ret.length > 0 && propertySort.length > 0 && (propertySort != "date" || propertyGroup.length > 0)) {
    ...
    ret.sort(function doSort(a, b) {
      function compareItems(a, b) {
        ...
        if (bString) return a[propertySort].localeCompare(b[propertySort]);
        var va = null;
        var vb = null;
        ...
        if (bRemain) {
          ...
          if (propertySort == "dateDue") {
            //REVIEW ZIG: ugly duplication in setReportData
            va = b[propertySort];
            vb = a[propertySort];
          } else {
            va = a[propertySort];
      ...
      if (cmp === 0) {
        cmp = b.date - a.date;
        if (cmp == 0 && b.rowid !== undefined && a.rowid !== undefined) cmp = b.rowid - a.rowid; //use entry order. covers transfers (and sometimes immediate spent, when it also falls on the same second) having same date
      }
      return cmp;
    });
  }
...
globalThis.getXFromUrl = function getXFromUrl(url, prefix) {
  if (!url || url.indexOf(prefix) != 0) return null;

  var remainUrl = url.slice(prefix.length);
  var iNextSlash = remainUrl.indexOf("/");
  if (iNextSlash >= 0) remainUrl = remainUrl.slice(0, iNextSlash);
  return remainUrl;
};

globalThis.getIdCardFromUrl = function getIdCardFromUrl(url) {
  return getXFromUrl(url, "https://trello.com/c/");
};

globalThis.getIdBoardFromUrl = function getIdBoardFromUrl(url) {
  return getXFromUrl(url, "https://trello.com/b/");
};

globalThis.elemShowHide = function elemShowHide(elem, bShow, ms) {
  if (bShow) {
    if (ms) elem.slideDown(ms);
    else elem.show();
  } else {
    if (ms) elem.slideUp(ms);
    else elem.hide();
  }
};

globalThis.isLicException = function isLicException() {
  return (
    navigator &&
    navigator.userAgent &&
...
globalThis.hitAnalytics = function hitAnalytics(category, action, bSkipNewbie) {
  sendExtensionMessage(
    { method: "hitAnalyticsEvent", category: category, action: action, bSkipNewbie: bSkipNewbie },
    function (response) {}
  );
};
