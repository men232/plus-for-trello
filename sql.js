...
function handleGetReport(request, sendResponse, bAllowWhileOpening, cRetries) {
	...
	g_db.executeSql(sql, values, function (result) {
		sendResponse({ status: STATUS_OK, rows: result.rows });
	},
	function errorTransaction(err) {
		logPlusError("error in handleGetReport: " + sql);
        console.error('[handleGetReport:err]', err);
...
function handleCardCreatedUpdatedMoved(data, rowParam, bVerifyBoardIsCardsBoard, tx, callback) {
	...
	function onOk(tx2, resultSet) {
		...
		if (!rowCard && resultSet.rows.length > 0) {
		    rowCard = cloneObject(resultSet.rows[0]); //clone to modify it
...
async function handleDeleteDB(request, sendResponseParam) {
    g_bOpeningDb = true;
	...
	var db = g_db;
	if (db == null) {
	    db = await rawOpenDb(); //open it without migration. maybe migration failed before
	}
...
function handleOpenDB(options, sendResponseParam, cRetries) {
    ...
    async function handleOpenDBWorker(options, sendResponseParam, cRetries) {
        var db = null;
        ...
        function finalResponse(thisResponse) {
            if (thisResponse.status == STATUS_OK) {
                if (db) {//db null returning the cache
                    ...
                    if (localStorage.getItem(LS_KEY_detectedErrorLegacyUpgrade)) {
                        handleShowDesktopNotification({
        ...
        function sendResponse(thisResponse) {
            ...
            handleGetReport(request,
                function (response) {
                    ...
                    if (response.status == STATUS_OK) {
                        console.warn('[handleGetReport] Empty report rows');
        ...
        M.migration(1, function (t) {
            ...
            localStorage.removeItem("oauth_token" + scopeOld);
            localStorage.removeItem("oauth_token_secret" + scopeOld);
            ...
            t.executeSql(sql, [],
                    function (tx2, results) {
                        if (results.rows.length > 0)
                            localStorage.setItem(LS_KEY_detectedErrorLegacyUpgrade, "1"); //existance of property means it detected. "reset sync" will remove it
                    },
