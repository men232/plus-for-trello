...
function updateNewLink() {
  ...
  function doUpdateWorker() {
    chrome.storage.local.get(
      [keyLastSyncViewed, keyLastSync, key_plus_datesync_last, keyplusSyncLastStatus],
      function (obj) {
        ...
        if (msplus_datesync_last !== undefined) {
          ...
          tooltipSync =
            tooltipSync +
            "\nLast sync " +
            getTimeDifferenceAsString(msplus_datesync_last, true) +
            " @" +
            dateLastSync.toLocaleTimeString();
        }
        ...
        if (
          rowidLastSyncViewed &&
          rowidLastSync &&
          rowidLastSyncViewedNew == rowidLastSyncViewed &&
          rowidLastSyncNew == rowidLastSync
        )
          return; //nothing to do

        rowidLastSyncViewed = rowidLastSyncViewedNew;
        rowidLastSync = rowidLastSyncNew;
        var urlReport = chrome.runtime.getURL("report.html");
        var elem = $("#reportLinkNew");
        elem.attr(
          "href",
          urlReport +
            "?chartView=s&pivotBy=year&orderBy=date&archived=-1&deleted=-1&afterRow=-1&afterRow=" +
            rowidLastSyncViewed +
            "&setLastRowViewed=true"
        );
        if (!g_bNoSE) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (false) {
    ...
    if (bWindows) $("body").addClass("agile_cmenu_body_windows");
  }
...
function getCardAndBoard(idCard, callback) {
  var sql =
    "SELECT c.name as nameCard, b.name as nameBoard FROM cards c JOIN boards b WHERE c.idCard=? AND c.idBoard=b.idBoard";
...
function updateButtonTitle(button, idCardTimer) {
  getCardAndBoard(idCardTimer, function (data) {
    if (data && data.board && data.card) button.prop("title", "Click to open card\n" + data.card + "\n" + data.board);
    else button.prop("title", "Click to open card"); // if card isnt on db
  });
}
  ...
  chrome.storage.sync.get([SYNCPROP_ACTIVETIMER], function (obj) {
    ...
    if (idCardTimer) {
      ...
      getCardTimerData(hash, function (objTimer) {
        ...
        if (stored === undefined || stored.msStart == null || stored.msEnd != null) return;
        ...
        button.click(function () {
          sendExtensionMessage(
            { method: "openCardWindow", idCard: idCardTimer, bForceTab: true },
            function (response) {}
          );
        });
      });
...
function loadPopup() {
  ...
  function setSyncIcon(bSync) {
    if (bSync) {
      imgSync.attr("src", chrome.runtime.getURL("images/syncingnow.gif"));
      imgSync.css("cursor", "default");
    } else {
      imgSync.attr("src", chrome.runtime.getURL("images/sync.png"));
  ...
  imgSync.click(function () {
    ...
    sendExtensionMessage({ method: "plusMenuSync" }, function (response) {
      ...
      else setSyncIcon(false);
    });
  });

  if (true) configData();
  else setTimeout(configData, 5000); //for debugging
}
...
function configData() {
  openPlusDb(function (response) {
    ...
    configureTimerElem(function () {
      if (false)
        //for easy debugging. must be false
        setTimeout(function () {
          listAllBoards();
        }, 12000);
      else listAllBoards();
...
function listAllBoards() {
  var sql =
    "SELECT b.idBoard, b.name, MAX(h.date) as maxDate FROM boards AS b LEFT OUTER JOIN history as H ON b.idBoard=h.idBoard WHERE b.idBoard <> ? AND b.bArchived=0 GROUP BY b.idBoard ORDER BY " +
    (g_bEnableTrelloSync ? "b.dateSzLastTrello" : "maxDate") +
    " DESC";
  var status = $("#progress");
  var cardResults = $("#agile_popup_cards_container");
  var urlBaseDashboard = chrome.runtime.getURL("dashboard.html") + "?";
  var urlBaseReport = chrome.runtime.getURL("report.html") + "?groupBy=idCardH&idBoard=";
  getSQLReport(sql, [IDBOARD_UNKNOWN], function (response) {
    ...
    if (rows === undefined || rows.length == 0) {
      ...
      else if ((localStorage.getItem("plus_first_trello_sync_completed") || "") != "true")
        textStatus = "First sync has not yet completed. Please wait.";
      else textStatus = "No boards with you as direct member. Add yourself to some boards.";
    ...
    for (; rows && i < rows.length; i++) {
      ...
      var imgDash = $("<img title='Burndown'>")
        .attr("src", chrome.runtime.getURL("images/chart-sm.png"))
        .addClass("agile_img_popup");
      var imgReport = $("<img title='Report'>")
    ...
    if (searchBox.val().length > 0)
      //happens if you 1) type, 2) zoom-in weekly report, 3) back
      doSearch();

    //fill sums of S/E
    var sql2 =
      "SELECT b.idBoard, sum(h.spent) as spent, sum(h.est) as est FROM boards AS b JOIN history as H ON b.idBoard=h.idBoard GROUP BY h.idBoard";
    getSQLReport(sql2, [], function (response2) {
      ...
      for (; i < rows.length; i++) {
        ...
        if (spentDisplay < 10) spentDisplay = row.spent;
        else spentDisplay = Math.round(row.spent);

        if (estDisplay < 10) estDisplay = row.est;
        else estDisplay = Math.round(row.est);
        elemCur.se.text(parseFixedFloat(spentDisplay) + " / " + parseFixedFloat(estDisplay));
        if (row.spent >= 0 && row.est > 0)
          elemCur.div.attr(
            "title",
            elemCur.div.attr("title") + "\n" + Math.round((row.spent * 100) / row.est) + "% complete"
          );
      }
    });
    //	}, 100);
...
function fillCardResults(divResults, val, bSearchComments, chSplit, cSearchCur) {
  ...
  if (bSearchComments) cDays = Math.round(cDays / 3); //4 months of data
  var sDateLimit = Math.round(Date.now() / 1000 - 60 * 60 * 24 * cDays);
  var paramsSql = [sDateLimit];

  var sql =
    "SELECT c.idCard, c.name AS nameCard, b.name as nameBoard, c.idBoard, MAX(cb.date) as maxDate FROM cards AS c LEFT OUTER JOIN cardbalance AS cb ON c.idCard=cb.idCard JOIN boards AS b ON b.idBoard=c.idBoard WHERE  (cb.date is NULL or cb.date >?) AND LOWER(c.name) GLOB ? GROUP BY c.idCard ORDER BY  " +
    (g_bEnableTrelloSync ? "c.dateSzLastTrello" : "maxDate") +
    " DESC LIMIT 10";

  if (bSearchComments)
    sql =
      "SELECT h.comment, c.idCard, c.name AS nameCard, b.name as nameBoard, c.idBoard, MAX(h.date) as maxDate FROM cards AS c JOIN history AS h ON c.idCard=h.idCard JOIN boards AS b ON b.idBoard=h.idBoard WHERE h.date>? AND LOWER(h.comment) GLOB ? GROUP BY c.idCard ORDER BY maxDate DESC LIMIT 10";

  if (bAndSearch) {
    //review zig: cant set together with bSearchComments
    sql =
      "SELECT c.idCard, c.name AS nameCard, b.name as nameBoard, c.idBoard, MAX(cb.date) as maxDate FROM cards AS c LEFT OUTER JOIN cardbalance AS cb ON c.idCard=cb.idCard JOIN boards AS b ON b.idBoard=c.idBoard WHERE  (cb.date is NULL or cb.date >?) ";
    ...
    function processEachCard(idCard, obj) {
      if (obj && obj.msEnd == null && obj.msStart) {
        ...
        if (data) {
          ...
          var imgTimer = $("<img title='Timer'>")
            .attr("src", chrome.runtime.getURL("images/timer-sm-on.png"))
            .addClass("agile_img_popup agile_noOpacity");
  ...
  getSQLReport(sql, paramsSql, function (response) {
    if (cSearchCur != g_cSearchTotal) return; //stop, will be processed again
    var rows = response.rows;
    var i = 0;
    ...
    for (; rows && i < rows.length; i++) {
      var row = rows[i];
      if (g_mapCards[row.idCard]) continue;
      var item = $("<div tabindex=0>").addClass("agile_board_dashboardItem").addClass("agile_board_dashboardItem_Card");
      var date = null;

      if (row.maxDate != null) date = new Date(row.maxDate * 1000).toLocaleDateString();
      else date = "never";
      ...
      //setPopupClickHandler(imgReport, urlReport);
      var titleUse = null;
      if (bSearchComments) titleUse = row.nameCard + "\nNote: " + row.comment + "\nOn: " + date;
      else titleUse = row.nameCard + "\nLast S/E on: " + date;
    ...
    divResults.empty();
    if (!bEmpty) {
      if (bSearchComments) divResults.append($("<b>Found in Plus comments:</b>"));
      else if (bAndSearch) divResults.append($("<b>Found by juggling search terms:</b>"));
      divResults.append(list);
    }
    if (bSearchComments) {
      var chSplit = "*";
      if (val.indexOf(chSplit) < 0) chSplit = " ";
