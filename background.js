...
function handleExtensionMessage(request, sendResponseParam, idTabSender, idWindowSender) {
    ...
    function doit() {
        if (request.method == "getConfigData") {
            ...
            sendResponse({ status: STATUS_OK, bCompletedFirstSync: ((localStorage.getItem("plus_first_trello_sync_completed") || "") == "true") });
        }
...
If you instead want to disable timer popups do so from Plus preferences.");
            ...
            localStorage.setItem('plus_rename_cards_pending_data', JSON.stringify({ pending: true, bOnlyCardsWithHistory: request.bOnlyCardsWithHistory }));
            sendResponse({ status: STATUS_OK });
        }
            ...
            localStorage.clear();
            if (savedId)
                localStorage.setItem(g_analytics.PROP_IDANALYTICS, savedId);
...
var g_analytics = {
    ...
    setDisabled: function (bDisabled) {
        if (bDisabled)
            localStorage.setItem(this.PROP_DISABLEANALYTICS, "true");
        else
            delete localStorage.removeItem(this.PROP_DISABLEANALYTICS);
            ...
            setTimeout(function () {
                ...
                xhr.onreadystatechange = function (e) {
                    if (xhr.readyState == 4 && xhr.status == 200) {
                        ...
                        localStorage.setItem(PROP_LS_CD1LAST, valCD1Cur);

                        if (valCD2Prev != valCD2Cur)
                        localStorage.setItem(PROP_LS_CD2LAST, valCD2Cur);

                        if (valCD3Prev != valCD3Cur)
                            localStorage.setItem(PROP_LS_CD3LAST, valCD3Cur);

                        if (valCD4Prev != valCD4Cur)
                            localStorage.setItem(PROP_LS_CD4LAST, valCD4Cur);

                        if (valCD5Prev != valCD5Cur)
                            localStorage.setItem(PROP_LS_CD5LAST, valCD5Cur);

                        if (valCD6Prev != valCD6Cur)
                            localStorage.setItem(PROP_LS_CD6LAST, valCD6Cur);
                    }
                };
