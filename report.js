...
function updateNamedReport(url) {
    if (g_namedReport)
        localStorage.setItem(g_namedParams.namedReport + ":" + g_namedReport, url);
}
...
function loadReport(params) {
    ...
    for (var iobj in elems) {
        ...
        if (g_namedReport == "_counts" && iobj == "list" && !localStorage.getItem(PROP_LS_bShowedDefaultCPMCountsListFilter) && params[iobj] == "doing") {
