...
globalThis.g_dataTotalSpentThisWeek = { str: null, weeknum: null };
globalThis.g_msSyncRateLimit = 1000 * 1; //1 second (used to be larger and more relevant back when syncing on spreadsheets with my developer key
globalThis.MSDELAY_FIRSTSYNC = 500;
globalThis.g_bOffline = false;
globalThis.g_cErrorSync = 0; //errors during sync period. hack alert: it is set to specific values, does not really reflect a real count but a state
globalThis.g_strTimerLast = "";
globalThis.g_idTimeoutTimer = null;
globalThis.PLUS_BACKGROUND_CALLER = true; //allows us to tell shared.js we are calling
globalThis.g_bInstalledNetworkDetection = false;
globalThis.g_bDetectTrelloNetworkActivity = false;
globalThis.g_cTrelloActivitiesDetected = 0;
globalThis.g_bLastPlusMenuIconError = false; //remembers if the icon last drew the red error X
globalThis.g_mapTimerWindows = {}; // {idWindow, can be undefined or invalid }
globalThis.PROP_LS_cViewedBuyDialog = "plus_viewed_buy_dialog"; //count stored as string
globalThis.g_msUpdateNotificationReceived = 0; //0 means not received
globalThis.g_nameMinimizeTimer = "Min";
...
globalThis.handleUpdateExtension = function handleUpdateExtension(details) {
  ...
  chrome.storage.local.set(pair, function () {
    ...
    else g_msUpdateNotificationReceived = Date.now();
  });
};
...
globalThis.getConfigData = function getConfigData(urlService, userTrello, callback, bSkipCache) {
  var data = null;
  //review zig: assumes g_optEnterSEByComment is loaded. should be. assert in IsEnabled ensures it wont continue if not.
  if (g_optEnterSEByComment.IsEnabled()) urlService = ""; //to be more consistent with trello sync when no google user was configured
  if (bSkipCache === undefined || bSkipCache === false) data = localStorage.getItem("plus_config_data");
  if (data !== undefined && data != null) data = JSON.parse(data);
...
globalThis.doCallUrlConfig = function doCallUrlConfig(urlConfig, userTrello, callback) {
  if (!urlConfig) {
    callback(null);
    return;
  }
...
globalThis.DATAVERSION_SIMPLEPLUS = 1;

globalThis.doGetJsonConfigSimplePlus = function doGetJsonConfigSimplePlus(url, user) {
  ...
  if (partLeft.indexOf("&") >= 0) partLeft = partLeft.split("&")[0];

  if (bNewSheet && partLeft.indexOf("/") >= 0)
    //new sheets have "/edit"
...
// number to string
globalThis.gid_to_wid = function gid_to_wid(gid) {
...
globalThis.standarizeSpreadsheetValue = function standarizeSpreadsheetValue(value) {
  if (value == "#VALUE!" || value == "--") value = "";
  if (typeof value == "string" && value.indexOf("'") == 0) value = value.substr(1);
  return value;
};

globalThis.handleRawSync = function handleRawSync(bFromAuto, sendResponseParam) {
  var PROP_SERVICEURL = "serviceUrl";

  function sendResponse(response) {
    if (sendResponseParam) sendResponseParam(response);
  }
    ...
    chrome.storage.sync.get([PROP_SERVICEURL], function (obj) {
      ...
      getConfigData(urlSync, userTrello, function (responseConfig) {
        ...
        handleOpenDB(null, function (responseOpen) {
          ...
          handleSyncDB(
            { config: responseConfig, bUserInitiated: !bFromAuto },
            function (responseSync) {
...
globalThis.g_strBadgeText = "";
globalThis.PLUS_COLOR_SPENTBADGE = "#B10013";

globalThis.getFormattedSpentBadgeText = function getFormattedSpentBadgeText() {
  var l = g_strBadgeText.length;
  if (l > 0 && l < 4) return g_strBadgeText + UNITS.current;
  return g_strBadgeText;
};

globalThis.setIconBadgeText = function setIconBadgeText(text, bAsTimer) {
  ...
  if (g_strTimerLast.length > 0) {
    chrome.action.setBadgeBackgroundColor({ color: "#2A88BE" });
    chrome.action.setBadgeText({ text: g_strTimerLast });
  } else {
    assert(!bAsTimer);
    chrome.action.setBadgeBackgroundColor({ color: PLUS_COLOR_SPENTBADGE });
    var textSet = "";
    if (g_optAlwaysShowSpentChromeIcon != OPT_SHOWSPENTINICON_NEVER) textSet = getFormattedSpentBadgeText(text);
    chrome.action.setBadgeText({ text: textSet });
  }
};

globalThis.calculateSyncDelay = function calculateSyncDelay(callback) {
  ...
  chrome.storage.local.get(keyPlusDateSyncLast, function (obj) {
    ...
    if (msDateSyncLast !== undefined) {
      ...
      if (deltaCur < g_msSyncRateLimit) delay = g_msSyncRateLimit - deltaCur;
    }
    setTimeout(function () {
      callback();
    }, delay);
...
globalThis.handleGetAllHashtags = function handleGetAllHashtags(sendResponse) {
  var request = {
    sql: "SELECT name FROM cards WHERE name LIKE '%#%' AND bDeleted=0 AND bArchived=0 AND idBoard <> ? AND idBoard in (SELECT idBoard from Boards where bArchived=0)",
    values: [IDBOARD_UNKNOWN],
  };
  handleGetReport(request, function (responseReport) {
    ...
    for (var iResult = 0; iResult < result.length; iResult++) {
      ...
      if (hLast && hCur.toLowerCase() == hLast.toLowerCase()) continue;
...
globalThis.handlePlusMenuSync = function handlePlusMenuSync(sendResponse) {
  loadBackgroundOptions(function () {
    handleRawSync(false, sendResponse);
  });
};

globalThis.handleRequestProPermission = function handleRequestProPermission(sendResponse) {
...
globalThis.handleGoogleSyncPermission = function handleGoogleSyncPermission(sendResponse) {
...
globalThis.g_idCardTimerLast = null;

globalThis.handleShowAllActiveTimers = function handleShowAllActiveTimers() {
  findAllActiveTimers(function (rgIdCards) {
    ...
    if (rgIdCards.length == 0) onFinishedAll(); //edge or impossible case
    else {
      rgIdCards.forEach(function (idCard) {
        doShowTimerWindow(idCard, function (status, properties) {
          ...
          if (status == STATUS_OK && properties && properties.bExisted) {
            cExisting++;
            if (properties.bMinimized) cMinimized++;
          }

          if (cProcessed == cTotal) onFinishedAll();
        });
      });
    }
  });
...
globalThis.processTimerCounter = function processTimerCounter(bLoadingExtension) {
  ...
  function getTimerText(response) {
    ...
    chrome.storage.sync.get([keyUnits, SYNCPROP_ACTIVETIMER, SYNCPROP_optAlwaysShowSpentChromeIcon], function (obj) {
      ...
      if (obj[SYNCPROP_ACTIVETIMER] !== undefined) idCardTimer = obj[SYNCPROP_ACTIVETIMER];
        ...
        if (g_idCardTimerLast == null && bLoadingExtension) {
          ...
          setTimeout(function () {
            ...
            doShowTimerWindow(idCardTimer, function (status) {
              ...
              findAllActiveTimers(function (rgIdCards) {
                rgIdCards.forEach(function (idCard) {
                  if (idCard != idCardTimer) doShowTimerWindow(idCard);
                });
              });
            });
          }, 2000);
      ...
      if (idCardTimer) {
        ...
        getCardTimerData(hash, function (objTimer) {
          ...
          if (unit == UNITS.hours) {
            if (time.hours > 9) text = "" + Math.round(UNITS.TimeToUnits(msEnd - msStart) * 10) / 10;
            else {
              if (time.hours == 0) text = "0:" + time.minutes; //cleaner
              else text = "" + time.hours + ":" + prependZero(time.minutes);
            }
          } else if (unit == UNITS.minutes) {
            if (time.minutes > 9999) text = "+9999";
            else text = "" + time.minutes;
          } else {
  ...
  getTimerText(function responseTimer(strTimer, msRemain) {
    ...
    if (strTimer.length == 0) {
      ...
      if ((bChanged && bWasEmpty) || bChangedIdCard) animateFlip();
    }
  });
};

globalThis.doInstallNetworkDetection = function doInstallNetworkDetection() {
  if (g_bInstalledNetworkDetection) return;
    ...
    function (details) {
      if (details.statusCode == 200 && g_bDetectTrelloNetworkActivity && details.method != "GET") {
        ...
        var bHandleIt = rgIgnore.every(function (str) {
          if (details.url.indexOf(str) >= 0) return false; //stop
          return true;
        });
        if (bHandleIt) handleDetectedTrelloActivity();
      }
    },
      ...
      if (
        ...
        chrome.tabs.get(details.tabId, function (tab) {
          if (tab && tab.highlighted) {
            //highlighted reduces chances we show the message for a tab that chrome is retrying in the background
...
globalThis.doShowBoardCardOfflineNotification = function doShowBoardCardOfflineNotification(url) {
  var idBoard = null;
  var idCard = null;
  if (url.toLowerCase().indexOf("/b/") >= 0) idBoard = url.split("/")[4];
  else if (url.toLowerCase().indexOf("/c/") >= 0) idCard = url.split("/")[4];
      ...
      if (idBoard) {
        ...
        handleGetReport(request, function (responseReport) {
          if (responseReport.status == STATUS_OK && responseReport.rows.length > 0) {
            handleShowDesktopNotification({
              timeout: 10000,
              idUse: g_prefixOfflineBoardNotification + idBoard,
              notification:
                "Chrome is offline. Click to open a card report for this board: " + responseReport.rows[0].name,
            });
          }
        });
      } else if (idCard) {
        ...
        handleGetReport(request, function (responseReport) {
          if (responseReport.status == STATUS_OK && responseReport.rows.length > 0) {
            ...
            handleShowDesktopNotification({
              timeout: 10000,
              idUse: g_prefixOfflineCardNotification + idBoard + ":" + idCard,
              notification:
                "Chrome is offline. Click to open a card report for this card and board: " +
                responseReport.rows[0].name,
            });
          }
        });
      }
...
globalThis.handleDetectedTrelloActivity = function handleDetectedTrelloActivity() {
...
globalThis.handleQueryTrelloDetectionCount = function handleQueryTrelloDetectionCount(sendResponse) {
  var cRet = g_cTrelloActivitiesDetected;
  g_cTrelloActivitiesDetected = 0;
  sendResponse({ status: STATUS_OK, count: cRet });
};

globalThis.g_bPlusExtensionLoadedOK = false;
globalThis.g_bRetryWhenNotLoadedOK = true;

globalThis.setUninstallURLCustom = function setUninstallURLCustom() {
...
globalThis.g_loaderDetector = {
  initLoader: function () {
    ...
    if (!globalThis.Promise) {
      ...
      setTimeout(function () {
        handleShowDesktopNotification({
          notification:
            "Sorry, your Chrome browser is outdated. Plus for Trello requires Chrome 33 or later.\nUpdate Chrome or remove Plus from the Chrome Menu + More Tools + Extensions.",
          timeout: 30000,
        });
      }, 1000);
    ...
    setTimeout(function () {
      //avoid global dependencies. however, this timeout could cause content script to call before we are ready. in messaging we handle it.
      g_analytics.init();
      loadBackgroundOptions(function () {
        thisLocal.init();
  ...
  init: function () {
    ...
    setInterval(function () {
      if (g_msRequestedSyncPause == 0) return;
      var msNow = Date.now();
    ...
    function doNetDetect() {
      if (g_bInstalledNetworkDetection) {
        if (intervalNetDetect) clearInterval(intervalNetDetect);
    ...
    function checkNeedsSync(bForce) {
      ...
      chrome.storage.sync.get([keySyncOutsideTrello], function (obj) {
        ...
        var bLastErrorWasOffline =
          g_lastStatusSyncCache &&
          ((g_lastStatusSyncCache.statusRead &&
            g_lastStatusSyncCache.statusRead.indexOf(Language.NOINTERNETCONNECTION) >= 0) ||
            (g_lastStatusSyncCache.statusWrite &&
              g_lastStatusSyncCache.statusWrite.indexOf(Language.NOINTERNETCONNECTION) >= 0));

        if (!bSyncOutsideTrello && !bLastErrorWasOffline) return;

        if (
          g_optEnterSEByComment.IsEnabled() &&
          (localStorage.getItem("plus_first_trello_sync_completed") || "") != "true"
        )
        ...
        chrome.storage.local.get(keyPlusDateSyncLast, function (obj) {
          var msDateSyncLast = obj[keyPlusDateSyncLast];
          var dateNow = new Date();
          if (!bForce && msDateSyncLast !== undefined && dateNow.getTime() - msDateSyncLast < g_msSyncRateLimit) return;
          handleRawSync(true);
        });
      });
    }
...
globalThis.checkAnalyticsActive = function checkAnalyticsActive() {
  if (!g_db)
    //active only if at least opened the db
    return;
...
globalThis.roundRect = function roundRect(ctx, x, y, width, height, radius, fill, stroke) {
...
globalThis.g_dataSyncLast = { stage: "", ms: 0 }; //to avoid spamming tooltip update
globalThis.g_animationFrames = 18;
globalThis.g_msAnimationSpeed = 17;
globalThis.g_rotation = 0;
globalThis.g_bHilitePlusIcon = false;

globalThis.ease = function ease(x) {
  return (1 - Math.sin(Math.PI / 2 + x * Math.PI)) / 2;
};

globalThis.animateFlip = function animateFlip() {
  if (true) {
    //review zig: temporarily stopped until rotation is fixed
...
globalThis.updatePlusIcon = function updatePlusIcon(bTooltipOnly) {
  setTimeout(function () {
...
globalThis.updatePlusIconWorker = function updatePlusIconWorker(bTooltipOnly) {
  bTooltipOnly = bTooltipOnly || false;
  if (g_bLastPlusMenuIconError) bTooltipOnly = false; //force it
  ...
  if (
    bTooltipOnly &&
    typeof g_syncStatus != "undefined" &&
    typeof g_dataSyncLast != "undefined" &&
    g_syncStatus.bSyncing
  ) {
    if (g_dataSyncLast.stage != g_syncStatus.stage) {
      g_dataSyncLast.stage = g_syncStatus.stage;
    } else {
      if (msNow - g_dataSyncLast.ms < 200) return;
    }
    g_dataSyncLast.ms = msNow;
  }

  chrome.storage.local.get(
    [keyLastSync, keyLastSyncViewed, key_plus_datesync_last, keyplusSyncLastStatus],
    function (obj) {
      ...
      if (rowidLastSync != null && (rowidLastSyncViewed == null || rowidLastSyncViewed < rowidLastSync)) bNew = true;
      var strBase = "images/icon19";
      if (bNew) {
        strBase += "new";
      }

      if (g_bHilitePlusIcon) strBase = "images/icon19hilite";

      //returns false if there is no sync error
      function setTooltipSyncStatus() {
        var tooltipPre = "Plus for Trello\n";
        var tooltip = "";
        if (bNew) tooltipPre = tooltipPre + "New S/E rows\n";
        ...
        if (g_dataTotalSpentThisWeek.str)
          tooltipPre =
            tooltipPre + g_dataTotalSpentThisWeek.weeknum + ": " + g_dataTotalSpentThisWeek.str + " Spent \n\n";
        ...
        if (g_cWriteSyncLock == 0 && g_cReadSyncLock == 0 && !g_syncStatus.bSyncing) {
          ...
          if (statusLastSync) syncStatus = buildSyncErrorTooltip(statusLastSync);
          tooltipPre = tooltipPre + syncStatus;
        }

        if (g_cWriteSyncLock > 0) tooltip = tooltip + "Writting S/E to spreadsheet...\n";
        ...
        if (g_bUpdateSyncNotificationProgress) {
          if (g_cWriteSyncLock == 0 && g_cReadSyncLock == 0 && !g_syncStatus.bSyncing) {
            ...
            chrome.notifications.update(
              IDNOTIFICATION_FIRSTSYNCPRORESS,
              {
        ...
        if (g_bOffline) tooltip = tooltip + "\nChrome is offline.";
        chrome.action.setTitle({ title: tooltipPre + tooltip });
      ...
      if (
        bErrorSync &&
        statusTooltip.statusStr &&
        statusTooltip.statusStr.indexOf(Language.NOINTERNETCONNECTION) >= 0
      ) {
        bErrorSync = false;
      }
      if (bTooltipOnly && bErrorSync) bTooltipOnly = false;

      if (!bTooltipOnly && globalThis.document) {
        var img = document.getElementById("imgPlusMenu");
        img.setAttribute("width", "19");
        img.setAttribute("height", "19");
        img.setAttribute("src", chrome.runtime.getURL(strBase + ".png"));
      ...
      function callbackPaint() {
        ...
        if (bErrorSync) colorBackground = colorErrorSync;
        else if (g_bOffline) colorBackground = colorOffline;
        ...
        if (
          g_optAlwaysShowSpentChromeIcon == OPT_SHOWSPENTINICON_ALWAYS &&
          g_strTimerLast.length > 0 &&
          g_strBadgeText.length > 0
        ) {
...
globalThis.g_rgPorts = [];

globalThis.broadcastMessage = function broadcastMessage(message, callback) {
  setTimeout(function () {
    var i = 0;
...
globalThis.handleExtensionMessage = function handleExtensionMessage(
  request,
  sendResponseParam,
  idTabSender,
  idWindowSender
  ...
  if (!g_bPlusExtensionLoadedOK) {
    ...
    if (!g_bRetryWhenNotLoadedOK) {
      ...
      setTimeout(function () {
        if (g_bPlusExtensionLoadedOK) doit();
        else {
  ...
  function handleQueryTrelloToken(sendResponse, attempt = 1) {
    const nameVariants = ["token", "cloud.session.token"];

    chrome.cookies.getAll({ domain: "trello.com" }, (cookies) => {
      const token = cookies.find((v) => nameVariants.includes(v.name));

      if (token) {
        console.log("handleQueryTrelloToken() - token captured 🥳");
        sendResponse({ cookie_value: token.value });
      } else {
        console.warn("handleQueryTrelloToken() - not able to read the token cookies!", cookies);
    ...
    if (request.method == "getConfigData") {
      ...
      getConfigData(
        ...
        function (retConfig) {
          ...
          else sendResponse({ config: retConfig });
        },
        bSkipCache
      );
    } else if (request.method == "setTrelloAuthData") {
      localStorage.setItem("plus_trello_auth_dsc", request.dsc);
      if (request.bInFrame) {
        setTimeout(function () {
          uninjectTrelloFrame();
          handleRawSync(true);
      ...
      sendResponse({
        status: STATUS_OK,
        bCompletedFirstSync: (localStorage.getItem("plus_first_trello_sync_completed") || "") == "true",
      });
      ...
      alert(
        "Enable 'Chrome panels' for much better timers.\n\
...
If you instead want to disable timer popups do so from Plus preferences."
      ...
      if (request.bOnlyTimer) {
        ...
        if (request.bSetSpentBadge) {
          chrome.storage.sync.get([SYNCPROP_optAlwaysShowSpentChromeIcon], function (obj) {
            setOptAlwaysShowSpentChromeIcon(obj[SYNCPROP_optAlwaysShowSpentChromeIcon]);
            if (g_strTimerLast.length == 0) setIconBadgeText(g_strBadgeText, false);
            updatePlusIcon();
          });
        } else {
          updatePlusIcon();
      ...
      if (request.bAnimate) animateFlip();
      sendResponse({ status: STATUS_OK });
      ...
      if (request.tooltip !== undefined) chrome.action.setTitle({ title: request.tooltip });
      sendResponse({});
      ...
      localStorage.setItem(
        "plus_rename_cards_pending_data",
        JSON.stringify({ pending: true, bOnlyCardsWithHistory: request.bOnlyCardsWithHistory })
      );
      sendResponse({ status: STATUS_OK });
      ...
      var savedId = localStorage.getItem(g_analytics.PROP_IDANALYTICS);
      localStorage.clear();
      if (savedId) localStorage.setItem(g_analytics.PROP_IDANALYTICS, savedId);
      ...
      localStorage.removeItem("plus_rename_cards_pending_data"); //exclusive
      ...
      var text = window.prompt(
        "Paste the text from the rename backup file (dont worry about the '...' at the end after pasting)"
      );
      var obj = null;
      var totalCards = 0;
      ...
      } else if (!text || obj.totalCards <= 0) status = "No cards to process.";
      else {
        totalCards = obj.totalCards;
      }
      if (obj) g_rgUndoCardRename = obj.cards;
      else g_rgUndoCardRename = null;
      sendResponse({ status: status, totalCards: totalCards });
    } else if (request.method == "queryTrelloToken") {
      handleQueryTrelloToken(sendResponse);
    } else sendResponse({});
  }
...
globalThis.handleCopyClipboard = function handleCopyClipboard(html, sendResponse) {
...
globalThis.isTaskbarBottom = function isTaskbarBottom() {
  return navigator.platform.indexOf("Win") >= 0 || navigator.userAgent.indexOf("CrOS") >= 0; //windows and chromebooks
};

globalThis.g_bHookedNotificationsButton = false;

globalThis.handleTimerWindowRestored = function handleTimerWindowRestored(idCard) {
  var map = g_mapTimerWindows[idCard];
  if (!map) return;
  var idWindow = map.idWindow;
  if (idWindow) chrome.windows.update(idWindow, { state: "minimized" });

  handleNotifClick(idCard);
};

globalThis.handleTimerWindowLoaded = function handleTimerWindowLoaded(idCard, bClearAndMinimize) {
  var map = g_mapTimerWindows[idCard];
  if (!map) return;
  var idWindow = map.idWindow;
  try {
    ...
    if (bClearAndMinimize) {
      chrome.notifications.clear(idNotificationForTimer(idCard), function (bWasCleared) {
        if (true)
          //dont use bWasCleared for case where ther notification is no longer there when closed by system (lock screen etc)
...
globalThis.handleNotifClick = function handleNotifClick(idCard) {
  ...
  loadSharedOptions(function () {
    getSQLReportShared("SELECT bDeleted FROM cards WHERE idCard=?", [idCard])
      .then(checkDeletedCard)
      .then(openCard)
      ["catch"](function (err) {
        //review: 'catch' syntax to keep lint happy
        if (err.message)
          //message is only set if user needs to see a message. else just means stop the chain
          sendDesktopNotification(err.message, 10000); //note: timer panels rely on this, as alerts dont work in panels
      });
  });
};

globalThis.doOpenCardInBrowser = function doOpenCardInBrowser(idCard, pos, bForceTab, bForcePopup) {
    ...
    if (!bForceTab) {
      chrome.storage.sync.get([SYNCPROP_CARDPOPUPTYPE], function (obj) {
        ...
        if (cardPopupType == CARDPOPUPTYPE.NO_POPUP) {
          if (bForcePopup) makeCardPopupWindow(idCard, pos, CARDPOPUPTYPE.POPUP_SOMEACTIONS);
          else openCardAsUrl();
        } else makeCardPopupWindow(idCard, pos, cardPopupType);
      });
      return;
    }
    openCardAsUrl();
  ...
  var msNow = Date.now();
  if (mapCardWindow && msNow - mapCardWindow.ms < 8000) {
    //recent card window. depends on UPDATE_STEP
    var tabid = mapCardWindow.tabid;

    //verify the tab is alive and well
    chrome.tabs.get(tabid, function (tab) {
      var idCardCur = null;
      if (!chrome.runtime.lastError && tab && tab.url) idCardCur = getIdCardFromUrl(tab.url);

      if (idCardCur != idCard) openCard();
      else {
        ...
        chrome.windows.update(tab.windowId, { focused: true }, function (window) {
          if (chrome.runtime.lastError || !window) openCard();
          else {
...
globalThis.doOpenBoardInBrowser = function doOpenBoardInBrowser(idBoard) {
  ...
  var msNow = Date.now();
  if (mapBoardWindow && msNow - mapBoardWindow.ms < 8000) {
    //recent Board window. depends on UPDATE_STEP
    var tabid = mapBoardWindow.tabid;

    //verify the tab is alive and well
    chrome.tabs.get(tabid, function (tab) {
      var idBoardCur = null;
      if (tab) idBoardCur = getIdBoardFromUrl(tab.url);

      if (idBoardCur != idBoard) openBoardAsUrl();
      else {
        //first select the window. else chrome can get weird about selecting the tab while the window is not active
        chrome.windows.update(tab.windowId, { focused: true }, function (window) {
          if (!window) openBoardAsUrl();
          else {
            //finally, select the tab
            chrome.tabs.update(tabid, { active: true, highlighted: true }, function (tab) {
              var idBoardCur = null;
...
globalThis.makeTimerPopupWindow = function makeTimerPopupWindow(idCard, bMinimized, ptClick) {
  var marginSafety = 20;
  var width = 240;
  var height = 88;
  ...
  chrome.windows.create(
    {
      url:
        chrome.runtime.getURL("timerwin.html") +
        "?idCard=" +
        idCard +
        "&nameCard=" +
        encodeURIComponent(data.nameCard) +
        "&nameBoard=" +
        encodeURIComponent(data.nameBoard) +
        "&minimized=" +
        (bMinimized ? "1" : "0"),
      width: width,
      height: height,
    ...
    function (window) {
      if (window) {
        var map = g_mapTimerWindows[idCard];
        if (map) map.idWindow = window.id;
      }
    }
  );
};

globalThis.makeCardPopupWindow = function makeCardPopupWindow(idCard, pos, cardPopupType) {
  assert(cardPopupType != CARDPOPUPTYPE.NO_POPUP);
  var marginSafety = 20;
  var width = 329;
  var height = 224;
  if (cardPopupType == CARDPOPUPTYPE.POPUP_NOACTIONS) height = 183;
  ...
  function create() {
    const lsprop_shownCardMiniMePopup = "plus_shown_card_mini_me_popup";
    if (!localStorage.getItem(lsprop_shownCardMiniMePopup)) {
      localStorage.setItem(lsprop_shownCardMiniMePopup, true);
      handleShowDesktopNotification({
    ...
    chrome.windows.create(
      {
        url: chrome.runtime.getURL("cardwin.html") + "?idCard=" + idCard + "&cpt=" + cardPopupType,
        width: width,
        height: height,
        type: "popup",
        left: leftPos,
...
globalThis.handleOfflineBoardNotificationClick = function handleOfflineBoardNotificationClick(notificationId) {
  var parts = notificationId.split(":");
  var idBoard = parts[1];
  var urlGo = chrome.runtime.getURL("report.html") + "?groupBy=idCardH&idBoard=" + encodeURIComponent(idBoard);
  window.open(urlGo, "_blank");
  chrome.notifications.clear(notificationId, function (bWasCleared) {});
};

globalThis.handleOfflineCardNotificationClick = function handleOfflineCardNotificationClick(notificationId) {
  var parts = notificationId.split(":");
    ...
    handleGetReport(request, function (responseReport) {
      if (responseReport.status == STATUS_OK && responseReport.rows.length > 0) {
        ...
        var urlGo =
          ...
          "?groupBy=idCardH&idBoard=" +
          encodeURIComponent(idBoard) +
          "&idBoard=" +
          encodeURIComponent(idBoard) +
          "&orderBy=posList&idCardSelect=" +
          encodeURIComponent(idCard);
        if (row.bArchived) urlGo += "&archived=-1";
        if (row.bDeleted) urlGo += "&deleted=-1";
        window.open(urlGo, "_blank");
        chrome.notifications.clear(notificationId, function (bWasCleared) {});
      }
    });
...
globalThis.hookNotificationActions = function hookNotificationActions() {
  if (g_bHookedNotificationsButton) return;
...
globalThis.g_prefixTimerNotification = "timer:";
globalThis.g_prefixOfflineBoardNotification = "openBoardReport:";
globalThis.g_prefixOfflineCardNotification = "openCardReport:";
globalThis.idNotificationForTimer = function idNotificationForTimer(idCard) {
  return g_prefixTimerNotification + idCard;
};

globalThis.idCardFromIdNotification = function idCardFromIdNotification(idNotification) {
...
globalThis.g_intervalTimerNotifications = null;

globalThis.handleTimerNotificationsUpdate = function handleTimerNotificationsUpdate() {
  if (!g_intervalTimerNotifications) g_intervalTimerNotifications = setInterval(updateTimerNotifications, 1000);
  ...
  function updateTimerNotifications() {
    chrome.notifications.getAll(function (notifications) {
      ...
      for (var idNotification in notifications) {
        if (!idNotification || idNotification.indexOf(g_prefixTimerNotification) != 0) continue;
  ...
  function updateTimerNotification(idNotification) {
    ...
    getCardTimerData(hash, function (objTimer) {
      ...
      if (stored === undefined || stored.msStart == null || stored.msEnd != null) {
        ...
        var notification = chrome.notifications.update(
          idNotification,
          {
            title: getTimerElemText(stored.msStart, Date.now(), false, false),
            buttons: [{ title: g_nameMinimizeTimer, iconUrl: chrome.runtime.getURL("images/minimize.png") }],
          },
          function (notificationId) {}
        );
      }
...
globalThis.createChromeNotification = function createChromeNotification(idNotification, obj, callback) {
...
globalThis.showTimerWindowAsNotification = function showTimerWindowAsNotification(idCard, nameCard, nameBoard) {
  hookNotificationActions();
  ...
  var notification = createChromeNotification(
    idNotification,
    {
      type: "basic",
      isClickable: true,
      iconUrl: chrome.runtime.getURL("images/timer-sm-on.png"),
      appIconMaskUrl: chrome.runtime.getURL("images/icon32alpha.png"),
      title: "00:00:00s",
      message: nameCard,
...
globalThis.doShowTimerWindow = function doShowTimerWindow(idCard, callbackParam, ptClick) {
  var data = g_mapTimerWindows[idCard];
  var bCalledCallback = false;

  function callback(status, properties) {
    if (bCalledCallback) return;
    bCalledCallback = true;
    if (callbackParam) callbackParam(status, properties);
  }

  if (data === undefined) create(idCard);
  ...
  function create(idCard) {
    handleOpenDB(null, function (responseOpen) {
      ...
      var request = {
        sql: "SELECT cards.name as nameCard, boards.name as nameBoard FROM cards JOIN boards on cards.idBoard=boards.idBoard WHERE idCard=?",
        values: [idCard],
      };
      handleGetReport(request, function (responseReport) {
        if (responseReport.status != STATUS_OK) {
...
globalThis.g_strLastNotification = "";
globalThis.g_dtLastNotification = null;
globalThis.g_mapNotificationToTimeoutClose = {};

globalThis.handleShowDesktopNotification = function handleShowDesktopNotification(request) {
  var dtNow = new Date();
  var dtDiff = 0;
  ...
  if (request.dontClose) timeout = 0;
    ...
    function (notificationId) {
      if (timeout) {
        ...
        timeoutLast = setTimeout(function () {
          if (g_mapNotificationToTimeoutClose[idUse] === timeoutLast) g_mapNotificationToTimeoutClose[idUse] = null;
...
globalThis.handleCreateSs = function handleCreateSs(sendResponse) {
  var url = null;
  var postData = null;
  if (false) {
    //no longer works with the stricter drive.file permission
    ...
    handleApiCall(
      url,
      { convert: false, ocr: false },
      true,
      function (response) {
        var id = null;
        if (response.data && response.data.id) id = response.data.id;
        if (id == null) response.status = "Unknown error creating spreadsheet";
    ...
    handleApiCall(
      ...
      function (response) {
        var id = null;
        if (response.data && response.data.id) id = response.data.id;
        if (id == null && response.status == STATUS_OK) response.status = "Unknown error creating spreadsheet";

        if (response.status != STATUS_OK) sendResponse({ status: response.status, id: null });
        else handleConfigNewSs(id, gid_to_wid(0), sendResponse);
      },
      JSON.stringify(postData),
      "application/json"
    );
...
globalThis.handleConfigNewSs = function handleConfigNewSs(idSs, wid, sendResponse) {
  //write ss header
  var row = 1;
  var i = 0;
...
globalThis.makeCellBatchEntry = function makeCellBatchEntry(idSs, wid, row, column, value) {
  var feedUrl =
    "https://spreadsheets.google.com/feeds/cells/" + idSs + "/" + wid + "/private/full/R" + row + "C" + column;
  var ret =
    "\
<entry>\
<batch:id>A" +
...
globalThis.handleApiCall = function handleApiCall(
  url,
  params,
  bRetry,
  sendResponse,
  ...
  getExtensionOauthToken(
    "To continue, approve the Google permissions. If not signed-in to Chrome, do so now.",
    ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive.file"],
    function (token) {
      if (token) {
...
globalThis.stringifyParams = function stringifyParams(parameters) {
  var params = [];
...
globalThis.handleGetPlusFeed = function handleGetPlusFeed(msLastPostRetrieved, sendResponse) {
  ...
  var url =
    "https://trello.com/1/lists/588790142d87061b84b7353c/actions?filter=createCard&fields=date&limit=1&memberCreator=false";

  xhr.open("GET", url, true);
  xhr.send();
};

globalThis.g_bRetryAuth = true;

globalThis.onAuthorized = function onAuthorized(
  url,
  params,
  sendResponse,
  oauth,
  ...
  if (contentType === undefined || contentType == null) contentType = "application/atom+xml";
    ...
    if (xhr.readyState == 4) {
      ...
      if (
        ...
        (xhr.statusText.indexOf("Token revoked") == 0 ||
          ...
          xhr.statusText.indexOf("Unauthorized") == 0)
      ) {
        //"Unauthorized" can happen if user removes token from https://accounts.google.com/IssuedAuthSubTokens
        ...
        if (xhr.status < 200 || xhr.status > 207) {
          if (xhr.status == 403) statusRet = "No spreadsheet permission to " + (postBody ? "write." : "read.");
          else statusRet = errFromXhr(xhr);
        ...
        } else sendResponse({ data: data, status: STATUS_OK });
      }
    }
  };
  ...
  if (oauth)
    //oauth not used for reading/writting a public spreadsheet
    xhr.setRequestHeader("Authorization", "Bearer " + oauth);
  if (bAddIfMatchStar) xhr.setRequestHeader("If-Match", "*");
  xhr.send(postBody);
};

globalThis.getCurrentSyncName = function getCurrentSyncName() {
  var name = "";
  if (g_bDisableSync || (g_strServiceUrl == "" && !g_optEnterSEByComment.IsEnabled()))
    ...
    if (g_bStealthSEMode) name = "sync-stealth"; //SYNCMETHOD.googleSheetStealth;
    else name = "sync-legacy"; //SYNCMETHOD.googleSheetLegacy;
  }
  return name;
};

globalThis.handleHitAnalyticsEvent = function handleHitAnalyticsEvent(category, action, bIncludeVersion, bSkipNewbie) {
  if (bSkipNewbie) {
    var msDelta = Date.now() - g_msStartPlusUsage;
    if (msDelta < 1000 * 60 * 60 * 4) return;
  }
  g_analytics.hit(
    { t: "event", ec: category, ea: (bIncludeVersion ? chrome.runtime.getManifest().version + ": " : "") + action },
    1000
  );
};

globalThis.g_analytics = {
  idGlobalAnalytics: "UA-72924905-1",
  idAnalytics: null,
  PROP_IDANALYTICS: "plus_analytics_id",
  PROP_DISABLEANALYTICS: "plus_analytics_disabled",
  bDisableAnalytics: true,
  setDisabled: function (bDisabled) {
    if (bDisabled) localStorage.setItem(this.PROP_DISABLEANALYTICS, "true");
    else delete localStorage.removeItem(this.PROP_DISABLEANALYTICS);
    this.bDisableAnalytics = bDisabled;
  },
  init: function () {
    if (this.idAnalytics) return;

    this.idAnalytics = localStorage.getItem(this.PROP_IDANALYTICS);
    this.bDisableAnalytics = localStorage.getItem(this.PROP_DISABLEANALYTICS) == "true";
    if (!this.idAnalytics) {
      this.idAnalytics = this.generateQuickGuid();
      localStorage.setItem(this.PROP_IDANALYTICS, this.idAnalytics);
    }
  },
  hit: function (params, msDelay) {
    if (isTestVersion()) return;

    if (this.bDisableAnalytics) return;
    ...
    var valCD1Prev = localStorage.getItem(PROP_LS_CD1LAST) || "";
    var valCD1Cur = g_bProVersion ? "Pro" : "Basic";
    if (bAlwaysSendCD || valCD1Prev != valCD1Cur)
      //analytics docs recommend to only send the parameter when it changed, for performance.
      payload = payload + "&cd1=" + valCD1Cur;

    chrome.storage.sync.get(
      [SYNCPROP_NO_SE, SYNCPROP_NO_EST, SYNCPROP_LIDATA, SYNCPROP_LIDATA_STRIPE, keySyncOutsideTrello],
      function (obj) {
        if (chrome.runtime.lastError) {
          console.error(chrome.runtime.lastError.message);
          return;
        ...
        var valCD2Prev = localStorage.getItem(PROP_LS_CD2LAST) ?? "";
        var cViewedBuyDialog = localStorage.getItem(PROP_LS_cViewedBuyDialog) ?? "0";
        var valCD2Cur =
          (liData && liData.li ? "ActiveCS" : liDataStripe && liDataStripe.li ? "ActiveStripe" : "Inactive") +
          "-" +
          cViewedBuyDialog;
        if (bAlwaysSendCD || valCD2Prev != valCD2Cur) payload = payload + "&cd2=" + valCD2Cur;

        var valCD3Prev = localStorage.getItem(PROP_LS_CD3LAST) ?? "";
        var valCD3Cur = getCurrentSyncName();
        if (bAlwaysSendCD || valCD3Prev != valCD3Cur) payload = payload + "&cd3=" + valCD3Cur;

        var valCD4Prev = localStorage.getItem(PROP_LS_CD4LAST) ?? "";
        var valCD4Cur = obj[keySyncOutsideTrello] ? "true" : "false";
        if (bAlwaysSendCD || valCD4Prev != valCD4Cur) payload = payload + "&cd4=" + valCD4Cur;

        var valCD5Prev = localStorage.getItem(PROP_LS_CD5LAST) ?? "";
        var valCD5Cur = g_optEnterSEByComment.getAllKeywordsExceptLegacy().length.toString();
        if (bAlwaysSendCD || valCD5Prev != valCD5Cur) payload = payload + "&cd5=" + valCD5Cur;

        var valCD6Prev = localStorage.getItem(PROP_LS_CD6LAST) ?? "";
        var valCD6Cur = UNITS.current;
        if (bAlwaysSendCD || valCD6Prev != valCD6Cur) payload = payload + "&cd6=" + valCD6Cur;

        var valCD7Prev = localStorage.getItem(PROP_LS_CD7LAST) ?? "";
        var valCD7Cur = obj[SYNCPROP_NO_SE] ? "true" : "false";
        if (bAlwaysSendCD || valCD7Prev != valCD7Cur) payload = payload + "&cd7=" + valCD7Cur;

        var valCD8Prev = localStorage.getItem(PROP_LS_CD8LAST) ?? "";
        var valCD8Cur = obj[SYNCPROP_NO_EST] ? "true" : "false";
        if (bAlwaysSendCD || valCD8Prev != valCD8Cur) payload = payload + "&cd8=" + valCD8Cur;
        ...
        setTimeout(function () {
          ...
          xhr.onreadystatechange = function (e) {
            if (xhr.readyState == 4 && xhr.status == 200) {
              if (valCD1Prev != valCD1Cur) localStorage.setItem(PROP_LS_CD1LAST, valCD1Cur);

              if (valCD2Prev != valCD2Cur) localStorage.setItem(PROP_LS_CD2LAST, valCD2Cur);

              if (valCD3Prev != valCD3Cur) localStorage.setItem(PROP_LS_CD3LAST, valCD3Cur);

              if (valCD4Prev != valCD4Cur) localStorage.setItem(PROP_LS_CD4LAST, valCD4Cur);

              if (valCD5Prev != valCD5Cur) localStorage.setItem(PROP_LS_CD5LAST, valCD5Cur);

              if (valCD6Prev != valCD6Cur) localStorage.setItem(PROP_LS_CD6LAST, valCD6Cur);
            }
          };
...
globalThis.g_bInstalledHeadersRequest = false;
globalThis.g_bInjectedTrelloFrame = false;

globalThis.handleHeadersReceived = function handleHeadersReceived(info) {
...
globalThis.uninjectTrelloFrame = function uninjectTrelloFrame() {
...
globalThis.injectTrelloFrame = function injectTrelloFrame() {
  if (!g_bInstalledHeadersRequest) {
...
globalThis.g_mapCardToMiniPopupWindow = {};
globalThis.g_mapCardToTab = {}; //we keep this map to know which tab has which card currently open. This method works without needing to add the "tabs" permission to the extension.
globalThis.g_mapBoardToTab = {}; //we keep this map to know which tab has which card currently open. This method works without needing to add the "tabs" permission to the extension.

globalThis.handlenotifyCardTab = function handlenotifyCardTab(idCard, tabid) {
  g_mapCardToTab[idCard] = { tabid: tabid, ms: Date.now() };
};

globalThis.handlenotifyBoardTab = function handlenotifyBoardTab(idBoard, tabid) {
 ...
 **/
globalThis.handleCheckLi = function handleCheckLi(sendResponse) {
  ...
  getExtensionOauthToken(
    "To continue, approve the Chrome Web Store permissions. If not signed-in to Chrome, do so now.",
    ...
    function (token) {
      if (token) {
        ...
        var message = "";
        if (chrome.runtime.lastError) message = chrome.runtime.lastError.message;
        if (!message) message = "Not signed-in to Chrome, or no permission for the Chrome webstore.";
        handleShowDesktopNotification({
  ...
  function checkPurchased(bJustPurchased, strErrorCustom, licCustom, onError) {
    ...
    function onLicenseUpdateFail(data) {
      ...
      if (onError) onError();
        ...
        if (data && data.response && data.response.errorType == "TOKEN_MISSING_ERROR")
    ...
    function onLicenseUpdate(data) {
      ...
      chrome.storage.sync.set(objNew, function () {
        //ok if fails
        if (BLastErrorDetected()) console.error(chrome.runtime.lastError.message);
        if (!liData.li) {
          if (bJustPurchased) respond(strErrorCustom || "Purchase failed");
          else doPurchase();
        } else {
  ...
  function doPurchase() {
    var cViewed = localStorage.getItem(PROP_LS_cViewedBuyDialog);
    if (cViewed) cViewed = parseInt(cViewed, 10) || 0;
    else cViewed = 0;
    cViewed++;
    localStorage.setItem(PROP_LS_cViewedBuyDialog, cViewed.toString());
    ...
    function onPurchaseFail(data) {
      ...
      var licCustom = null;
      if (data && data.response && data.response.errorType) strError = data.response.errorType;
      ...
      if (data && data.checkoutOrderId) licCustom = JSON.stringify(data);
      else {
...
globalThis.handleReloadExtension = function handleReloadExtension() {
  setTimeout(function () {
...
globalThis.getExtensionOauthToken = function getExtensionOauthToken(strNotification, scopes, callback) {
  var idWait = null;
  var bWaiting = true;
  setTimeout(function () {
    if (!bWaiting) return;
  ...
  chrome.identity.getAuthToken({ interactive: true, scopes: scopes }, function (token) {
    ...
    bWaiting = false;
    if (idWait) chrome.notifications.clear(idWait, function (wasCleared) {});
