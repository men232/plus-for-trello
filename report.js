...
const PROP_LS_bShowedDefaultCPMCountsListFilter = "plus_showed_default_cpm_counts_list_filter";
const CF_PREFIX = "cf:";

const g_chartViews = {
  //do not modify existing options, as those could be in saved user's bookmarks
  s: "s",
  e: "e",
  r: "r",
  ser: "ser",
...
var g_colours = {
  //thanks http://stackoverflow.com/a/1573141/2213940
  aliceblue: "#f0f8ff",
  antiquewhite: "#faebd7",
  aqua: "#00ffff",
  aquamarine: "#7fffd4",
...
var g_dataFormatUser = {
  key: KEY_FORMAT_PIVOT_USER,
  interval: null,
  cLastWrite: 0,
  cCur: 0,
  format: { u: { c: g_colorDefaultUnder, v: null }, o: { c: g_colorDefaultOver, v: null } },
};
var g_dataFormatBoard = {
  key: KEY_FORMAT_PIVOT_BOARD,
  interval: null,
  cLastWrite: 0,
  cCur: 0,
  format: { u: { c: g_colorDefaultUnder, v: null }, o: { c: g_colorDefaultOver, v: null } },
...
function getCleanHeaderName(name) {
  ...
  if (iLast > 0) ret = ret.substr(0, iLast);
...
function buildUrlFromParams(params, bNoPopupMode, prefixCustom) {
  if (bNoPopupMode) {
    ...
    if (params["popup"] === undefined && g_bPopupMode) params["popup"] = "1";
  }
...
function updateNamedReport(url) {
  if (g_namedReport) localStorage.setItem(g_namedParams.namedReport + ":" + g_namedReport, url);
}
...
window.onerror = function (msg, url, lineNo, columnNo, error) {
  ...
  if (string.indexOf(substring) > -1) {
    ...
    message = ["Message: " + msg, "URL: " + url, "Line: " + lineNo, "Error object: " + JSON.stringify(error)].join(
      " - "
    );
  }
  console.log(message);
...
function updateUrlState(params) {
  if (g_namedReport) params[g_namedParams.namedReport] = g_namedReport;
  if (g_sortListNamed) params[g_namedParams.sortListNamed] = JSON.stringify(g_sortListNamed);
...
function loadStorageGlobals(callback) {
  ...
  chrome.storage.sync.get(
    [
      SYNCPROP_NO_SE,
      SYNCPROP_NO_EST,
      KEY_bIgnoreZeroECards,
      KEY_FORMAT_PIVOT_USER,
      KEY_FORMAT_PIVOT_BOARD,
      KEY_bEnableTrelloSync,
      keybEnterSEByCardComments,
      keyrgKeywordsforSECardComment,
    ],
    function (objs) {
      if (objs[KEY_FORMAT_PIVOT_USER] !== undefined) g_dataFormatUser.format = objs[KEY_FORMAT_PIVOT_USER];
      if (objs[KEY_FORMAT_PIVOT_BOARD] !== undefined) g_dataFormatBoard.format = objs[KEY_FORMAT_PIVOT_BOARD];
...
window.addEventListener("resize", function () {
  if (g_iTabCur != null) selectTab(g_iTabCur, undefined, true);

  if (g_iTabCur == ITAB_CHART) {
    if (g_chartContainer) g_chartContainer.redraw();
  }
  if (g_bBuildSqlMode) doResizeFromParent();
});
...
function selectTab(iTab, href, bForce) {
  ...
  function postSelect() {
    if (g_iTabCur == ITAB_CHART) setTimeout(fillChart, 100); //wait for final layout
  }
  ...
  postSelect();
  if (params["tab"] != iTab) {
    if (params["tab"] || iTab != 0) {
      //not just an optimization. Print (ctrl+print) causes a resize. updating the url causes the print dialog to go away in windows chrome.
      params["tab"] = iTab;
...
function selectTabUI(iTab, href) {
  if (g_bBuildSqlMode) return iTab;
  var i = 0;
  var selector = null;
  ...
  for (; i < g_rgTabs.length; i++) {
    ...
    if (i == iTab || (href && href == cur)) {
      ...
      if (elemsHide) elemsHide = elemsHide.add($(cur));
      else elemsHide = $(cur);
    }
  }
  if (selector) {
    ...
    function fixScroller() {
      if (elemsHide) elemsHide.hide();
      var heightWindow = window.innerHeight;
      elem.show();
      if (g_bPopupMode && heightWindow > 470) heightWindow = 470; //prevent weird animations in popup when we exceed the original height in calculations
...
function findMatchingKeywords(term, autoResponse) {
  ...
  if (g_optEnterSEByComment.IsEnabled()) rg = g_optEnterSEByComment.rgKeywords;

  autoResponse(
    term == ""
      ? rg
      : rg.filter(function (item) {
...
function findMatchingLabels(term, autoResponse) {
  ...
  if (idBoard.length > 0) {
    ...
    sql =
      "SELECT distinct(labels.name) FROM labels join boards on labels.idBoardShort=boards.idBoard where boards.name LIKE ?";
    cWhere++;
  ...
  if (term != "") {
    ...
    if (g_bDummyLabel) sql = sql + " AND labels.idLabel<>'" + IDLABEL_DUMMY + "'";
    params.push("%" + term + "%");
  }
...
function findMatchingUsers(term, autoResponse) {
  ...
  getSQLReport(sql, params, function (response) {
    ...
    console.warn("[findMatchingUsers]", response);

    var ret = rows.map((v) => v.user);
...
function setupNotifications() {
  if (g_portBackground != null) return;
...
function updateURLPart(part) {
  ...
  var elem = $("#" + part);
  var val = elem.val();
  if (elem[0].type == "checkbox") val = elem[0].checked ? "true" : "";
  if (params[part] != val) {
    params[part] = val;
    updateUrlState(params);
  }
...
function createReportBookmark() {
  ...
  chrome.storage.sync.get([SYNCPROP_idFolderParentBookmark], function (objs) {
    ...
    if (idFolderPlus) {
      chrome.bookmarks.get(idFolderPlus, function (bookmarks) {
        if (bookmarks && bookmarks.length == 1) saveBookmark(bookmarks[0].id);
        else stepFindFolder();
      });
    } else {
      stepFindFolder();
  ...
  function stepCreateFolder() {
    ...
    chrome.bookmarks.create({ parentId: idChromeBookmarksBar, title: nameFolderParent }, function (bookmark) {
      if (bookmark) stepSaveFolderAndBookmark(bookmark.id);
    });
  }
  ...
  function saveBookmark(idParent) {
    ...
    chrome.bookmarks.create({ parentId: idParent, title: title, url: url }, function (bookmark) {
      if (!bookmark) alert("Error saving bookmark :(");
      else {
...
function updateOutputFormat(format) {
  ...
  if (!format || format == "csv")
    msg =
      "Can cause <A href='http://stackoverflow.com/questions/4438589' target='_blank'>issues with special characters</A> when opening in Excel.";
  else
    msg =
      "Note about the <A href='https://support.microsoft.com/en-us/help/948615' target='_blank'>warning in Excel</A>.";
...
function loadAll() {
  ...
  $("#selectAllColumns").bsmSelect({
    ...
    plugins: [$.bsmSelect.plugins.sortable({ axis: "y", opacity: 0.5 }, {}), $.bsmSelect.plugins.compatibility()],
  });
  ...
  $("#checkOutputReport").change(function () {
    ...
    if (this.checked) elemOutputFormatHelp.show();
    else elemOutputFormatHelp.hide();
  });
  ...
  if (!g_bBuildSqlMode) hitAnalytics("Reports", "open-" + (g_bPopupMode ? "inPopup" : "window"), true);

  if (namedReport) {
    g_namedReport = namedReport;
    if (params["useStoredNamed"]) {
      var urlNew = localStorage.getItem(g_namedParams.namedReport + ":" + namedReport);
      if (urlNew) {
  ...
  if (!g_bPopupMode) $("body").removeClass("agile_report_minSize");
  else $(".agile_report_filters").addClass("agile_report_filters_smaller");
  ...
  loadTabs($("#tabs"));
  if (g_bNoSE) $(".agile_tab_pivot").hide();

  if (bDisableSort) $("#orderBy").prop("disabled", true);
  ...
  if (g_bPopupMode) {
    ...
    dockOut.attr("src", chrome.runtime.getURL("images/dockout.png"));
    dockOut.show();
    dockOut.css("cursor", "pointer");

    var back = $("#backImg");
    back.attr("src", chrome.runtime.getURL("images/back.png"));
    back.show();
  ...
  openPlusDb(function (response) {
    ...
    if (!g_bBuildSqlMode) setupNotifications();

    $("#agile_reload_page_link")
      .off()
      .click(function (e) {
        e.preventDefault();
        var params = getUrlParams();
        if (g_bAddParamSetLastRowViewedToQuery) params["setLastRowViewed"] = "true";
    ...
    elemGroupBy.change(function () {
      ...
      function handleCustom(valDefault) {
        ...
        if (valGroup == "custom") {
          ...
          var strGroup = window.prompt(
            "Enter groups separated by '-'\n\
...
Pick from:\n\
Team - Board - List - Card - Hashtag1 - Hashtags - Labels - User - Note - Keyword - Date - Month - Week",
            valDefault
          );
          if (!strGroup) elemGroupBy.val(g_lastGroupSelection);
          else {
            ...
            for (var iGroup = 0; iGroup < groups.length; iGroup++) {
              ...
              if (mapped) groups[iGroup] = mapped;
              else {
    ...
    $("#heightZoomChart").change(function () {
      ...
      if (g_dataChart) g_dataChart.params["heightZoomChart"] = parseInt($("#heightZoomChart").val(), 10) || 0;
      fillChart(true);
    });
    ...
    $("#colorChartBackground").change(function () {
      ...
      if (g_dataChart) g_dataChart.params["colorChartBackground"] = $("#colorChartBackground").val();
    ...
    function addFocusHandler(elem) {
      ...
      elem.off("focus.plusForTrello").on("focus.plusForTrello", function () {
        if (this.value == "" || this.value == specialAll) $(this).autocomplete("search", specialAll);
      });
    }
...
function configChartTab() {
  ...
  if (copyWindow.length > 0) {
    copyWindow.attr("src", chrome.runtime.getURL("images/copy.png"));
    ...
    copyWindow.off().click(function () {
      ...
      if (!domain) return;
      if (bCancelFromAlertLargeSize(domain, true)) return;
...
function configPivotFormat(elemFormat, dataFormat, tableContainer, iTab) {
  ...
  function applyFormat(bFirstTime) {
    if (bFirstTime)
      applyFormatWorker(
        bFirstTime
      ); //review zig: should be ok in setTimeout but here to reduce risk of making this change.
    else
      setTimeout(function () {
        applyFormatWorker(bFirstTime);
      }, 10);
  }
  ...
  function applyFormatWorker(bFirstTime) {
    ...
    if (bFirstTime && (bNoFormat || (valUnder === null && valOver === null))) return; //performance
    ...
    function workerCells() {
      ...
      if (cells === undefined) {
        ...
        if (!bFirstTime) g_cacheCells[dataFormat.key] = cells; //cache when called from format change so its fast as the user changes values
      }
      ...
      cells.each(function () {
        ...
        } else if (valUnder == null && valOver == null) color = colorNormal;
        else if (valUnder != null && val < valUnder) {
          color = colorUnder;
          bUsedUnder = true;
        } else if (valOver != null && val > valOver) color = colorOver;
        else if (!bStrictFormat && (valUnder != null || valOver != null)) {
          //in between
          var distance = 0;
          if (valUnder != null && valOver != null) distance = valOver - valUnder;
          else if (valUnder != null) distance = valUnder;
          else distance = valOver;
          distance = distance / 4;
        ...
        if (el.hasClass("agile_pivotCell_Zero"))
          colorText =
            color; //prevent filling report with zeros which clutter it. value is there but with color equal to background
        else colorText = colorContrastWith(color, rgb, g_colorTrelloBlack);
...
function savePivotFormat(dataFormat, colorUnder, colorOver, valUnder, valOver, formatType) {
  ...
  function saveToSync(bNow) {
    ...
    setTimeout(function () {
      ...
      if (currentFormat != lastFormat) return;
      var pair = {};
      var cCur = dataFormat.cCur; //separate from global format
      pair[dataFormat.key] = dataFormat.format;
      chrome.storage.sync.set(pair, function () {
        if (chrome.runtime.lastError === undefined) dataFormat.cLastWrite = Math.max(dataFormat.cLastWrite, cCur);
        else g_cSyncSleep = 5; //will sleep next x cicles
      });
    }, wait);
  }
  ...
  if (before != after) {
    ...
    if (dataFormat.interval == null) {
      ...
      dataFormat.interval = setInterval(function () {
        if (dataFormat.cCur != dataFormat.cLastWrite) saveToSync(false);
...
function remapGroupByToDisplay(str, bLoose) {
  ...
  for (var iGroup = 0; iGroup < groups.length; iGroup++) {
    ...
    if (!bMapped) {
      if (bLoose) continue;
      else return ""; //means error REVIEW zig no caller checking this
    }
  }
  return groups.join("-");
}
...
function getParamAndPutInFilter(elem, params, name, valDefault) {
  ...
  if (!bShowHide) str = valDefault;
  if (value && value != "") str = decodeURIComponent(value);
  if (name.indexOf("check") == 0) elem[0].checked = str == "true";
  else {
    elem.val(str);
    if (name == "groupBy") g_lastGroupSelection = str;

    if (elem.val() != str) {
      //allow user to type a random filter from the url
      if (elem.is("select")) {
        var strDisplay = str;
        if (name == "groupBy") strDisplay = remapGroupByToDisplay(str);
...
function loadReport(params) {
  ...
  if (params.setLastRowViewed == "true") g_bNeedSetLastRowViewed = true;
  else g_bNeedSetLastRowViewed = false;
  ...
  if (g_bProVersion) $("#labelOptionsProOnly").hide();

  var editLabels = $("#label");
  var editKeyword = $("#keyword");
  g_bShowKeywordFilter =
    g_optEnterSEByComment.IsEnabled() && g_optEnterSEByComment.getAllKeywordsExceptLegacy().length > 1;

  if (params["checkNoColorsChartCount"])
    //old property
    params["checkNoColorsChart"] = params["checkNoColorsChartCount"]; //migrate
  if (params["stackCount"])
    //old property
    params["stackBy"] = params["stackCount"]; //migrate

  if (
    params["sinceSimple"] &&
    params["sinceSimple"].toLowerCase() == FILTER_DATE_ADVANCED.toLowerCase() &&
    !params["weekStart"] &&
    !params["weekEnd"] &&
    !params["monthStart"] &&
    !params["monthEnd"]
  ...
  var elems = {
    stackBy: "",
    checkNoColorsChart: "false",
    checkBGColorChart: "false",
    colorChartBackground: "#FFFFFF",
    chartView: g_chartViews.cardcount,
    keyword: "showhide",
    groupBy: "",
    pivotBy: "",
    orderBy: "date",
    showZeroR: "",
    sinceSimple: sinceSimple,
    weekStart: "",
    weekEnd: "",
    monthStart: "",
    monthEnd: "",
    ...
    afterRow: "showhide",
    checkNoCharts: "false",
    checkAddCustomFields: "false",
    checkAddMembers: "false",
    checkNoLabelColors: "false",
    checkNoBracketNotes: false,
    checkOutputCardShortLink: "false",
    checkOutputBoardShortLink: "false",
    checkOutputReport: "false",
    outputFormat: "csv",
    checkOutputCardIdShort: "false",
    checkHideAnnotationTexts: "false",
    checkHideZoomArea: false,
    checkSyncBeforeQuery: "false",
    checkNoPartialE: "false",
    checkNoSystemNumberFormat: "false",
  };

  if (params["groupBy"] == "custom") params["groupBy"] = ""; //too late here

  for (var iobj in elems) {
    var elemCur = $("#" + iobj);
    elemCur.off("keypress.plusForTrello").on("keypress.plusForTrello", function (event) {
      if (g_bIgnoreEnter) return;
    ...
    if ((iobj == "idBoard" || iobj == "idCard") && params[iobj]) hiliteOnce(elemCur);
    //localStorage.setItem(PROP_LS_bShowedDefaultCPMCountsListFilter, "");
    if (
      g_namedReport == "_counts" &&
      iobj == "list" &&
      !localStorage.getItem(PROP_LS_bShowedDefaultCPMCountsListFilter) &&
      params[iobj] == "doing"
    ) {
      localStorage.setItem(PROP_LS_bShowedDefaultCPMCountsListFilter, "true");
  ...
  var elemOutputFormatHelp = $("#noteExportFormat");
  if (params["checkOutputReport"] == "true") elemOutputFormatHelp.show();
  else elemOutputFormatHelp.hide();
  updateOutputFormat(params["outputFormat"]);
  if (g_bShowKeywordFilter) editKeyword.parent().show();
  else {
    editKeyword.parent().hide();
    if ($("#orderBy").val().toLowerCase().indexOf("keyword") < 0) $("#orderBy option[value*='keyword']").remove();
    if ($("#groupBy").val().toLowerCase().indexOf("keyword") < 0) $("#groupBy option[value*='keyword']").remove();
  }
  ...
  $("#dockoutImg")
    .off()
    .click(function () {
      //cant use setPopupClickHandler because url could have changed if user navigated inside
    ...
    .click(function () {
      chrome.permissions.request(
        ...
        function (granted) {
          ...
          onQuery(false, true, function (status) {
            if (status == STATUS_OK) createReportBookmark();
          });
        }
      );
    });
  ...
  function onQueryWorker(bFirstTime, bOnlyUpdateUrl, callbackQuery) {
    if (bFirstTime && g_bBuildSqlMode) bFirstTime = false;

    g_cacheCells = {}; //clear cache
    if (false) {
      //review zig: figure out how to make this work.
      var iForms = 0;
      var forms = $("form");
      function handleFormsSubmit(iform, forms) {
        setTimeout(function () {
          document.forms[forms[iform].name].submit();
          if (iform + 1 < forms.length) handleFormsSubmit(iform + 1, forms);
        }, 100);
      }
    ...
    if (!bOnlyUpdateUrl) {
      ...
      if (bFirstTime) btn.text("•••");
    }

    for (var iobj in elems) {
      if (iobj == "tab") continue;
      var elemCur = $("#" + iobj);
      if (iobj.indexOf("check") == 0)
        ...
        if (iobj == "sinceSimple" && elems[iobj] != FILTER_DATE_ADVANCED) groupDateAdvanced.find("input").val(""); //review: implement a "postGet" event defined per field so each field handles this
      }
    }
  ...
  if (g_bBuildSqlMode) containerOptions.hide();
  else containerOptions.show();

  if (g_bBuildSqlMode || g_bPopupMode) containerTemplates.hide();
  else containerTemplates.hide();
  ...
  if (!g_bBuildSqlMode && Object.keys(params).length > 0 && !bDontQuery) {
    //dont execute query automatically
    if (g_bPopupMode) onQuery(true);
    else
      setTimeout(function () {
        onQuery(true);
      }, 10);
    ...
    if (bFromMarkAllViewed) {
      $("#reportBottomMessage")
        .show()
        .html(
          "s/e rows marked viewed. Query a new report or <button id='report-close-window'>close this window</button>"
        );
      $("#report-close-window")
    ...
    if (g_bBuildSqlMode) onQuery(true);
    else hiliteOnce($("#buttonFilter"));
  }
}
...
function completeString(str, pattern) {
  ...
  while (str.length < c) str = str + pattern.charAt(str.length);
  return str;
}
...
function buildSqlParam(param, params, table, sqlField, operator, state, completerPattern, btoUpper, advancedParams) {
  ...
  if (btoUpper === undefined) btoUpper = true;
  var val = params[param];
  if (val == "") return "";

  var bString = typeof val == "string";
  if (completerPattern) val = completeString(val, completerPattern);
  var sql = "";
  var parts = null;
  if (bString) {
    val = val.trim();
    if (btoUpper) val = val.toUpperCase();
  }

  //review zig: need more generic way to interpret parameters without hardcoding all here
  if (param == "eType") val = g_mapETypeParam[val];

  if (param == "sinceSimple") {
    parts = val.split("-");
    if (parts.length < 2) return ""; //ignore if value is not in tuple format. caller deals with those (advanced, all, etc)
    var now = new Date();
    now.setHours(0, 0, 0, 0);
    var delta = (parseInt(parts[1], 10) || 0) - 1;
    if (parts[0] == "W") delta = delta * 7 + DowMapper.posWeekFromDow(now.getDay());
  ...
  if (param == "monthStart" || param == "monthEnd") {
    ...
    if (parts.length == 3) {
      ...
      if (yearParsed > 1900 && monthParsed > 0 && dayParsed > 0) {
        ...
        if (param == "monthEnd") dateParsed.setHours(23, 59, 59, 999);
        sqlField = "date";
  ...
  var valElems = [val];
  if (bAllowOrAnd) {
    if (val.indexOf(" AND ") > 0) opOrAnd = " AND ";
    else if (val.indexOf(" OR ") > 0) opOrAnd = " OR ";
  ...
  valElems.forEach(function (val) {
    ...
    if (bAllowOrAnd && val.charAt(0) == "!") {
      ...
      if (advancedParams.bLabelMode) {
        ...
        if (opOrAnd) opOrAnd = " OR "; //apply !A AND !B AND !C --> !(A OR B OR C)
      }
    }

    if (bString && btoUpper) sql += "UPPER(" + table + sqlField + ") " + opNot + operator + " ?";
    else sql += table + sqlField + " " + operator + " ?";

    if (bMultiple && cProcessed != valElems.length) sql = sql + opOrAnd;
    if (!advancedParams.bNoWhereAnd) cFiltersAdd++;
  ...
  if (advancedParams.bLabelMode) {
    if (opOrAnd == " AND ") {
      ...
      (cNegated > 0 && opOrAndOrig == " OR ") || //no negation with OR
      (advancedParams.bNegateAll && cNegated != valElems.length)
    ) {
      //when negating, negate all
      advancedParams.errorParse =
        "When using ! (negation) with multiple terms, all terms must be negated with AND, as in: !a AND !b AND !c";
    }
  }

  if (bMultiple) sql += ")";

  if (advancedParams.errorParse) sql = "";
  else {
...
function buildSql(elems) {
  ...
  function buildAllParams(state, bTable, bExcludeWeekFilter, bOnlyWithDueDates) {
    ...
    if (elems["label"]) {
      if (g_bProVersion) {
        ...
        var sqlLabels =
          "SELECT LC.idCardShort FROM LABELCARD as LC JOIN LABELS as L on LC.idLabel=L.idLabel WHERE " +
          buildSqlParam("label", elems, "", "L.name", "LIKE", state, undefined, undefined, advancedParams);
        if (advancedParams.errorParse) {
          if (cErrors == 0)
            sendDesktopNotification(
              "Error: unsupported label filter. Please hover the labels filter for help: " + advancedParams.errorParse,
              12000
            );
          cErrors++;
        } else {
          sql +=
            (state.cFilters > 0 ? " AND" : " WHERE") +
            " C.idCard" +
            (advancedParams.bNegateAll ? " NOT" : "") +
            " in (" +
            sqlLabels +
            ")";
        }
  ...
  var bHasUnion = false;
  var sql =
    "select H.rowid as rowid, H.keyword as keyword, H.user as user, H.week as week, H.month as month, H.spent as spent, H.est as est, \
                CASE WHEN (H.eType=" +
    ETYPE_NEW +
    ") then H.est else 0 end as estFirst, \
  ...
  if (
    ...
    sql +=
      " UNION ALL \
                select " +
      ROWID_REPORT_CARD +
      " as rowid, '' as keyword, '' as user, '' as week, " +
      (g_bBuildSqlMode
        ? "strftime('%Y',datetime(dateDue, 'unixepoch'))||'-'||strftime('%m',datetime(dateDue, 'unixepoch'))"
        : "case when C.dateSzLastTrello is null then '' else substr(C.dateSzLastTrello,0,8) end") +
      " as month, 0 as spent, 0 as est, \
                0 as estFirst, " +
      (g_bBuildSqlMode
        ? "C.dateDue"
        : "CASE when C.dateSzLastTrello is null then 0 else cast(strftime('%s',C.dateSzLastTrello) as INTEGER) end") +
      " as date " +
      ", '' as comment, C.idCard as idCardH, C.idBoard as idBoardH, \
                T.idTeam as idTeamH, T.name as nameTeam,T.nameShort as nameTeamShort, L.name as nameList, L.pos as posList, C.name as nameCard, C.idShort as idShort, B.name as nameBoard, " +
      ETYPE_NONE +
      " as eType, \
                CASE WHEN (C.bArchived+B.bArchived+L.bArchived)>0 then 1 else 0 end as bArchivedCB, C.bDeleted as bDeleted, C.dateDue as dateDue, C.dateCreated as dateCreated \
                FROM CARDS as C \
...
var g_progress = {
  ...
  show: function (bShow) {
    if (bShow) this.m_container.show();
    else this.m_container.hide();
  },
  text: function (text) {
...
function configReport(elemsParam, bRefreshPage, bOnlyUrl, callbackParam) {
  ...
  if (customColumns.length == 1 && customColumns[0] == "") customColumns = [];
  if (!bIncludeMembers && customColumns.indexOf("members") >= 0) bIncludeMembers = true;
  var bCalledBackMain = false;
  function callbackMain(status) {
    if (bCalledBackMain) return; //prevent double callback (on error cases maybe callback throws)
    bCalledBackMain = true;
    if (callbackParam) callbackParam(status);
  }
  ...
  if (!g_bProVersion) {
    if (bSyncBeforeQuery || elems.customColumns || bIncludeCustomFields || bIncludeMembers) {
      ...
      if (!bOnlyUrl) sendDesktopNotification("To use 'Pro' report options, enable 'Pro' from the Plus help pane", 7000);
    }
  }

  if (bIncludeMembers && groupBy.indexOf("idCardH") < 0) {
    bIncludeMembers = false;
    if (!bOnlyUrl) sendDesktopNotification("To show card members, include a group by 'card'.", 7000);
  }

  //Compact the url for easier reading by removing common defaults

  if (elems["eType"] == "all")
    //do this before updateUrlState so it doesnt include this default in the url REVIEW zig change so its elem value IS "" (see sinceDate)
    elems["eType"] = ""; //this prevents growing the URL with the default value for eType

  if (elems["deleted"] === "") elems["deleted"] = "0"; //default to "Not deleted"

  if (elems["archived"] === "") elems["archived"] = "0"; //default to "Not archived"

  var rgelemsFalse = [
    "checkAddCustomFields",
    "checkAddMembers",
    "checkNoCrop",
    "checkBGColorChart",
    "checkNoCharts",
    ...
    "checkNoLabelColors",
    "checkNoPartialE",
    "checkSyncBeforeQuery",
    "checkOutputCardShortLink",
    "checkOutputBoardShortLink",
    "checkOutputCSV",
    "checkOutputCardIdShort",
    "checkNoSystemNumberFormat",
    "checkHideAnnotationTexts",
    "checkHideZoomArea",
    "checkNoBracketNotes",
  ];
  ...
  rgelemsFalse.forEach(function (elem) {
    var obj = elems[elem];
    if (obj === "false") elems[elem] = "";
  });

  if (elems["checkOutputReport"] === "false") {
    elems["checkOutputReport"] = "";
    if (elems["outputFormat"] === "csv") elems["outputFormat"] = "";
  }
  ...
  if (!g_bBuildSqlMode) {
    ...
    if (g_orderByWhenSortList && g_orderByWhenSortList != elems["orderBy"] && g_sortListNamed) g_sortListNamed = null;
  ...
  if (!g_bBuildSqlMode) setBusy(true);
  if (bRefreshPage) {
    assert(!g_bBuildSqlMode);
  ...
  if (g_bPopupMode) {
    var strTitleReport;
    if (orderBy == "remain") strTitleReport = "Remain";
    else strTitleReport = "Spent";
  ...
  openPlusDb(function (response) {
    ...
    if (bSyncBeforeQuery) {
      ...
      sendExtensionMessage({ method: "plusMenuSync" }, function (response) {
        if (response.status != STATUS_OK) alert(response.status);
        g_progress.text("");
        doSQLReportPart();
      });
    } else {
    ...
    function doSQLReportPart() {
      ...
      getSQLReport(sqlQuery.sql, sqlQuery.values, function (response) {
        g_progress.text("");
        if (response.status != STATUS_OK) {
          showError(response.status);
          callbackMain(response.status);
              ...
              getSQLReport(sqlCheckBoard, [elems["idBoard"]], function (response) {
                ...
                if (response.rows.length > 0) {
                  ...
                  if (
                    ...
                    sendDesktopNotification(
                      "Make sure you are a direct member of this board (not just a Trello team member) to view this report.",
                      10000
                    );
                  }
                }
          ...
          setReportData(rows, options, elems, sqlQuery, function onOK() {
            ...
            if (options.bCheckOutputCSV || options.bCheckOutputXLS) {
              setTimeout(function () {
                ...
                if (options.bCheckOutputCSV) ExcellentExport.csv(elem, elem);
                else ExcellentExport.excel(elem, elem);
              }, 1000);
            }
            callbackMain(STATUS_OK);
          });
...
function transformAndMarkSkipCardRows(rows, transformRow) {
  ...
  for (var i = 0; i < cLength; i++) {
    row = rows[i];
    if (row.bSkip) continue;
    if (transformRow) transformRow(row);
    if (row.bSkip) continue;
    assert(row.idCardH);
    if (row.rowid !== ROWID_REPORT_CARD) {
      if (!mapIdCards[row.idCardH]) mapIdCards[row.idCardH] = {};
  ...
  for (idCard in mapIdCards) {
    ...
    if (item.bSE && item.i !== undefined) rows[item.i].bSkip = true;
  }
}
...
function splitRowsBy(rows, colName, mapCardsToLabels) {
  ...
  if (bHashtags) nameField = "hashtagFirst";
  else nameField = "labels";

  for (var i = 0; i < cItems; i++) {
    row = rows[i];
    if (row.bSkip) continue;
    assert(row[nameField] === undefined);
    if (bHashtags) rgItems = getHashtagsFromTitle(row.nameCard || "", false);
    else rgItems = mapCardsToLabels ? (mapCardsToLabels[row.idCardH] || { rgNameLabels: [] }).rgNameLabels : []; //mapCardsToLabels can be null if !pro
    if (rgItems.length == 0) row[nameField] = "";
    else {
      for (iSplit = 0; iSplit < rgItems.length; iSplit++) {
        if (iSplit == 0) row[nameField] = rgItems[0];
        else {
  ...
  getSQLReport(sql, paramsSql, function (response) {
    ...
    getSQLReport(sql, [], function (response) {
      ...
      response.rows.forEach(function (rowLabel) {
        ...
        if (!mapColorFromName[name]) mapColorFromName[name] = color;
      });
      ...
      for (var idCardLoop in mapCardsToLabels) {
        ...
        if (options.bNoLabelColors) {
          ...
          rgLabels.forEach(function (label) {
            ...
            rgLabelsDecorated[iLabel] =
              '<span class="' +
              strClassText +
              'report_label_color" style="background-color:' +
              mapLabelColors[label.idLabel] +
              ';">' +
              mapLabelNames[label.idLabel] +
              "</span>";
            iLabel++;
          });
...
function skipRowsWithPartialE(rows, bOrderAsc) {
  ...
  function processRow(row) {
    ...
    dateLast = row.date;
    if (row.bSkip || row.rowid === ROWID_REPORT_CARD) return;
...
function setReportData(rowsOrig, options, urlParams, sqlQuery, callbackOK) {
  ...
  if (sqlQuery.bHasUnion) {
    ...
    var tdata = null;
    if (weekStart || weekEnd) tdata = { weekStart: weekStart, weekEnd: weekEnd };
    transformAndMarkSkipCardRows(rowsOrig, function (row) {
      ...
      if (!week) {
        ...
        row.week = week;
        if (tdata) {
          if ((weekStart && week < weekStart) || (weekEnd && week > weekEnd)) row.bSkip = true;
        }
      }
    });
  }
  ...
  if (options.bExcludeCardsWithPartialE) skipRowsWithPartialE(rowsOrig, sqlQuery.bOrderAsc);

  var bShowCard = groupBy == "" || groupBy.indexOf("idCardH") >= 0 || groupBy.indexOf("labels") >= 0; //review zig: dup elsewhere
  var bShowLabels =
    bShowCard && g_bProVersion && (options.customColumns.length == 0 || options.customColumns.indexOf("labels") >= 0);
  ...
  function stepExtraFields(callback) {
    ...
    for (i = 0; i < rowsOrig.length; i++) {
      ...
      if (options.bAddCustomFields && row.idBoardH && row.idBoardH != IDBOARD_UNKNOWN) {
        if (!boards[row.idBoardH]) boards[row.idBoardH] = true;
      }
      if (row.idCardH) {
        if (!cards[row.idCardH]) cards[row.idCardH] = true;
      }
    }
    ...
    function doneGetExtraFieldsData(status, boardDataIn, cardDataIn) {
      ...
      for (var idBoardShort in boardDataIn) {
        ...
        bf.forEach(function (entry) {
          ...
          if (entry.type == "list") objSet.options = entry.options;
          cfmetaDataOut[entry.id] = objSet;
          var idMap = (objSet.type + "-" + objSet.name.trim()).toLowerCase();
          var foundIdMap = mapFindDups[idMap];
          if (foundIdMap) objSet.idMaster = foundIdMap;
          else mapFindDups[idMap] = entry.id;
          if (bCheckbox) cfmetaDataOut[entry.id].bCheckbox = true;
        });
      }

      var rgCFIds = [];
      for (var idCF in cfmetaDataOut) {
        if (!cfmetaDataOut[idCF].idMaster) rgCFIds.push(idCF);
      }
      rgCFIds.sort(function (a, b) {
        ...
        if (cmp == 0) cmp = cfmetaDataOut[a].type.localeCompare(cfmetaDataOut[b].type);
        return cmp;
      });
          ...
          if (cd.customFieldItems) {
            ...
            cd.customFieldItems.forEach(function (entry) {
              ...
              if (bdata) {
                ...
                var valSet = bdata.bCheckbox
                  ? entry.value && entry.value.checked == "true"
                    ? { number: 1 }
                    : { number: 0 }
                  : entry.value;
                if (bdata.type == "list") {
                ...
                if (valSet) cardDataCur.cf[entry.idCustomField] = valSet;
              }
            });
          }
        } catch (ex) {
      ...
      if (messageErrorLast) alert(messageErrorLast);
    ...
    getBackgroundPage(function (bkPage) {
      var rgBoards = [];
      var boardData = {};
      for (prop in boards) rgBoards.push({ id: prop });
      ...
      function stepCards(status) {
        ...
        for (prop in cards) rgCards.push({ id: prop });
        ...
        function onProcessItem(tokenTrello, item, iitem, postProcessItem) {
          ...
          bkPage.getCardData(
            tokenTrello,
            idCard,
            "id&customFieldItems=true" + (options.bAddMembers ? "&members=true&member_fields=username" : ""),
            false,
            function (response) {
              if (response.status == STATUS_OK) cardData[idCard] = response.card;
      ...
      function stepBoards() {
        ...
        function onProcessItem(tokenTrello, item, iitem, postProcessItem) {
          ...
          bkPage.getBoardData(tokenTrello, false, idBoard, "/customFields", function (response) {
            if (response.status == STATUS_OK) boardData[idBoard] = response.board;
            postProcessItem(response.status, item, iitem);
          });
        }
      }
  ...
  function stepGroup(mapCardsToLabels, extraFieldsData) {
    ...
    if (bGroupMultipleHashtags) splitRowsBy(rowsOrig, "hashtags");
    ...
    if (rowsOrig.length > cRowsOrigBefore)
      sendDesktopNotification(
        "This report has duplicated counts and S/E sums due to grouping of cards with multiple labels or hashtags.",
        8000
      );

    if (groupBy.length > 0 || bHasCFData || bHasExtraData || (orderBy.length > 0 && orderBy != "date"))
      //assumes new rows are only inserted when grouping is set
      rowsGrouped = groupRows(rowsOrig, groupBy, orderBy, bCountCards, extraFieldsData);
    if (rowsGrouped.length > 3000 && bPivotByWeek) {
      //week is the default. else user likely changed it on purpose so dont keep reminding this tip
      const bNoCharts = urlParams["checkNoCharts"] == "true";
      var strAlert = "To speed up this report:";
      strAlert += "\n• Set pivot by 'Year'";

      if (!bNoCharts) strAlert += "\n• Check 'No charts' in Options.";
  ...
  function fillDOM(mapCardsToLabels, extraFieldsData, customColumns, callbackOK) {
    var bShowMonth =
      urlParams["sinceSimple"].toUpperCase() == FILTER_DATE_ADVANCED.toUpperCase() &&
      (urlParams["monthStart"].length > 0 || urlParams["monthEnd"].length > 0);
    var headersSpecial = {};
    g_bReportsUseSystemNumberFormat = !options.bNoSystemNumberFormat;
    var html = getHtmlDrillDownTooltip(
      customColumns,
      rowsGrouped,
      mapCardsToLabels,
      extraFieldsData,
    ...
    if (tableElem.length > 0 && rowsGrouped.length > 0) {
      ...
      if (sortList.length == 0 && orderBy) {
        var elemMatch = $("#orderBy option").filter(function () {
          return $(this).val() == orderBy;
        });
        if (elemMatch.length > 0) {
          var textSort = getCleanHeaderName(elemMatch[0].innerText);
          var ascdesc = 0;
          if (orderBy != "dateDue" && (orderBy == "date" || typeof rowsGrouped[0][orderBy] != "string")) ascdesc = 1;
    ...
    fillPivotTables(
      rowsOrig,
      $(".agile_report_container_byUser"),
      $(".agile_report_container_byBoard"),
      urlParams,
      options.bNoTruncate
    );
...
function namedToIndexedSortList(list, table) {
  ...
  var ret = [];
  list.forEach(function (elem) {
    if (elem.length != 2 || typeof elem[0] != "string") return;
    var iFound = cols[elem[0].toLowerCase()];
    if (iFound === undefined) return;
...
function configureLastViewedRowButton() {
  ...
  chrome.storage.local.get([keyLastSyncViewed], function (obj) {
    ...
    if (rowidLastSync !== undefined && g_rowidLastSyncRemember < 0) g_rowidLastSyncRemember = rowidLastSync; //needed when user already marked all as viewed, so there are no rows.
...
function saveDataChart(rows, urlParams, options) {
  ...
  for (var iProp = 0; iProp < pGroups.length; iProp++) {
    ...
    } else if (pCur == "idTeamH") pGroups[iProp] = "nameTeam";
    else if (pCur == "idBoardH") pGroups[iProp] = "nameBoard";
    else if (pCur == "hashtags") {
      pGroups[iProp] = "hashtagFirst";
    } else bId = false;

    if (pCur.indexOf("date") == 0) bDateGroups = true;
    ...
    if (bId && stackBy != pCur) prependIds.push(pCur);
  ...
  for (var iRow = 0; iRow < rows.length; iRow++) {
    ...
    for (iProp = 0; iProp < pGroups.length; iProp++) {
      ...
      if (propNameLoop == "comment") valProp = removeBracketsInNote(valProp);
      else if (propNameLoop == "nameList" && groupBy == "idBoardH-nameList") {
        if (!mapSortLabels[valProp]) mapSortLabels[valProp] = rowCur.posList;
      }
    ...
    if (bSingleBoard && strNameBoardSingle.length == 0) strNameBoardSingle = rowCur.nameBoard;
    //datasets
    ...
    if (bRemain || rowCur.spent != 0 || (bShowR && rowCur.est != 0)) {
      if (!bRemain && rowCur.spent != 0) {
        ...
        if (rowCur.spent < 0) bHasNegatives = true;
      }
      if (bShowR) {
        var rCalc = parseFixedFloat(rowCur.est - rowCur.spent);
        if (rCalc != 0) {
          if (rCalc < 0) bHasNegatives = true;
...
function bCancelFromAlertLargeSize(domain, bDownloading) {
  ...
  if (domain.length > MAX_BARS) {
    ...
    if (bDownloading) strAlert += "\n" + "Converting a large chart to PNG may fail.";
    strAlert += "\nAre you sure?";
    if (!confirm(strAlert)) return true; //cancel
  }
...
function fillChart(bForce) {
  ...
  var bStacked =
    typeChart == g_chartViews.s ||
    typeChart == g_chartViews.e ||
    typeChart == g_chartViews.r ||
    typeChart == g_chartViews.cardcount;
  if (bStacked) {
    elemStackPre.show();
    elemStack.show();
  } else {
  ...
  if (typeChart == g_chartViews.burndown) spanNoColors.hide();
  else spanNoColors.show();

  if (g_lastChartFilled == typeChart && !bForce) return;

  if (g_bNoGroupChart) $("#chartMessage").text(g_strMessageNoGroupChart);
  ...
  if (typeChart == g_chartViews.burndown) {
    ...
    if (bStacked) chartStacked(typeChart, bForce, callbackCancel);
    else if (typeChart == g_chartViews.ser) chartSER(bForce, callbackCancel);
    else if (typeChart == g_chartViews.e1vse) charte1vse(bForce, callbackCancel);
    else if (typeChart == g_chartViews.echange) charteChange(bForce, callbackCancel);
  }
  if (!bCancel) g_lastChartFilled = typeChart;
}
...
function dlabelRealFromEncoded(dlabel) {
  ...
  for (var iPart = 0; iPart < parts.length; iPart++) {
    ...
    if (valPart == DLABELPART_SKIP) continue;
    if (iPart > 0) dlabelDisplay += g_yFieldSeparator;
...
function getCommonChartParts(elemChart, domain, colorsForScale, legendTexts) {
  ...
  var colorBk =
    g_dataChart.params["checkBGColorChart"] == "true" ? g_dataChart.params["colorChartBackground"] || "#FFFFFF" : null;
  $("#chartPrintContainer").css("background-color", colorBk || "transparent");

  var ret = {};
  ret.colorScale = new Plottable.Scales.Color().domain(legendTexts);
  if (colorsForScale) ret.colorScale.range(colorsForScale);
  if (colorsForScale.length > 1)
    ret.legend = new Plottable.Components.Legend(ret.colorScale)
      .xAlignment("center")
      .yAlignment("top")
      .maxEntriesPerRow(legendTexts.length > g_maxLegendsPerColumn ? g_maxLegendColumns : 1);
  else ret.legend = null;
...
function prependChartTitle(table, dataChart, domain) {
  ...
  var strTitle = "";
  if (dataChart.strNameBoardSingle) strTitle = dataChart.strNameBoardSingle;

  if (domain.length == 1) {
    var strDlabel = dlabelRealFromEncoded(domain[0]);
    if (strDlabel) {
      //adding a break with empty next line causes ... on first line.
...
function clearInteractions() {
  ...
  for (var iInter = 0; iInter < cInter; iInter++) {
    ...
    if (inter.evt.detachFrom) inter.evt.detachFrom(inter.to);
  }
  g_interactions = [];
}
...
function chartSER(bForce, callbackCancel) {
  ...
  if (!g_dataChart.bRemain) {
    ...
    if (g_dataChart.dataR.length == 0) textMessage += "There is no Remain to chart. ";
  }

  if (textMessage) textMessage += "Try the 'Card count' chart from the list above. ";

  if (isReportWithPartialE(g_dataChart.params)) {
    hiliteOnce($("#sinceSimple"));
    hiliteOnce($("#headerOptions"));
    textMessage +=
      "To make this chart, do not filter by Date or click <b>Options</b> to exclude cards with partial Estimates.";
  }
  ...
  var domain = g_dataChart.domains[dname];
  if (callbackCancel(dname)) return;
  ...
  if (bNoColors) chart.attr("stroke", "black");

  if (!g_bPopupMode) chart.animated(!bForce);

  if (!g_dataChart.bRemain) chart.addDataset(new Plottable.Dataset(g_dataChart.dataS, { iColor: 0 }));
  if (g_dataChart.bShowR) chart.addDataset(new Plottable.Dataset(g_dataChart.dataR, { iColor: 1 }));
...
function charte1vse(bForce, callbackCancel) {
  ...
  if (bNoColors) colors = ["#FFFFFF", "#DDDDDD"];
...
function addChartEstWarning(elemChartMessage) {
  if (g_dataChart.params["sinceSimple"] && g_dataChart.params["checkNoPartialE"] !== "true") {
    elemChartMessage.html(
      "Note: This report is filtered by date, which may cause unexpected E,R values. Click <b>Options</b> and Exclude cards with partial estimates."
    );
    hiliteOnce($("#sinceSimple"));
    hiliteOnce($("#headerOptions"));
  }
...
function chartBurndown(bForce) {
  ...
  elemChartMessage.html(
    "<div style='margin-top:1em;'><A href='" +
      url +
      "' target='_blank'><img src=images/chart-sm.png style='vertical-align: middle;margin-right:5px;'/>Burndown chart</A> based on this report filters.</div>"
  );
  return;
}
...
function charteChange(bForce, callbackCancel) {
  ...
  if (bNoColors) colors = ["#DDDDDD", "#000000", "#FFFFFF"];
  ...
  for (var ids = 0; ids < dE1.length; ids++) {
    ...
    if (delta > 0) dPlus.push({ x: parseFixedFloat(delta), y: dE1[ids].y });
    if (delta < 0) dMinus.push({ x: parseFixedFloat(delta), y: dE1[ids].y });
  }
  ...
  var chart = new Plottable.Plots.StackedBar(Plottable.Plots.BarOrientation.horizontal)
    ...
    .addDataset(new Plottable.Dataset(dMinus, { iColor: 2, type: "dMinus" }))
    .labelsEnabled(true)
    .addClass("chartReportStyle");
  chartSetup(chart);
  if (bNoColors) chart.attr("stroke", "black");
  ...
  chart
    ...
    .labelFormatter(function (text, datum, i, ds) {
      var pre = "";
      if (ds) {
        var md = ds.metadata();
        if (md && md.type == "dPlus") pre = "+";
      }
      return pre + text;
    });
...
function getChartZoomFactor() {
  ...
  if (zoom < middle) {
    //-10 to 0, distribute (1-minFactor)
    factor = minFactor + ((1 - minFactor) / zoomMax) * (zoom + zoomMax);
  } else if (zoom > middle) {
    //0 to 10, distribute so 10 maps to 3
...
function getChartHeight(domain, cPartsGroupFinal, dataPrepended, cBars, heightMinDefault) {
  ...
  if (heightMinDefault) heightChartCalc = Math.max(heightMinDefault, heightChartCalc);
...
function chartStacked(type, bForce, callbackCancel) {
  ...
  if (type == g_chartViews.cardcount) {
    ...
    if (dataChart.length == 0) textMessage += "There are no counts to chart. ";
  } else {
    if (type == g_chartViews.s) dataChart = g_dataChart.dataS;
    else if (type == g_chartViews.e) dataChart = g_dataChart.dataE;
    else if (type == g_chartViews.r) dataChart = g_dataChart.dataR;

    assert(dataChart);
    if (dataChart.length == 0) textMessage += "There is no data to chart. ";

    if (type == g_chartViews.s && bOrderRemain) textMessage += "Order by something different than 'R'. ";
  }
  ...
  if (stackBy) {
    ...
    if (iGroupStack < 0 && stackBy == "hashtagFirst")
      //review zig: ugly side-effect of having those two groups (historical note: hashtags added later, hashtagFirst kept for backwards compatibility)
      iGroupStack = pGroups.indexOf("hashtags");

    if (iGroupStack < 0) {
      sendDesktopNotification(
        "Error: the report 'Group by' must contain your stacking. Pick 'Custom' from the group-by dropdown and append it, then Query.",
        10000
      );
      $("#stackBy").val("");
      updateURLPart("stackBy");
    } else if (g_dataChart.iPartGroupExclude >= 0 && g_dataChart.iPartGroupExclude < iGroupStack) iGroupStack--;
  }
  ...
  if (iGroupStack >= 0) {
    ...
    if (!bNoColors) {
      ...
      colors = [
        "#000000", //Black, only for empty property "-"  0
        ...
        if (!mapDlabelsNew[partNew.toLowerCase()]) {
          ...
          if (partNew != "-") {
            if (stackBy == "labels" && !bNoColors) {
              ...
              if (colNew) colors[iColor] = colNew;
            }

            legendTextsInfo.push({ text: partNew, i: iColor });
            iColor++; //move to the next color
            if (iColor >= colors.length) colors.push(colors[((iColor - 1) % (lengthColorsOrig - 1)) + 1]); //remap to 1-20
          }
        }
      } else {
    ...
    if (stackBy != "labels") {
      ...
      legendTextsInfo.sort(function (a, b) {
        ...
        if (at != bt) {
          //force sort
          if (at == legendEmpty) return -1;
          if (bt == legendEmpty) return 1;
        }
        if (mapSort) {
          var aMap = mapSort[at];
          var bMap = mapSort[bt];
          if (typeof aMap !== "undefined" && typeof bMap !== "undefined") return aMap - bMap;
        }
  ...
  var common = getCommonChartParts(elemChart, domain, colors, legendTexts);
  if (bNoColors) common.legend = null;
  ...
  for (var iDS = 0; iDS < dataChart.length; iDS++) {
    ...
    if (!datasets[partNew]) datasets[partNew] = [];

    datasets[partNew].push({
      x: itemCur.x,
      y: prepend + parts.join(g_yFieldSeparator),
      iColor: mapLegendToIColor[partNew.toLowerCase()],
    });
  }
  ...
  for (iSet = 0; iSet < legendTextsInfo.length; iSet++) {
    ...
    if (dsAdd) chart.addDataset(new Plottable.Dataset(dsAdd));
  }
  ...
  function IColorRemapFromD(d) {
    ...
    if (bNoColors && iColor > 0) iColor = 1;
  ...
  if (iGroupStack >= 0) {
    if (!bNoColors && type == g_chartViews.cardcount) {
      chart.attr("stroke", function (d) {
        var iColor = IColorRemapFromD(d);
        if (iColor == 0) return "black"; //make these a little thicker so its clear its the black one
      });
    }
    table = [
  ...
  var heightChartCalc = getChartHeight(
    domain,
    cPartsGroupFinal,
    dataPrepended,
    1,
    getLegendHeightApprox(legendTexts.length)
  );
  elemChart.css("height", heightChartCalc);
  g_chartContainer.renderTo("#chart");
  if (iGroupStack >= 0 && !bNoColors) {
    //when there are too many legend items and few bars, the legend might get cropped.
    var heightLegend = common.legend.requestedSpace(common.legend._width, Infinity).minHeight;
    if (heightLegend > heightChartCalc) elemChart.css("height", heightLegend);
  }
  handleMouseMoveTooltips(elemChart, chart, common, legendTexts);
}
  ...
  pointer.onPointerMove(function (p) {
    ...
    if (closest && closest[0]) {
      ...
      if (closest.datum != null) {
        ...
        var legendText = "";
        if (typeof closest.datum.iColor !== "undefined") iColor = closest.datum.iColor;
        else {
          var md = closest.dataset.metadata();
          if (md) iColor = md.iColor;
        }
        if (typeof iColor !== "undefined") {
          legendText = legendTexts[iColor];
          if (legendText) legendText = escapeHtml(legendText) + "<br >";
        }
...
function fillPivotTables(rows, elemByUser, elemByBoard, urlParams, bNoTruncate) {
  ...
  function handleClickZoom(table) {
    table[0].addEventListener(
      ...
      function (ev) {
        ...
        if (data.bPivotByWeek || data.bPivotByMonth) params["pivotBy"] = PIVOT_BY.day;
        else if (data.bPivotByYear) params["pivotBy"] = PIVOT_BY.month;
        else params["tab"] = 0;

        if (data.bRemoveSimpleDateFilter) params["sinceSimple"] = FILTER_DATE_ADVANCED;
  ...
  function addClickZoom(tdElem, urlParams, replaces, bRemoveSimpleDateFilter, title) {
    title = title || "";
    if (title != "") tdElem.prop("title", title);

    if (bPivotByDate) return; //REVIEW todo
  ...
  for (; iCol < tables.header.length; iCol++) {
    ...
    if (!bPivotByDate) {
      replaces = [
        { name: pivotStart, value: val },
        { name: pivotEnd, value: val },
      ];
      if (val.length > 0) {
  ...
  for (; iRow < tables.byUser.length; iRow++) {
    ...
    for (iCol = 1; iCol < tables.header.length; iCol++) {
      ...
      if (val == 0) tdElem.addClass("agile_pivotCell_Zero");
      trUser.append(tdElem);
      replaces = [
        { name: pivotStart, value: strHeader },
        { name: pivotEnd, value: strHeader },
      ];
      if (bLastRow) {
        //last row
      ...
      if (bPivotByWeek) tdElem.data("agile_week", strHeader);
    }
    elemTableUser.append(trUser);
  }
  ...
  for (iRow = 0; iRow < tables.byBoard.length; iRow++) {
    ...
    if (!bNoTruncate) nameBoard = strTruncate(nameBoard);
    ...
    for (iCol = 1; iCol < tables.header.length; iCol++) {
      ...
      if (bPivotByWeek) tdElem.data("agile_week", strHeader); //used later to detect current week column
    }
    elemTableBoard.append(trBoard);
  }
...
function configAllPivotFormats() {
  if (g_bBuildSqlMode) return;
  configPivotFormat(
    $("#tabs-1 .agile_format_container"),
    g_dataFormatUser,
    $(".agile_report_container_byUser"),
    ITAB_BYUSER
  );
  configPivotFormat(
    $("#tabs-2 .agile_format_container"),
    g_dataFormatBoard,
    $(".agile_report_container_byBoard"),
    ITAB_BYBOARD
  );
...
function calculateTables(rows, pivotBy) {
  ...
  for (; i < rows.length; i++) {
    ...
    } else if (bPivotByYear) pivotCur = "" + dateStart.getFullYear();
  ...
  var rgUserRows = [];
  var rgBoardRows = [];
  for (i in users) rgUserRows.push(users[i]);
  rgUserRows.sort(doSortUser);

  for (i in boards) rgBoardRows.push(boards[i]);
...
function getHtmlDrillDownTooltip(
  customColumns,
  rows,
  mapCardsToLabels,
  extraFieldsData,
  ...
  if (extraFieldsData) cfmetaData = extraFieldsData.cfmetaData || null;
  ...
  function includeCol(strCol) {
    if (strCol.indexOf(CF_PREFIX) == 0) return true;
    if (!bCustomColumns) return true;
  ...
  function pushHeader(name, base, bIsAggregate, bExtended) {
    ...
    if (!bIsAggregate && groupBy != "") {
      for (var iProp = 0; iProp < colData.length; iProp++) {
        if (partsGroup.indexOf(colData[iProp]) >= 0) break; //found
      }
      if (iProp >= colData.length)
        //not found
        name += g_postFixHeaderLast;
    }
    var obj = { name: name };
    if (bExtended) obj.bExtend = true;
    header.push(obj);
  }
  ...
  if (bShowKeyword) pushHeader("Keyword");

  var bRPopup = bOrderR && g_bPopupMode;
  var bShowDate = bCustomColumns ? includeCol("dateString") : !bRPopup || bGroupedByDate;
  if (bShowDate) pushHeader("Date", "dateString");

  if (bCustomColumns) bShowMonth = includeCol("month");
  var bCardGrouping = groupBy.indexOf("idCardH") >= 0;
  var bShowBoard = bCustomColumns
    ? includeCol("board")
    : groupBy == "" || groupBy.indexOf("idBoardH") >= 0 || bCardGrouping;
  ...
  var bShowTeam = bCustomColumns
    ? includeCol("team")
    : groupBy.indexOf("idTeamH") >= 0 || (!g_bPopupMode && bShowBoard);
  ...
  var bShowList = bCustomColumns
    ? includeCol("nameList")
    : (!bRPopup || groupBy.indexOf("nameList") >= 0) &&
      g_bEnableTrelloSync &&
      (groupBy == "" || groupBy.indexOf("nameList") >= 0 || orderBy.indexOf("posList") >= 0 || bShowCard);
  ...
  function pushCardHeader() {
    if (bShowCardShortLink) pushHeader("Card shortLink", "cardShortLink");

    if (bShowCardNumber) pushHeader("Card #", "cardNumber");
    if (bShowCard) {
  ...
  if (bShowWeek) pushHeader("Week", "week");
  ...
  if (bShowCardCount) pushHeader("Card count", "cardCount", true);
  var bGroupByCardOrNone = groupBy == "" || bCardGrouping;
  var bShowArchived = bCustomColumns
    ? includeCol("archived")
    : g_bEnableTrelloSync && bGroupByCardOrNone && archived != "1" && archived != "0";
  var bShowDeleted = bCustomColumns
    ? includeCol("deleted")
    : g_bEnableTrelloSync && bGroupByCardOrNone && deleted != "1" && deleted != "0";
  if (bShowMonth) pushHeader("Month", "month");

  var bShowUser = bCustomColumns ? includeCol("user") : g_bNoSE ? false : groupBy == "" || groupBy.indexOf("user") >= 0;
  if (bShowUser) pushHeader("User");
  ...
  if (bShowList) pushHeader("List", "nameList");
  ...
  if (bShowLabels) pushHeader(bGroupedByLabels ? "Label" : "Labels", "labels");

  if (bShowMembers) pushHeader("Members", "members");
  ...
  if (bShowS) pushHeader("S", "", true);
  if (bShowEFirst) pushHeaderAggregate("E 1ˢᵗ", "e1st");
  if (bShowE) pushHeaderAggregate("E");
  if (bShowRemain) pushHeaderAggregate("R");

  var bShowComment = bCustomColumns
    ? includeCol("note")
    : !g_bNoSE && (groupBy == "" || groupBy.indexOf("comment") >= 0);
  if (bShowComment) pushHeaderExtended("Note");

  var bShowEtype = bCustomColumns ? includeCol("eType") : !g_bNoSE && !g_bNoEst && groupBy == "";

  if (bShowEtype) pushHeader(COLUMNNAME_ETYPE, "eType");

  if (bShowArchived) pushHeader("Archived");

  if (bShowDeleted) pushHeader("Deleted");
  ...
  function callbackRowData(row) {
    if (idCardSelect && row.idCardH == idCardSelect) row.bSelectedRow = true;
    bPushedCard = false;
    if (row.rowid && row.rowid > g_rowidLastSyncRemember)
      //review zig: hacky way so we dont loop the array twice. would be nice if this was outside of view
    ...
    if (bShowKeyword) pushCol({ name: escapeHtml(row.keyword), bNoTruncate: true });
    if (bShowDate) pushCol({ name: bGroupedByDate ? dateString : dateTimeString, bNoTruncate: true });
    if (bShowDueDate) {
    ...
    function pushCardRow() {
      ...
      if (bShowCard) {
        ...
        else urlCard = "https://trello.com/c/" + row.idCardH;

        pushCol({
          name:
            "<A title='Go to Trello card' target='_blank' href='" +
            urlCard +
            "'>" +
            escapeHtml(bNoTruncate ? row.nameCard : strTruncate(row.nameCard)) +
            "</A>",
          bNoTruncate: true,
        });
      }
    ...
    if (bShowCardCount)
      //note if !countCards we must output something if user picked the custom column. 1 makes more sense than 0
      pushCol({ name: String(row.countCards || 1), bNoTruncate: true });

    if (bShowMonth) pushCol({ name: row.month ? row.month : getCurrentMonthFormatted(daterow), bNoTruncate: true });

    if (bShowUser) pushCol({ name: row.user, bNoTruncate: bNoTruncate });
    ...
    if (bShowTeam) {
      ...
      pushCol({
        name:
          "<A title='Go to Trello team' target='_blank' href='" +
          urlTeam +
          "'>" +
          escapeHtml(bNoTruncate ? nameTeam : strTruncate(nameTeam)) +
          "</A>",
        bNoTruncate: true,
      });
    }

    if (bShowBoardShortLink) pushCol({ name: row.idBoardH == IDBOARD_UNKNOWN ? "" : row.idBoardH, bNoTruncate: true });

    if (bShowBoard) {
      var urlBoard = "https://trello.com/b/" + row.idBoardH;
      pushCol({
        name:
          "<A title='Go to Trello board' target='_blank' href='" +
          urlBoard +
          "'>" +
          escapeHtml(bNoTruncate ? row.nameBoard : strTruncate(row.nameBoard)) +
          "</A>",
        bNoTruncate: true,
      });
    }
    ...
    if (bShowList) {
      ...
      if (!bNoTruncate) strListUse = strTruncate(strListUse, g_cchTruncateShort);
    ...
    if (bShowHashtag) {
      ...
      if (bGroupedByFirstHashtag || bGroupedByHashtags) {
        ...
        if (rgHashForCard.length > 0) nameHSF = rgHashForCard[0];
        else nameHSF = "";
      }
    ...
    if (bShowMembers) {
      ...
      if (mapCard) {
        ...
        if (data && data.members) nameMembers = data.members.join(", ");
      }
    ...
    if (bShowS) pushCol({ type: "S", name: sPush, bNoTruncate: true });
    if (bShowEFirst) pushCol({ type: "EFirst", name: parseFixedFloat(row.estFirst), bNoTruncate: true }); //not type "E". that is used when showing sum of row selections
    if (bShowE) pushCol({ type: "E", name: estPush, bNoTruncate: true });

    if (bShowRemain) {
      var remainCalc = parseFixedFloat(row.est - row.spent);
      if (bOrderR && remainCalc == 0) return [];
      pushCol({ type: "R", name: remainCalc, bNoTruncate: true }); //type "R" just so it generates the transparent zero
    }
    if (bShowComment)
      pushCol({
        name: escapeHtml(options.bNoBracketNotes ? removeBracketsInNote(row.comment) : row.comment),
        bNoTruncate: bNoTruncate,
      });

    if (bShowEtype) pushCol({ name: nameFromEType(row.eType), bNoTruncate: true });

    if (bShowArchived) pushCol({ name: row.bArchivedCB > 0 ? "Yes" : "No", bNoTruncate: true });

    if (bShowDeleted) pushCol({ name: row.bDeleted ? "Yes" : "No", bNoTruncate: true });
    ...
    if (!bShowComment) {
      ...
      title += " - " + row.nameCard;
      title += " - " + row.comment;
      if (row.rowid == ROWID_REPORT_CARD) title += "\n(no s/e)";
