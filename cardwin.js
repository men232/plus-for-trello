...
document.addEventListener("DOMContentLoaded", function () {
  ...
  dockOut.attr("src", chrome.runtime.getURL("images/dockoutlarge.png"));
  dockOut.show();
  dockOut.css("cursor", "pointer");
  dockOut.off().click(function () {
  ...
  if (navigator.onLine) loadTrello();
  else {
    elemIframe.append(
      $(
        "<div style='margin-top:3em;padding-left:1em;padding-right:1em;'>Waiting for internet connection to load the card.</div>"
      )
    );
  ...
  function setupIframeWatch() {
    ...
    if (iframe.length == 0) setTimeout(setupIframeWatch, 200);
    else {
