...
var HelpButton = {
  ...
  create: function () {
    ...
    var icon = $("<img>").attr(
      "src",
      chrome.runtime.getURL(bPlusDisplayDisabled ? "images/iconspenthelpwarn.png" : "images/iconspenthelp.png")
    );
    icon.addClass("agile-spent-icon-header agile-main-plus-help-icon");
    icon.attr("title", "Plus Help");
    if (!bPlusDisplayDisabled) b.append(spanTour);
    ...
    configureSsLinks(b);
    setTimeout(function () {
      if (newerStoreVersion()) hiliteOnce(icon, null, null, 3);
    }, 1000);
    return b;
  },
  display: function () {
    ...
    function checkHeader(obj) {
      ...
      var header = $("#header");
      if (header.length == 0) retry();
      else {
        if (header.find("." + obj.strClass).size() == 0) {
          var elemMemberMenu = header.find('[data-testid="header-create-menu-button"]');
          if (elemMemberMenu.length == 0) retry();
          else elemMemberMenu.parent().append(obj.create());
        }
      }
    }
  },
...
function insertPlusFeed(bForce) {
  if (g_intervalCheckPlusFeed != null && !bForce) return;
  ...
  function doGetFeed() {
    ...
    chrome.storage.sync.get(key, function (obj) {
      var data = obj[key];
      if (data !== undefined) stateFeed = data;
      var msNow = Date.now();
      ...
      if (isPlusDisplayDisabled()) msWait = 1000 * 60 * 60 * 0.5; //half hour in case there is a new g+ post about an issue fixed
      if (msNow - stateFeed.msLastQuery > msWait) {
        setTimeout(function () {
          //delay a few seconds because usually happens on trello page load, wait until that settles
          sendExtensionMessage(
            { method: "getPlusFeed", msLastPostRetrieved: stateFeed.msLastPostReadByUser },
            function (response) {
              ...
              if (JSON.stringify(itemsSave).length > 500)
                //prevent huge sync item. should never happen since each item is small and we allow 2 only
                itemsSave = [];
              ...
              if (true) {
                chrome.storage.sync.set(objSave, function () {
                  if (chrome.runtime.lastError === undefined) insertPlusFeedWorker(stateFeed, key);
                });
              }
            }
          );
  ...
  setTimeout(function () {
    doGetFeed();
  }, 1000); //use timeout so icon doesnt jump left after inserted (let plus header breathe)

  if (g_intervalCheckPlusFeed != null) return;
...
function insertPlusFeedWorker(stateFeed, key) {
  ...
  if (stateFeed.msLastPostReadByUser < stateFeed.msLastPostRetrieved) {
    if (msNow - stateFeed.msLastPostRetrieved < dmsOldestShow) {
      if (g_msStartPlusUsage && stateFeed.msLastPostRetrieved < g_msStartPlusUsage) bShowNewIcon = false;
      else bShowNewIcon = true;
    }
  }
  ...
  if (bShowNewIcon || bShowRecentIcon) {
    if (icon.length == 0) {
      ...
      icon.click(function () {
        chrome.storage.sync.get(key, function (obj) {
          ...
          if (
            stateOrig.msLastPostRetrieved != stateFeed.msLastPostRetrieved ||
            stateOrig.msLastPostReadByUser != stateFeed.msLastPostReadByUser ||
            stateFeed.msUserClicked - stateOrig.msUserClicked > 1000 * 60 * 5
          ) {
            //protect sync quota for 5min if only msUserClicked changed
            var objSave = {};
            stateFeed.items = [];
            objSave[key] = stateFeed;
            chrome.storage.sync.set(objSave, function () {});
        ...
        icon.attr("src", chrome.runtime.getURL(pathImgRecent));
    ...
    icon.attr("src", chrome.runtime.getURL(bShowNewIcon ? "images/new.png" : pathImgRecent));
    var iitem = 0;
